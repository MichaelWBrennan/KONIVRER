    ...
    try {
        ...
        for (const record of records) {
            ...
            const milestoneCategories = {
                'profile_setup': ['profile_created', 'profile_completed', 'avatar_uploaded'],
                'first_interaction': ['first_login', 'dashboard_viewed', 'first_click'],
                'feature_discovery': ['feature_tutorial_started', 'feature_tutorial_completed', 'tooltip_viewed'],
                'integration_setup': ['integration_connected', 'webhook_configured', 'api_key_created'],
                'first_success': ['first_merge', 'first_deployment', 'first_collaboration'],
                'advanced_features': ['advanced_feature_used', 'custom_workflow_created', 'automation_enabled']
            };
...
async function calculateCategoryProgress(userId, category, milestones) {
    try {
        ...
        const params = {
            TableName: process.env.TABLE_NAME,
            FilterExpression: 'begins_with(configKey, :prefix) AND contains(configKey, :userId)',
        ...
        const completedMilestones = result.Items
            .map(item => item.data.milestoneType)
            .filter(milestone => milestones.includes(milestone));
        
        return {
            completed: completedMilestones.length,
            total: milestones.length,
            percentage: Math.round((completedMilestones.length / milestones.length) * 100),
...
async function updateUserJourney(userId, onboardingEvent) {
    try {
        ...
        let journeyData = { stages: [], currentStage: 'getting_started', completedMilestones: [] };
        ...
        journeyData.currentStage = determineCurrentStage(journeyData.completedMilestones);
...
function determineCurrentStage(completedMilestones) {
    ...
    if (milestoneTypes.includes('advanced_feature_used')) {
        return 'power_user';
    } else if (milestoneTypes.includes('first_merge') || milestoneTypes.includes('first_deployment')) {
