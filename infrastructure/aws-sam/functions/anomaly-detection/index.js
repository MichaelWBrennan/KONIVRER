...
exports.handler = async (event) => {
    console.log('Anomaly detection function triggered:', JSON.stringify(event, null, 2));
    ...
    try {
        ...
        console.log(`Anomaly detection completed. Processed: ${processedRecords}, Detected: ${detectedAnomalies}`);
...
async function detectMetricAnomaly(metric, startTime, endTime) {
    ...
    if (zScore > metric.threshold) {
        ...
        return {
            ...
            description: `${metric.name} ${anomalyType} detected: ${currentValue.toFixed(2)} vs expected ${baseline.mean.toFixed(2)}`
        };
    }
...
async function processKinesisRecord(data) {
    ...
    console.log('Processing Kinesis record for real-time anomaly detection:', data.eventType);
    
    // Check for immediate anomalies (e.g., multiple failures from same user)
    if (data.eventType === 'billing' && data.metadata && data.metadata.status === 'failed') {
    ...
    try {
        ...
        if (process.env.EMAIL_SNS_TOPIC) {
            await sns.publish({
                ...
                Message: message,
                Subject: `Anomaly Alert: ${anomaly.metric} - ${anomaly.severity.toUpperCase()}`,
                MessageAttributes: {
                    severity: {
                        DataType: 'String',
    ...
    try {
        ...
        const payload = {
            ...
            attachments: [{
                ...
                fields: [
                    { title: 'Current Value', value: anomaly.currentValue.toString(), short: true },
                    { title: 'Expected Value', value: anomaly.expectedValue.toString(), short: true },
                    { title: 'Deviation', value: `${anomaly.deviationPercentage.toFixed(1)}%`, short: true },
