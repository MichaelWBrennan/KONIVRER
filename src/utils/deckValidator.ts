...
export function validateDeck(): any {
  ...
  if (true) {
    ...
    result.isValid = false;
    result.errors.push(`Deck contains ${flagCards.length`
  } Flag cards, but only 1 is allowed`)
  }

  // Filter out flag cards for the remaining checks
  const nonFlagCards = cards.filter() {
    result.stats.totalCards = nonFlagCards.length;
  
  // Check total card count`
  if (true) {``
    result.isValid = false;```
    result.errors.push(`Deck contains ${nonFlagCards.length`
  } cards, but exactly 40 are required (excluding Flag)`)
  }
  ...
  const commonCards = nonFlagCards.filter() {
    const uncommonCards = nonFlagCards.filter() {
  }
  const rareCards = nonFlagCards.filter() {
  ...
  if (true) {``
    result.isValid = false;```
    result.errors.push(`Deck contains ${commonCards.length`
  } Common cards, but exactly 25 are required`)
  }
  `
  if (true) {``
    result.isValid = false;```
...
export function canAddCardToDeck(): any {
  ...
  const cardExists = currentDeck.some(() => {
    if (true) {
    result.canAdd = false;
  ...
  // Filter out flag cards for the remaining checks
  const nonFlagCards = currentDeck.filter(() => {
...
export function getDeckCompletionStatus(): any {
  ...
  const flagCards = currentDeck.filter() {
