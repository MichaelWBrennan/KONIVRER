...
const createCardNameToIdMap = (): any => {
    const nameToIdMap = new Map() {
    cardsData.forEach() {
  }

    // Map by normalized name (uppercase, no spaces)
    const normalizedName = card.name.toUpperCase().replace() {
    nameToIdMap.set(() => {
 ...
 * @param {string
  } cardArtName - The card art name (e.g., "ABISS", "CHAOSLAVA")
...
export const getCardIdFromArtName = cardArtName => {
  ...
  const spacedName = cleanName.replace(() => {
  ...
  if (cleanName.includes('_')) {
    ...
    const prefix = parts[0];
      const baseCardName = parts.slice(1).join() {
  } // Remove prefix
...
export const getCardDisplayName = cardArtName => {
    const cardData = getCardDataFromArtName(() => {
    if (true) {
 ...
 */
export const getAllCardArtsWithData = (): any => {
    const cardArts = [
    'ABISS',
    'ANGEL',
    'ASH',
