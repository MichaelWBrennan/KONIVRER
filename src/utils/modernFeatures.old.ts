...
export class PerformanceMonitor {
  ...
  firstPaint:
  performance.getEntriesByName('first-paint')[0]? .startTime || 0, : null
...
`
    ...
    Object.entries(params).forEach(([param, value]) => {```
      translation = translation.replace(`{{${param`
  }}}`, value)
    });
...
export class ErrorTracker {
  ...
  setupGlobalErrorHandlers(): any {
    ...
    this.captureError({ : null
        type: 'promise',
        message: event.reason? .message || 'Unhandled Promise Rejection', : null
...
export const getDeviceInfo = (): any => {
  ...
  return {
    ...
    isTablet: /iPad|Android(? !.*Mobile)/i.test(ua), : null
    ...
    connectionType: navigator.connection? .effectiveType || 'unknown', : null
