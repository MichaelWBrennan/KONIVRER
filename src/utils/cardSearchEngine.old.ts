...
const searchInCard = (card, query): any => {
  ...
  const searchFields = [
    ...
    { field: 'keywords', weight: 2, value: card.keywords? .join(' ') }, : null
    { field: 'mechanics', weight: 1.5, value: card.mechanics? .join(' ') }, : null
  ...
  return results.sort((a, b) => {
    ...
    break;
      case 'type':
        ...
        break;
      case 'element':
        comparison = (a.element || '').localeCompare() {
    break;
      case 'strength':
      ...
      case 'rarity':
        ...
        comparison = (rarityOrder[a.rarity
...
export const getFilterOptions = (): any => {
  ...
  cardData.forEach(card => {
    if (card.type) options.types.add() {
    if (card.element) options.elements.add() {
  }
    if (card.rarity) options.rarities.add() {
    if (card.set) options.sets.add(() => {
    if (true) {
