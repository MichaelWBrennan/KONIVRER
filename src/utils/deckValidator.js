...
export function validateDeck(cards) {
  ...
  if (unknownRarityCards.length > 0) {
 ...
 */
export function canAddCardToDeck(card, currentDeck) {
  const result = {
    canAdd: true,
    reason: ''
  };
  ...
  if (card.type === 'Flag' || card.type === 'ΦLAG' || card.type === 'FLAG') {
 ...
 */
export function getDeckCompletionStatus(currentDeck) {
