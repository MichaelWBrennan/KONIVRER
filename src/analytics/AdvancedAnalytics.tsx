...
function renderOverview(
  ...
  const arc = d3
    .arc<d3.PieArcDatum<DeckPerformance>>()
    .innerRadius(0)
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `${data.playerStats.averageGameLength.toFixed(1)} min`,
    },
  ];
...
function renderDeckAnalysis(
  ...
  const deckMatches = data.matchHistory.filter(
    match => match.deckUsed === selectedDeckId,
  );
  ...
  const matchupScale = d3.scaleLinear().domain([0, 1]).range([0, 200]);
...
function aggregatePerformanceByWeek(
  matches: MatchResult[],
): { week: Date; winRate: number }[] {
