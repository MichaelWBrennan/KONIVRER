...
export class RankingEngine {
  ...
  processGameResult(
    ...
    this.playerData.experienceLevel = Math.min(
      100,
      this.playerData.experienceLevel + 1,
    );
  ...
  applyTimeWeightedAdjustment(ratingChange, gameResult) {
    if (
      ...
      this.playerData.recentPerformance.length === 0
    ) {
      return ratingChange;
    }
  ...
  updatePlaystyleData(playstyleMetrics) {
    ...
    Object.keys(this.playerData.playstyle).forEach(key => {
      this.playerData.playstyle[key] = Math.min(
        1.0,
  ...
  updatePlayerStats(gameResult, mmrChange, gameDuration) {
    ...
    if (this.playerData.isPlacement) {
      ...
      if (
        this.playerData.placementMatches >= this.eloConstants.provisionalGames
      ) {
  ...
  getTierFromSkill(
    ...
    for (const [tierKey, tierData] of Object.entries(this.tiers)) {
      if (
        conservativeRating >= tierData.skillRange[0] &&
        conservativeRating < tierData.skillRange[1]
      ) {
        ...
        for (const [bandKey, bandData] of Object.entries(tierData.bands)) {
          if (
            confidence >= bandData.confidenceRange[0] &&
            confidence < bandData.confidenceRange[1]
          ) {
  ...
  calculateMatchQuality(opponentData) {
    ...
    if (
      this.options.enableConfidenceBasedMatching &&
      this.matchmaking.confidenceMatching.enabled
    ) {
    ...
    if (
      this.options.enableMultiFactorMatchmaking &&
    ...
    if (
      this.options.enablePlaystyleCompatibility &&
    ...
    if (
      this.options.enableTimeWeightedPerformance &&
  ...
  calculateDeckArchetypeCompatibility(playerArchetype, opponentArchetype) {
    ...
    if (
      this.deckMatchups[playerArchetype] &&
      this.deckMatchups[playerArchetype][opponentArchetype]
    ) {
    ...
    if (
      this.playstyleCompatibility[playerArchetype] &&
      this.playstyleCompatibility[playerArchetype][opponentArchetype]
    ) {
  ...
  calculatePlaystyleCompatibility(playerPlaystyle, opponentPlaystyle) {
    ...
    const aggressionComplement =
      1 -
      Math.abs(playerPlaystyle.aggression + opponentPlaystyle.aggression - 1);
    const riskTakingComplement =
      1 -
      Math.abs(playerPlaystyle.riskTaking + opponentPlaystyle.riskTaking - 1);
  ...
  calculatePreferencesScore(opponentData) {
    ...
    if (
      preferences.preferredArchetypes &&
      preferences.preferredArchetypes.length > 0 &&
      opponentData.deckArchetype
    ) {
      if (
    ...
    if (
      preferences.preferredOpponents &&
      preferences.preferredOpponents.length > 0 &&
      opponentData.id
    ) {
