...
export class PersonalityManager {
  constructor(personalityKey = 'scholar') {
   ...
   */
  updateMood(gameEvent) {
    switch (gameEvent.type) {
      case 'good_play':
        this.moodModifier = Math.min(1, this.moodModifier + 0.1);
        break;
      case 'bad_play':
        this.moodModifier = Math.max(-1, this.moodModifier - 0.2);
        break;
      case 'winning':
        this.moodModifier = Math.min(1, this.moodModifier + 0.15);
        break;
      case 'losing':
        this.moodModifier = Math.max(-1, this.moodModifier - 0.25);
        break;
      case 'surprised':
   ...
   */
  getThinkingTime(complexity = 1, confidence = 0.5) {
   ...
   */
  getPowerPreference(availablePower, situation) {
    ...
    for (const power of availablePower) {
      ...
      if (situation.needsAggression && traits.aggression > 0.6) {
        score += power * 0.1; // Prefer higher power when aggressive
      }
      
      if (situation.needsEfficiency && traits.resourceConservation > 0.6) {
   ...
   */
  adaptToPlayer(playerBehavior) {
    // Increase adaptation level over time
    this.adaptationLevel = Math.min(5, this.adaptationLevel + 0.1);
    
    // Counter-adapt to player strategies
    if (playerBehavior.aggression > 0.7 && this.currentPersonality.traits.aggression < 0.5) {
      // Player is very aggressive, become more defensive
      this.moodModifier -= 0.1;
    }
    
    if (playerBehavior.resourceConservation > 0.8) {
