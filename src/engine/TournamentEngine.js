...
export class TournamentEngine {
  ...
  adaptTournamentStructure(tournament) {
    ...
    if (
      ...
      playerCount >= 16 &&
      tournament.timeConstraints.isTimeLimited
    ) {
  ...
  createPairings() {
    ...
    switch (this.formats[format].pairingMethod) {
      ...
      case 'bracket':
        pairings = this.createBracketPairings(
          activePlayers,
          round,
          this.formats[format].eliminationType,
        );
        break;
      case 'roundRobin':
    ...
    matches.forEach(match => {
      if (match.bracket === 'main') {
        ...
        tournament.brackets.consolation
      ) {
  ...
  createRoundRobinPairings(players, round) {
    ...
    if (round > totalRounds) {
      throw new Error(
        `Round ${round} exceeds maximum rounds (${totalRounds}) for round robin`,
      );
    }
  ...
  calculateMetaBonus(player1, player2) {
    ...
    if (
      player1.deckArchetype &&
      player2.deckArchetype &&
