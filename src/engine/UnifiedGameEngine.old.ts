...
export class UnifiedGameEngine {
  ...
  private handleDeckOut(playerId: string): void {
    ...
    this.logGameEvent(`${player.name} has no more cards in their deck!`);
  ...
  public nextPhase(): void {
    ...
    this.logGameEvent(`${activePlayer.name}'s ${currentPhase} phase ended. Starting ${this.state.phase} phase.`);
  ...
  public endTurn(): void {
    ...
    this.logGameEvent(`${activePlayer.name}'s turn ended. Starting ${newActivePlayer.name}'s turn ${this.state.turn}.`);
  ...
  public playCard(playerId: string, cardId: string, targets: string[] = []): boolean {
    ...
    if (!player.isActive || this.state.phase !== 'main') {
      this.logGameEvent(`${player.name} cannot play a card now`);
      return false;
    }
    ...
    if (cardIndex === -1) {
      this.logGameEvent(`${player.name} tried to play a card that is not in their hand`);
      return false;
    }
    ...
    switch (card.type) {
      case 'Familiar':
        ...
        this.logGameEvent(`${player.name} played ${card.name}`);
        break;
      ...
      case 'Spell':
        ...
        this.logGameEvent(`${player.name} cast ${card.name}`);
        break;
   ...
   */
  public activateAbility(playerId: string, cardId: string, abilityIndex: number, targets: string[] = []): boolean {
    const player = this.getPlayerById(playerId);
    if (!player) return false;
    
    // Find the card in play
    const card = this.findCardInPlay(playerId, cardId);
    if (!card) {
      this.logGameEvent(`${player.name} tried to activate an ability on a card that is not in play`);
      return false;
    }
    
    // Check if the card has the ability
    if (!card.abilities || !card.abilities[abilityIndex]) {
      this.logGameEvent(`${player.name} tried to activate an ability that does not exist`);
      return false;
    }
    ...
    if (ability.cost && !this.canPayCost(playerId, { elements: ability.cost } as Card)) {
      this.logGameEvent(`${player.name} cannot pay the cost for ${card.name}'s ability`);
      return false;
    }
    
    // Check if the card is rested (for activated abilities)
    if (card.rested && !ability.trigger) {
      this.logGameEvent(`${player.name} cannot activate ${card.name}'s ability because it is rested`);
      return false;
    }
    
    // Pay the ability cost
    if (ability.cost) {
      this.payCost(playerId, { elements: ability.cost } as Card);
    }
    ...
    this.resolveAbilityEffect(playerId, card, ability, targets);
    
    this.logGameEvent(`${player.name} activated ${card.name}'s ability`);
    
    // Emit ability activated event
    this.emitEvent('abilityActivated', { 
      playerId, 
      card, 
      ability, 
      targets 
  ...
  public attack(playerId: string, cardId: string, targetId?: string): boolean {
    ...
    if (!player.isActive || this.state.phase !== 'combat') {
      this.logGameEvent(`${player.name} cannot attack now`);
      return false;
    }
    ...
    if (cardIndex === -1) {
      this.logGameEvent(`${player.name} tried to attack with a card that is not in play`);
      return false;
    }
    ...
    if (card.type !== 'Familiar') {
      this.logGameEvent(`${player.name} tried to attack with a non-Familiar card`);
      return false;
    }
    
    // Check if the card is rested
    if (card.rested) {
      this.logGameEvent(`${player.name} cannot attack with ${card.name} because it is rested`);
      return false;
    }
    
    // Move the card to the combat zone
    player.field.splice(cardIndex, 1);
    player.combat.push(card);
    ...
    if (!targetId) {
      ...
      this.dealDamage(opponent.id, card.strength || 0);
      
      this.logGameEvent(`${player.name} attacked ${opponent.name} directly with ${card.name} for ${card.strength} damage`);
    } else {
      // Find the target card
      const target = this.findCardInPlay(undefined, targetId);
      if (!target) {
        this.logGameEvent(`${player.name} tried to attack an invalid target`);
        return false;
      }
      
      // Resolve combat between the two cards
      this.resolveCombat(card, target);
      
      this.logGameEvent(`${player.name}'s ${card.name} attacked ${target.name}`);
    }
  ...
  public block(playerId: string, cardId: string, attackerId: string): boolean {
    ...
    if (this.state.phase !== 'combat') {
      this.logGameEvent(`${player.name} cannot block now`);
      return false;
    }
    ...
    const cardIndex = player.field.findIndex(card => card.id === cardId);
    if (cardIndex === -1) {
      this.logGameEvent(`${player.name} tried to block with a card that is not in play`);
      return false;
    }
    ...
    // Check if the card is a Familiar
    if (card.type !== 'Familiar') {
      this.logGameEvent(`${player.name} tried to block with a non-Familiar card`);
      return false;
    }
    
    // Check if the card is rested
    if (card.rested) {
      this.logGameEvent(`${player.name} cannot block with ${card.name} because it is rested`);
      return false;
    }
    
    // Find the attacking card
    const attacker = this.findCardInPlay(undefined, attackerId);
    if (!attacker) {
      this.logGameEvent(`${player.name} tried to block an invalid attacker`);
      return false;
    }
    ...
    // Resolve combat between the two cards
    this.resolveCombat(attacker, card);
    
    this.logGameEvent(`${player.name} blocked with ${card.name}`);
  ...
  private resolveCombat(attacker: Card, blocker: Card): void {
    ...
    const attackerHasFirstStrike = this.hasKeyword(attacker, 'first_strike');
    const blockerHasFirstStrike = this.hasKeyword(blocker, 'first_strike');
    ...
    if (this.hasKeyword(attacker, 'trample') && attacker.strength! > blocker.health!) {
      ...
      if (blockerController) {
        this.dealDamage(blockerController.id, excessDamage);
  ...
  private dealCardDamage(source: Card, target: Card): void {
    ...
    if (this.hasKeyword(target, 'shield')) {
      ...
      this.removeKeyword(target, 'shield');
      this.logGameEvent(`${target.name}'s shield absorbed the damage`);
      return;
    }
    ...
    this.logGameEvent(`${source.name} dealt ${damage} damage to ${target.name}`);
  ...
  private destroyCard(card: Card): void {
    ...
    let removed = removeFromZone(controller.field);
    if (!removed) {
    ...
    if (removed) {
      ...
      controller.discard.push(card);
      
      this.logGameEvent(`${card.name} was destroyed`);
  ...
  private defeatPlayer(playerId: string): void {
    ...
    this.logGameEvent(`${player.name} was defeated`);
    
    // Check if the game is over
    const remainingPlayers = this.state.players.filter(p => p.lifePoints > 0);
    
    if (remainingPlayers.length === 1) {
      // One player remains, they win
      this.endGame(remainingPlayers[0].id, 'Last player standing');
  ...
  private resolveSpellEffect(playerId: string, card: Card, targets: string[]): void {
    // This would be more complex in a real implementation
    // For now, just log it
    this.logGameEvent(`Resolving spell effect for ${card.name}`);
   ...
   */
  private resolveAbilityEffect(playerId: string, card: Card, ability: Card['abilities'][0], targets: string[]): void {
    // This would be more complex in a real implementation
    // For now, just log it
    this.logGameEvent(`Resolving ability effect for ${card.name}`);
  ...
  private removeKeyword(card: Card, keyword: Keyword): void {
    ...
    if (card.keywords) {
      const index = card.keywords.indexOf(keyword);
      if (index !== -1) {
    ...
    if (card.modifiers && card.modifiers.keywords) {
      const index = card.modifiers.keywords.indexOf(keyword);
      if (index !== -1) {
  ...
  private handleAITurn(playerId: string): void {
    ...
    this.logGameEvent(`AI player ${player.name} is thinking...`);
    
    // Get AI decisions
    const decisions = this.aiEngine.getDecisions(this.state, playerId);
    ...
    // End turn
    this.nextPhase(); // Main phase
    this.nextPhase(); // Combat phase
    this.nextPhase(); // End phase
  }
...
class AIDecisionEngine {
  ...
  private getMainPhaseDecisions(state: GameState, player: Player): AIDecision[] {
    ...
    if (!player.hasPlayedAzoth) {
      const azothCards = player.hand.filter(card => card.type === 'Azoth');
    ...
    for (const card of player.hand) {
      ...
      switch (card.type) {
        case 'Familiar':
          ...
          reasoning = `Play ${card.name} to build board presence`;
          break;
  ...
  private getResponsePhaseDecisions(state: GameState, player: Player): AIDecision[] {
    ...
    if (state.lastAction && state.lastAction.type === 'attack' && state.lastAction.playerId === opponent.id) {
      ...
      const attackingCard = opponent.combat.find(card => card.id === attackingCardId);
