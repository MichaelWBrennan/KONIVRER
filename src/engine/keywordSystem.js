 ...
 * @returns {Object} Updated game state
 */
function applyAmalgamEffect(gameState, playerId, card, playMethod) {
 ...
 * @returns {Object} Updated game state
 */
function applyQuintessenceEffect(gameState, playerId, card, playMethod) {
 ...
 */
function getElementUsedForCard(gameState, playerId, card, elementType) {
  // This would track how much of each element was spent on the card
  // For now, return the base element cost or 1 as default
  if (card.elementsUsed && card.elementsUsed[elementType]) {
 ...
 */
function getBrillianceValidTargets(gameState, maxStrength) {
  const validTargets = [];
 ...
 * @returns {Array} Valid targets
 */
function getGustValidTargets(gameState, maxStrength) {
  const validTargets = [];
 ...
 * @returns {Array} Valid targets
 */
function getSubmergedValidTargets(gameState, maxStrength) {
  const validTargets = [];
 ...
 * @returns {Array} Valid targets
 */
function getVoidValidTargets(gameState) {
  const validTargets = [];
 ...
 */
function executeBrillianceTarget(gameState, targetCard, targetPlayerIndex, targetZone) {
 ...
 */
function executeGustTarget(gameState, targetCard, targetPlayerIndex, targetZone) {
  const targetPlayer = gameState.players[`player${targetPlayerIndex + 1}`];
 ...
 */
function executeSubmergedTarget(gameState, targetCard, targetPlayerIndex, targetZone) {
  const targetPlayer = gameState.players[`player${targetPlayerIndex + 1}`];
 ...
 */
function executeVoidTarget(gameState, targetCard, targetPlayerIndex, targetZone) {
  const targetPlayer = gameState.players[`player${targetPlayerIndex + 1}`];
 ...
 */
function removeCardFromZone(player, card, zone) {
