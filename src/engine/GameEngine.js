...
class GameEngine {
  ...
  startGame() {
    ...
    this.addToGameLog('phase', `${this.getCurrentPlayer().name}'s turn 1 has begun`);
  ...
  summonFamiliar(playerId, cardId, azothPaid) {
    ...
    // Check if in the correct phase
    if (this.gameState.phase !== 'main' && this.gameState.phase !== 'post-combat') {
      throw new Error('Can only summon Familiars during Main or Post-Combat phase');
    }
    ...
    // Log the action
    this.addToGameLog('summon', `${player.name} summoned ${card.name} with ${strengthPaid} strength counters`);
  ...
  declareBlock(playerId, blockers) {
    ...
    blockers.forEach(({ blocker: blockerId, attacker: attackerId }) => {
      ...
      const attackerCard = attacker.field.find(card => card.id === attackerId && card.attacking);
      if (!attackerCard) {
  ...
  resolveCombatDamage() {
    ...
    attackingCreatures.forEach(attackingCard => {
      if (attackingCard.blockedBy) {
        ...
        const blockerCard = defender.field.find(card => card.id === attackingCard.blockedBy);
        ...
        this.addToGameLog('damage', `${attackingCard.name} and ${blockerCard.name} dealt damage to each other`);
        
        // Check if creatures are destroyed
        if (blockerCard.damage >= blockerCard.toughness) {
          this.destroyCreature(1 - this.gameState.currentPlayer, blockerCard.id);
        }
        ...
        this.dealDamageToPlayer(1 - this.gameState.currentPlayer, attackingCard.power);
        this.addToGameLog('damage', `${attackingCard.name} dealt ${attackingCard.power} damage to ${defender.name}`);
      }
    });
  ...
  dealDamageToPlayer(playerId, amount) {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      if (lifeCard.abilities && lifeCard.abilities.some(ability => ability.type === 'burst')) {
        // Play the card for free
        this.addToGameLog('burst', `${player.name} revealed ${lifeCard.name} with Burst!`);
        ...
        this.addToGameLog('damage', `${player.name} drew ${lifeCard.name} from life cards`);
      }
    }
  ...
  activateAbility(playerId, cardId, abilityIndex, targets = []) {
    ...
    this.addToGameLog('ability', `${player.name} activated ${card.name}'s ability`);
  ...
  passPriority(playerId) {
    ...
    this.addToGameLog('pass', `${this.getPlayerById(playerId).name} passed priority`);
  ...
  endTurn(playerId) {
    ...
    this.addToGameLog('turn', `${this.getCurrentPlayer().name}'s turn ${this.gameState.turn} has begun`);
  ...
  resolveStack() {
    ...
    switch (stackItem.type) {
      ...
      case 'ability':
        ...
        this.addToGameLog('resolve', `${stackItem.card.name}'s ability resolved`);
        break;
      ...
      case 'burst':
        ...
        this.addToGameLog('resolve', `${stackItem.card.name}'s burst ability resolved`);
        break;
    }
  ...
  resolveAbilityEffect(stackItem) {
    ...
    // For now, we'll just log that it resolved
    console.log(`Resolving ability: ${stackItem.card.name} - ${stackItem.ability.text}`);
  ...
  startPhase() {
    ...
    switch (this.gameState.phase) {
      ...
      case 'main':
        this.addToGameLog('phase', 'Main Phase - play cards and summon Familiars');
        break;
  ...
  verifyAzothPayment(player, card, azothPaid) {
    ...
    if (azothPaid.length < card.cost) {
      throw new Error(`Not enough Azoth paid. Required: ${card.cost}, Paid: ${azothPaid.length}`);
    }
