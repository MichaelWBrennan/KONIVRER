...
class AIDecisionEngine {
   ...
   */
  async planTurn(gameState, analysis) {
   ...
   */
  generateAvailableActions(gameState) {
    const aiPlayer = gameState.players.ai;
    const actions = [];
    ...
    aiPlayer.hand.forEach((card, index) => {
      if (this.canPlayCard(card, aiPlayer)) {
        ...
        if (card.type === 'ELEMENTAL' && card.genericCost > 0) {
          ...
          for (let cost = card.genericCost; cost <= maxCost; cost++) {
            actions.push({
              type: 'play_card',
              card,
              cardIndex: index,
   ...
   */
  async prioritizeActions(actions, analysis) {
   ...
   */
  async evaluateAction(action, analysis) {
    const scores = {
   ...
   */
  async executeTurnPlan(turnPlan, gameState) {
   ...
   */
  async executeCardPlay(action, gameState) {
    const { card, cardIndex, method, genericCost } = action;
    
    if (method === 'summon' && card.type === 'ELEMENTAL') {
   ...
   */
  findOptimalFieldSlot(gameState, card) {
    const aiField = gameState.players.ai.field;
   ...
   */
  async addThinkingPause(action) {
    let pauseTime = 500; // Base pause
    
    // Longer pauses for complex decisions
    if (action.type === 'play_card' && action.genericCost > 2) {
      pauseTime += 1000;
    }
    
    if (action.evaluation && action.evaluation.confidence < 0.6) {
   ...
   */
  learnFromTurn(gameState, turnPlan) {
  ...
  calculateMaxAffordableCost(card, player) {
