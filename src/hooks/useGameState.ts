...
const createInitialZones : any : any : any : any = (): Record<string, GameZone> => ({
  hand: {
    id: 'hand',
    name: 'Hand',
...
const createInitialPlayer : any : any : any : any = (id: string, name: string): PlayerState => ({
  id,
  name,
...
export const useGameState : any : any : any : any = () => {
  ...
  const [selectedCards, setSelectedCards] : any : any : any : any = useState<Card[]>([]);
  ...
  const selectCard : any : any : any : any = useCallback((card: Card) => {
    setGameState(prev => {
      const newState : any : any : any : any = { ...prev };
      // Find and update the card's selection state
      for (const player of newState.players) {
        for (const zone of Object.values(player.zones)) {
          const cardIndex : any : any : any : any = zone.cards.findIndex(c => c.id === card.id);
          if (cardIndex !== -1) {
  ...
  const doubleClickCard : any : any : any : any = useCallback((card: Card) => {
  ...
  const rightClickCard : any : any : any : any = useCallback((card: Card) => {
  ...
  const startDrag : any : any : any : any = useCallback((card: Card, position: { x: number; y: number }) => {
    // Determine valid drop zones based on card type and current zone
    const validDropZones: KonivrverZoneType[] : any : any : any : any = ['field', 'removedFromPlay', 'hand'];
    const cardType : any : any : any : any = card.type || card.lesserType;
    if (cardType && (cardType.toLowerCase().includes('instant') || cardType.toLowerCase().includes('sorcery'))) {
      validDropZones.push('stack');
    }

    const sourceZone : any : any : any : any = findCardZone(card.id);
    setDragState({
      isDragging: true,
      draggedCard: card,
      dragOffset: position,
  ...
  const findCardZone : any : any : any : any = useCallback((cardId: string): string | undefined => {
  ...
  const moveCard : any : any : any : any = useCallback((cardId: string, targetZoneId: string, playerIndex: number = 0) => {
    // Type guard to check if targetZoneId is a valid KonivrverZoneType
    const isValidZoneType : any : any : any : any = (zoneId: string): zoneId is KonivrverZoneType => {
      const validZones: KonivrverZoneType[] : any : any : any : any = [
    ...
    setGameState(prev => {
      ...
      for (const player of newState.players) {
        for (const [, zone] of Object.entries(player.zones)) {
          const cardIndex : any : any : any : any = zone.cards.findIndex(c => c.id === cardId);
          if (cardIndex !== -1) {
      ...
      if (sourceCard && isValidZoneType(targetZoneId) && newState.players[playerIndex].zones[targetZoneId]) {
        ...
        const resetCard : any : any : any : any = { 
          ...sourceCard, 
          isSelected: false, 
  ...
  const handleZoneDrop : any : any : any : any = useCallback((zoneId: string) => {
    if (dragState.draggedCard) {
      moveCard(dragState.draggedCard.id, zoneId);
    }
  }, [dragState.draggedCard, moveCard]);

  const nextTurn : any : any : any : any = useCallback(() => {
    setGameState(prev => ({
      ...prev,
  ...
  const nextPhase : any : any : any : any = useCallback(() => {
    ...
    const currentIndex : any : any : any : any = phases.indexOf(gameState.phase);
    const nextIndex : any : any : any : any = (currentIndex + 1) % phases.length;
