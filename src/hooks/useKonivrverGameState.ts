...
const createKonivrverZones  : any : any : any : any = (): Record<KonivrverZoneType, GameZone> => ({
  field: {
    id: 'field',
    name: 'Field',
    cards: [],
...
const createKonivrverPlayer  : any : any : any : any = (id: string, name: string): PlayerState => ({
  id,
  name,
...
export const useKonivrverGameState  : any : any : any : any = () => {
  // Initialize KONIVRER game state
  const [gameState, setGameState]  : any : any : any : any = useState<GameState>(() => ({
    players: [
  ...
  const [dragState, setDragState]  : any : any : any : any = useState<DragState>({
    isDragging: false,
    dragOffset: { x: 0, y: 0 },
    validDropZones: []
  });
  ...
  const getCurrentPlayerLife  : any : any : any : any = useCallback(() => {
    const currentPlayer  : any : any : any : any = gameState.players[gameState.currentPlayer];
    return currentPlayer.zones.lifeCards?.cards.length || 0;
  }, [gameState.currentPlayer, gameState.players]);

  // Start drag operation
  const startDrag  : any : any : any : any = useCallback((card: Card, sourceZone: KonivrverZoneType) => {
  ...
  // End drag operation
  const endDrag  : any : any : any : any = useCallback(() => {
    setDragState({
      isDragging: false,
      dragOffset: { x: 0, y: 0 },
      validDropZones: []
  ...
  // Handle zone drop
  const handleZoneDrop  : any : any : any : any = useCallback((targetZone: KonivrverZoneType) => {
    if (!dragState.draggedCard || !dragState.sourceZone) return;

    const { draggedCard, sourceZone }  : any : any : any : any = dragState;

    setGameState(prev => {
      const newState  : any : any : any : any = { ...prev };
      const currentPlayerIndex  : any : any : any : any = newState.currentPlayer;
      const player  : any : any : any : any = newState.players[currentPlayerIndex];

      // Remove card from source zone
      const sourceZoneCards  : any : any : any : any = player.zones[sourceZone].cards;
      const cardIndex  : any : any : any : any = sourceZoneCards.findIndex(c => c.id === draggedCard.id);
      if (cardIndex >= 0) {
  ...
  const nextPhase  : any : any : any : any = useCallback(() => {
    setGameState(prev => {
      const phases: KonivrverPhase[]  : any : any : any : any = ['preGame', 'start', 'main', 'combat', 'postCombat', 'refresh'];
      const currentPhaseIndex  : any : any : any : any = phases.indexOf(prev.phase);
      const nextPhaseIndex  : any : any : any : any = (currentPhaseIndex + 1) % phases.length;
  ...
  // Draw a card
  const drawCard  : any : any : any : any = useCallback(() => {
    setGameState(prev => {
      const newState  : any : any : any : any = { ...prev };
      const player  : any : any : any : any = newState.players[newState.currentPlayer];
      const deck  : any : any : any : any = player.zones.deck;
      const hand  : any : any : any : any = player.zones.hand;
      
      if (deck.cards.length > 0) {
        const drawnCard  : any : any : any : any = deck.cards.pop()!;
  ...
  const playCard  : any : any : any : any = useCallback((card: Card) => {
    // Check if player has enough Azoth
    const currentPlayer  : any : any : any : any = gameState.players[gameState.currentPlayer];
    const hasEnoughAzoth  : any : any : any : any = card.elements.every(element => {
      const azothKey  : any : any : any : any = element.toLowerCase() as keyof typeof currentPlayer.azothPool;
    ...
    setGameState(prev => {
      ...
      const handIndex  : any : any : any : any = player.zones.hand.cards.findIndex(c => c.id === card.id);
      if (handIndex >= 0) {
        ...
        card.elements.forEach(element => {
          const azothKey  : any : any : any : any = element.toLowerCase() as keyof typeof player.azothPool;
