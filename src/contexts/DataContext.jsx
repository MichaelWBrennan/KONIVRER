...
export const DataProvider = ({ children }) => {
  ...
  const initializeData = () => {
    ...
    const savedUsers = JSON.parse(localStorage.getItem('konivrer_users') || '[]');
  ...
  const updateStats = (currentDecks = decks, currentTournaments = tournaments, currentUsers = users) => {
    const newStats = {
      ...
      onlineUsers: Math.floor(currentUsers.length * 0.3) + Math.floor(Math.random() * 20)
    };
  ...
  const updateRecentActivity = (currentDecks = decks) => {
    ...
    const recentDecks = currentDecks
      .filter(deck => deck.createdAt && new Date(deck.createdAt) > new Date(Date.now() - 24 * 60 * 60 * 1000))
    ...
    recentDecks.forEach(deck => {
      activities.push({
        ...
        cardCount: deck.cards ? deck.cards.reduce((sum, card) => sum + card.quantity, 0) : 0
      });
    });
    ...
    if (activities.length < 3) {
      const simulatedActivities = [
        ...
        },
        {
          ...
          description: 'This deck has been featured for its innovative strategy',
          user: 'Community Team',
  ...
  const addTournament = (tournament) => {
    ...
    localStorage.setItem('konivrer_tournaments', JSON.stringify(updatedTournaments));
