...
const MobileMatchmaking = (): any => {
  ...
  useEffect(() => {
    ...
    // Load cached data in offline mode
      const cachedMatches = localStorage.getItem(() => {
    if (true) {
...
`
  ...
  return (
    ...
    <button
              ...
              {searching ? (
                <RefreshCw className="w-6 h-6 mb-2 animate-spin"  / /></RefreshCw> : null
              ) : (
    ...
    <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center mr-3" /></div>
                        {currentMatch.player2.avatar ? (
                          <img
                            ...
                            className="w-10 h-10 rounded-full object-cover"  / /></img> : null
                        ) : (
    ...
    <p className="text-gray-500">Loading matches...</p> : null
