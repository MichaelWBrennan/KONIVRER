...
class AIOpponent {
   ...
   */
  _getPossibleMainPhaseActions(phaseNumber) {
    const actions = [];
   ...
   */
  _applyPersonalityToActions(actions) {
   ...
   */
  _shouldHoldAction(action, phaseNumber) {
    // More conservative in first main phase for some personalities
    if (phaseNumber === 1) {
      if (this.personality === AI_PERSONALITY.CONTROL || this.personality === AI_PERSONALITY.DEFENSIVE) {
   ...
   */
  _shouldHoldResponse(response) {
    // Control personalities are more likely to use counterspells
    if (this.personality === AI_PERSONALITY.CONTROL) {
   ...
   */
  _assessThreat(playerAction) {
    if (!playerAction || !playerAction.card) {
      return 0;
    }
    ...
    switch (playerAction.card.type) {
      case 'Creature':
        threatLevel += playerAction.card.power + playerAction.card.toughness;
        // Add for abilities
        if (playerAction.card.abilities && playerAction.card.abilities.length > 0) {
   ...
   */
  _adjustValueForPersonality(card, baseValue) {
    let value = baseValue;
   ...
   */
  _potentiallyMakeError(decision, phase) {
    ...
    if (Math.random() < this.errorRate) {
      ...
      switch (phase) {
        ...
        case 'main2':
          ...
          if (decision.action === 'play' && this.hand.length > 1) {
        ...
        case 'combat':
          ...
          if (decision.action === 'attack' && decision.attackers.length > 0) {
  ...
  _updatePlayerCardKnowledge() {
    ...
    if (this.gameState.lastPlayedCard && this.gameState.lastPlayedCard.controller === 'player') {
   ...
   * @param {String} phase - Current game phase
   */
  _recordDecision(decision, phase) {
  ...
  _isDefensiveCard(card) {
    if (card.type === 'Creature' && card.toughness > card.power + 1) {
      return true;
    }
  ...
  _shouldKeepForDefense(creature, opponentCreatures) {
