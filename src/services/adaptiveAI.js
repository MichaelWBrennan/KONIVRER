...
class AdaptiveAI {
   ...
   */
  applyStrategyToActions(actions) {
  ...
  shouldHoldAction(action) {
    ...
    if (this.gameState.playerField.length > this.gameState.aiField.length + 1) {
   ...
   */
  selectCardForAzoth() {
    if (this.gameState.aiHand.length === 0) {
      return null;
    }
  ...
  adaptStrategy() {
    ...
    if (this.gameState.aiLife < this.gameState.playerLife) {
    ...
    } else if (this.gameState.aiLife > this.gameState.playerLife) {
   ...
   */
  makeSlightlySuboptimalDecision(decision, phase) {
   ...
   */
  getRandomUniqueCards(pool, count) {
    // Shuffle the pool
