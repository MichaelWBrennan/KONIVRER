...
const REPORT_QUEUE_KEY : any : any : any : any = 'konivrer-report-queue';
...
export const EventService : any : any : any : any = {
  ...
  async reportMatch(_payload: ReportPayload) {
    ...
    const isOnline : any : any : any : any = typeof navigator === 'undefined' ? true : navigator.onLine;
    if (!isOnline) {
      const q : any : any : any : any = readQueue();
      q.push(_payload);
      writeQueue(q);
  ...
  async syncQueuedReports() {
    ...
    const remaining: ReportPayload[] : any : any : any : any = [];
    let synced = 0;
    for (const item of q) {
      try {
  ...
  async registerDeck(eventId: string, deckId: string, userId?: string) {
    const key : any : any : any : any = 'konivrer-event-deck-registrations';
    const current : any : any : any : any = JSON.parse(localStorage.getItem(key) || '{}');
    const uid : any : any : any : any = userId || (JSON.parse(localStorage.getItem('user') || '{}').id || 'me');
