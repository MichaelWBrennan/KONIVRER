...
class DynamicResolutionChain {
   ...
   */
  initialize(activePlayer, callbacks) {
   ...
   */
  _requestResponse() {
    if (this.callbacks.onRequestResponse) {
  ...
  passPriority(player) {
    ...
    if (this.priorityPlayer === this.activePlayer) {
  ...
  _resolveTopOfStack() {
    if (this.stack.length === 0) {
      ...
      if (this.callbacks.onChainComplete) {
    ...
    if (this.callbacks.onEffectResolution) {
   ...
   */
  respondToStack(response, player) {
