...
class UnifiedService {
  ...
  async initialize(user = null) {
    ...
    try {
      ...
      if (env.BACKEND_URL && this.userProfile) {
  ...
  async syncWithServer() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.recentTournaments) {
        ...
        if (response.data.recentMessages) {
   ...
   */
  getSearchHistory(type, limit = 10) {
    if (!this.searchHistory[type]) {
      return [];
    }
   ...
   */
  addToRecentTournaments(tournament) {
    // Remove if already exists
   ...
   * @returns {Promise<Object>} Search results
   */
  async searchDecks(query, filters = {}, options = {}) {
    // Add to search history
    if (query) {
   ...
   * @returns {Promise<Object>} Search results
   */
  async searchTournaments(query, filters = {}, options = {}) {
    // Add to search history
    if (query) {
   ...
   */
  async getUnifiedPlayerProfile(userId) {
    try {
      // In a real implementation, this would call the API
      // For now, we'll return mock data
      return {
   ...
   */
  async joinTournament(tournamentId, deckId) {
    try {
      // Load the deck
   ...
   */
  async startMatch(deckId, matchmakingOptions = {}) {
    try {
      // Load the deck
      const deck = this.decks.loadDeck(deckId);
   ...
   */
  filterCards(cards, query, filters) {
    return cards.filter(card => {
      ...
      if (query) {
        ...
        if (!nameMatch && !textMatch && !keywordMatch) {
          return false;
        }
      }
      ...
      // Filter by strength
      if (filters.strength) {
        if (filters.strength.min && card.power < filters.strength.min) {
          return false;
        }
        if (filters.strength.max && card.power > filters.strength.max) {
          return false;
        }
      }
      ...
      // Filter by cost
      if (filters.cost) {
        if (filters.cost.min && card.cost < filters.cost.min) {
          return false;
        }
        if (filters.cost.max && card.cost > filters.cost.max) {
          return false;
        }
      }
   ...
   */
  sortCards(cards, sortBy = 'name', sortOrder = 'asc') {
   ...
   */
  filterDecks(decks, query, filters) {
   ...
   */
  sortDecks(decks, sortBy = 'lastModified', sortOrder = 'desc') {
   ...
   */
  mergeSearchHistory(serverHistory) {
    Object.keys(serverHistory).forEach(type => {
      if (!this.searchHistory[type]) {
        this.searchHistory[type] = [];
      }
