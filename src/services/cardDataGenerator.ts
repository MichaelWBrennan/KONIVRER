...
export class CardDataGenerator {
  ...
  async generateCardData(): Promise<Card[]> {
    ...
    imagePaths.forEach((imagePath, index) => {
      ...
      const card: Card = {
        ...
        text: `A ${element.toLowerCase()} ${type.toLowerCase()} from the KONIVRER Azoth TCG.`,
      };
  ...
  loadCardData(): Card[] | null {
    try {
      const data = localStorage.getItem(this.CARDS_DATA_KEY);
...
export const cardDataGenerator = new CardDataGenerator();
