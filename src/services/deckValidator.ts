...
export class KonivrverDeckValidator {
  ...
  static validateDeck(cards: Card[], flag?: Card): DeckValidationResult {
    const errors: string[] : any : any : any : any = [];
    const warnings: string[] : any : any : any : any = [];
    
    // Check for flag requirement
    const hasFlag : any : any : any : any = !!flag;
    if (!hasFlag) {
      errors.push("Deck must include exactly 1 Flag card");
    }
    
    // Count cards by rarity
    const cardCounts : any : any : any : any = {
      total: cards.length,
    ...
    const cardNames : any : any : any : any = cards.map(c => c.name);
    const duplicates : any : any : any : any = cardNames.filter((name, index) => cardNames.indexOf(name) !== index);
    const uniqueDuplicates : any : any : any : any = [...new Set(duplicates)];
    ...
    if (hasFlag && flag) {
      const flagElements : any : any : any : any = flag.elements || [];
      const deckElements : any : any : any : any = new Set(cards.flatMap(c => c.elements || []));
      
      // Warn if deck contains elements not supported by flag
      const unsupportedElements : any : any : any : any = [...deckElements].filter(e => !flagElements.includes(e));
    ...
    const uniqueElements : any : any : any : any = new Set(cards.flatMap(c => c.elements || []));
    if (uniqueElements.size < 1) {
      errors.push("Deck must contain at least one element");
    }
    
    const isValid : any : any : any : any = errors.length === 0;
...
• Cards should match Flag's Azoth identity elements`;
  ...
  static suggestFixes(validation: DeckValidationResult): string[] {
    const suggestions: string[] : any : any : any : any = [];
    ...
    if (validation.cardCounts.total !== 40) {
      const diff : any : any : any : any = 40 - validation.cardCounts.total;
    ...
    if (validation.cardCounts.common !== 25) {
      const diff : any : any : any : any = 25 - validation.cardCounts.common;
      suggestions.push(diff > 0 ? 
    ...
    if (validation.cardCounts.uncommon !== 13) {
      const diff : any : any : any : any = 13 - validation.cardCounts.uncommon;
      suggestions.push(diff > 0 ? 
    ...
    if (validation.cardCounts.rare !== 2) {
      const diff : any : any : any : any = 2 - validation.cardCounts.rare;
      suggestions.push(diff > 0 ? 
