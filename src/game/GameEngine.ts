...
export class GameEngine {
    ...
    if (isTouchDevice) {
      ...
      const handleTouchMove = (e: TouchEvent) => {
        ...
        if (distance > 10) {
          ...
          // Handle camera rotation for ArcRotate camera touch controls
  ...
  private initCamera(): void {
    ...
    // ArcRotate camera for strategic card game view
    ...
    // Set reasonable bounds for the camera
    this.camera.lowerRadiusLimit = 5;
    this.camera.upperRadiusLimit = 25;
    this.camera.lowerBetaLimit = 0.1;
    this.camera.upperBetaLimit = Math.PI / 2;

    console.log('[GameEngine] ArcRotate camera initialized for card game');
  }
