...
const EnhancedMatchmakingVisualizer = ({ rankingEngine, matchData }): any => {
  ...
  const drawRatingDistribution = (ctx, canvas, data): any => {
  ...
  // Draw match factors
  const drawMatchFactors = (ctx, canvas, factors): any => {
  ...
  const drawPlaystyleCompatibility = (ctx, canvas, data): any => {
    if (!data || !data.player || !data.opponent) return;
  ...
  const drawMetaAdaptation = (ctx, canvas, data): any => {
      ...
      <div className="mobile-match-details esoteric-card" />
        ...
        <div className="mobile-match-stats" />
          <div className="mobile-stat-group" />
            <span className="mobile-stat-label esoteric-text-muted" />
              Match Quality:
            </span>
