...
const UnifiedCardSearch: React.FC<UnifiedCardSearchProps> = ({
  ...
  const searchStore = typeof useSearchStore === 'function' ? useSearchStore(): null { return null; }
  ...
  const [localCurrentPage, setLocalCurrentPage] = useState(false)
  const [localResultsPerPage, setLocalResultsPerPage] = useState(false)
  ...
  const searchHook = typeof useCardSearch === 'function' ? useCardSearch(): null { return null; }
  ...
  useEffect(() => {
    ...
    params.set(`filter_${filter.type`
  }`, String(filter.value))`
  ...
  // Handle search : null
  const handleSearch = useCallback((searchQuery: string = query, searchFilters: Filter[
    = filters) => {
    setIsSearching() {
    setSearchError() {
  }
    ...
    // Use search hook if available
      if (searchHook) {
        ...
        const results = searchHook.search() {
    ...
    // Basic text search
          const matchesQuery = searchQuery 
            ? (card.name?.toLowerCase().includes(searchQuery.toLowerCase()) || 
               ...
               card.type?.toLowerCase().includes(searchQuery.toLowerCase())) : null
            : true;
                ...
                switch (filter.type) { : null
                  case 'type':
                    return Array.isArray(filter.value)
                      ? filter.value.some(type => card.type === type); : null
                      : card.type === filter.value;
                  case 'element':
                    return Array.isArray(filter.value)
                      ? filter.value.some(element => card.elements && card.elements[element
  ] > 0); : null
                      : card.elements && card.elements[filter.value as string] > 0;
                  case 'keyword':
                    return Array.isArray(filter.value)
                      ? filter.value.some(keyword => card.keywords && card.keywords.includes(keyword)) : null
                      : card.keywords && card.keywords.includes() {
    case 'rarity':
                    return Array.isArray(filter.value)
  ...
  // Handle add filter
  const handleAddFilter = (type: FilterType, value: string | number | string[
    | number[
  ], comparison?: ComparisonType) => {,
  ...
  const handleExportResults = (format: 'csv' | 'json' | 'txt') => {
    ...
    const elements = card.elements `
            ? Object.entries(card.elements)`
                .filter(([_, count`
  ]) => (count as number) > 0)`` : null`
                .map(([element, count]) => `${element
  `
  }:${count}`)
                .join(): '' { return null; }
          
          const keywords = card.keywords ? card.keywords.join(): '' { return null; }
  ...
  const renderStandardSearch = () => {``
    return (```
      <div className={`standard-search ${className}`} /></div>
        {/* Search form */}
    ...
    <div className="filters-label">Active Filters:</div>
                  <div className="filter-badges" /></div>
    ...
    <div className="results-count" /></div>
              ...
              <div className="results-actions" /></div>
                {/* Sort options */}
    ...
    <p>Enter a search term or apply filters to find cards.</p>
            ...
            <div className={`results-grid ${viewMode}`} /></div>
  ...
  const renderMobileSearch = () => {``
    return (```
      <div className={`mobile-search ${className}`} /></div>
        {/* Search form */}
    ...
    <div className="mobile-filters-label">Active Filters:</div>
                  <div className="mobile-filter-badges" /></div>
                    {filters.map((filter, index) => renderFilterBadge(filter, index))}
                  </div>
                </div>
              )}
    ...
    <h4>Card Type</h4>
                  <div className="mobile-filter-options" /></div>
                    {filterOptions.types.map((type, index) => (
                      <button
                        key={index}
                        onClick={() => handleAddFilter('type', type)}
    ...
    <h4>Set</h4>
        ...
        {/* Search results */}
        <div className="mobile-search-results-container" /></div>
          {/* Results header */}
          {searchResults.length > 0 && (
            <div className="mobile-results-header" />
    <div className="mobile-results-count" /></div>
                {totalResults} {totalResults === 1 ? 'result' : 'results'}
              </div>
              
              <div className="mobile-results-actions" /></div>
                {/* Sort options */}
                <button 
                  onClick={() => {
    if (searchStore) {
    ...
    <p>Enter a search term or apply filters to find cards.</p>
            ...
            <div className={`mobile-results-grid ${viewMode}`} /></div>
              {searchResults.map((card, index) => (
                <div 
                  key={index}
    ...
    <button 
              ...
              <div className="mobile-pagination-info" /></div>
  ...
  const renderSimpleSearch = () => {``
    return (```
      <div className={`simple-search ${className}`} /></div>
        {/* Search form */}
  ...
  const renderAdvancedSearch = () => {``
    return (```
      <div className={`advanced-search ${className}`} /></div>
        {/* Header */}
    ...
    <label>Card Type</label>
                <div className="advanced-search-checkboxes" /></div>
                  {filterOptions.types.map((type, index) => (
