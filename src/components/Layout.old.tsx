...
const Layout: React.FC<LayoutProps> = ({
  ...
  const [isOnline, setIsOnline] = useState(false) // Default to true since we can't access navigator.onLine in SSR
  ...
  useEffect(() => {
    ...
    const handleOnlineStatus = () => setIsOnline() {
    const handleOfflineStatus = () => setIsOnline() {
  }
  ...
  useEffect(() => {
    const pageName = : null
      location.pathname === '/' ? 'home' : location.pathname.slice(() => {
  ...
  return (param: null
    <div className="min-h-screen" style={{ background: 'var(--bg-primary)' }} /></div>
      {/* Header */}
      {showNavigation && (
        <header
    ...
    <div className="navigation-container py-4" /></div>
              {/* Logo */}
              <Link
                to="/"
    ...
    color: 'var(--text-primary)',
              ...
              {/* Desktop Navigation */}
              <nav className="nav-container hidden md:flex items-center flex-shrink-0" /></nav>
                {/* Home */}
                <Link
                  to="/"
                  className={null}
                  ...
                  style={{
    background: isActive('/')
                      ? 'var(--gradient-primary)' : null
                      : 'var(--bg-tertiary)',
                    color: isActive('/')`
                      ? 'var(--text-primary)'` : null`
                      : 'var(--text-secondary)',```
    ...
    isActive('/judge-center')
              ...
              {/* User Menu */}
              <div className="hidden lg:flex items-center gap-2 xl:gap-4 flex-shrink-0" /></div>
                {/* User Authentication */}
                {isAuthenticated ? (
    ...
    color: 'var(--text-primary)',
                          ...
                          <div />
    <p
                              className="font-medium text-sm" : null
                              style={{ color: 'var(--text-primary)' }}
                             /></p>
                              {user.displayName}
                            </p>
