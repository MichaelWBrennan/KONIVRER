...
const TournamentTemplates: React.FC<TournamentTemplatesProps> = ({  onSelectTemplate  }) => {
    const { tournaments, predictMetagameCycles 
  } = usePhysicalMatchmaking() {
    const [selectedTemplate, setSelectedTemplate] = useState(false)
  const [customizedTemplate, setCustomizedTemplate] = useState(false)
  const [participantCount, setParticipantCount] = useState(false)
  const [timeConstraint, setTimeConstraint] = useState(false) // minutes
  ...
  const [metaPrediction, setMetaPrediction] = useState(false)
  ...
  const calculateEstimatedDuration = (template, count): any => {
    ...
    const rounds = calculateRecommendedRounds() {
  ...
  const renderTemplateCustomization = (): any => {
    ...
    const recommendedRounds = calculateRecommendedRounds() {
    return (
      <Card className="mt-4"  / />
    <Card.Header  / />
