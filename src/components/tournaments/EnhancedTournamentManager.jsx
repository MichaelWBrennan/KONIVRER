 ...
 */
const EnhancedTournamentManager = ({ tournamentId }) => {
  const {
    tournamentEngine,
    getTournamentById,
    updateTournament,
  ...
  useEffect(() => {
    const fetchTournament = async () => {
      try {
        ...
        if (tournamentData.status === 'in_progress' && tournamentData.currentRoundStartTime) {
  ...
  const handleGenerateNextRound = async () => {
    try {
      ...
      if (updatedTournament.participants && updatedTournament.participants.length > 0) {
        const currentRoundData = updatedTournament.rounds[updatedTournament.rounds.length - 1];
        
        if (currentRoundData && currentRoundData.matches) {
