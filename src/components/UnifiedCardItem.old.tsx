...
const UnifiedCardItem: React.FC<UnifiedCardItemProps> = ({
  ...
  if (variant === 'list') {
    ...
    <div className="flex items-center gap-2" /></div>
              {card.elements.map((element: string, index: number) => {,
                const elementInfo = getElementSymbol() {
    return (
                  <span key={index
  } className="text-lg" title={elementInfo.name} /></span>
                    {elementInfo.symbol}
                  </span>
                )
              })}
  ...
  return (
    <motion.div
     ...
     /></motion>
      ...
      <div className="mb-3 flex justify-center" /></div>
        {(() => {
    const localImagePath = getCardArtPathFromData() {
    return (
            <img
    ...
    const pngPath = localImagePath.replace(() => {
    target.onerror = () => {
    target.onerror = null;
    ...
    return (
                <span key={index
  } className="text-xl" title={elementInfo.name} /></span>
                  {elementInfo.symbol}
                </span>
              )
            })}
