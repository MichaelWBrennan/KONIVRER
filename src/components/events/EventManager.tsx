...
const EventManager: React.FC<EventManagerProps>  : any : any : any : any = ({ 
  eventId, 
  currentUserId, 
  isOrganizer, 
  isJudge 
}) => {
  const [event, setEvent]  : any : any : any : any = useState<Event | null>(null);
  const [pairings, setPairings]  : any : any : any : any = useState<Pairing[]>([]);
  const [matches, setMatches]  : any : any : any : any = useState<Match[]>([]);
  const [standings, setStandings]  : any : any : any : any = useState<Standing[]>([]);
  const [loading, setLoading]  : any : any : any : any = useState(true);
  const [error, setError]  : any : any : any : any = useState<string | null>(null);
  const [activeTab, setActiveTab]  : any : any : any : any = useState('overview');
  
  // Modal states
  const [showGeneratePairings, setShowGeneratePairings]  : any : any : any : any = useState(false);
  const [showMatchResult, setShowMatchResult]  : any : any : any : any = useState(false);
  const [generatingPairings, setGeneratingPairings]  : any : any : any : any = useState(false);

  useEffect(() => {
    fetchEventData();
    
    // Initialize WebSocket connection
    const wsSocket  : any : any : any : any = io('/events', {
      auth: {
  ...
  const fetchEventData  : any : any : any : any = async () => {
    try {
      const response  : any : any : any : any = await fetch(`/api/events/${eventId}`, {
        headers: {
      ...
      const eventData  : any : any : any : any = await response.json();
      setEvent(eventData);
  ...
  const fetchPairings  : any : any : any : any = async () => {
    try {
      const response  : any : any : any : any = await fetch(`/api/events/${eventId}/pairings`, {
  ...
  const fetchMatches  : any : any : any : any = async () => {
    try {
      const response  : any : any : any : any = await fetch(`/api/events/${eventId}/matches`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` },
      });
      if (response.ok) {
        const data  : any : any : any : any = await response.json();
  ...
  const fetchStandings  : any : any : any : any = async () => {
    try {
      const response  : any : any : any : any = await fetch(`/api/events/${eventId}/standings`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` },
      });
      if (response.ok) {
        const data  : any : any : any : any = await response.json();
  ...
  const generatePairings  : any : any : any : any = async () => {
    try {
      setGeneratingPairings(true);
      
      const response  : any : any : any : any = await fetch(`/api/events/${eventId}/pairings/generate`, {
        method: 'POST',
        headers: {
      ...
      const result  : any : any : any : any = await response.json();
      showNotification(
  ...
  const publishPairings  : any : any : any : any = async (round: number) => {
    try {
      const response  : any : any : any : any = await fetch(`/api/events/${eventId}/pairings/publish?round=${round}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
        },
  ...
  const showNotification  : any : any : any : any = (message: string, type: 'success' | 'error' | 'info') => {
    // Implementation would depend on your notification system
    console.log(`${type.toUpperCase()}: ${message}`);
  };

  const getCurrentRoundPairings  : any : any : any : any = () => {
    return pairings.filter(p => p.roundNumber === event?.currentRound);
  };

  const getCurrentRoundMatches  : any : any : any : any = () => {
    return matches.filter(m => m.round === event?.currentRound);
  };

  const getMatchProgress  : any : any : any : any = () => {
    const currentMatches  : any : any : any : any = getCurrentRoundMatches();
    if (currentMatches.length === 0) return 0;
    
    const completed  : any : any : any : any = currentMatches.filter(m => m.status === 'completed').length;
...
const PairingsTab: React.FC<any>  : any : any : any : any = ({ pairings, onReportResult }) => (
  <div>
    <ListGroup>
...
const StandingsTab: React.FC<any>  : any : any : any : any = ({ standings }) => (
  <div>
    <ListGroup>
...
const MatchesTab: React.FC<any>  : any : any : any : any = ({ matches }) => (
  <div>
    <ListGroup>
...
const AdminTab: React.FC<any>  : any : any : any : any = () => (
  <div>
    <Card>
      <Card.Body>
...
const MatchResultForm: React.FC<any>  : any : any : any : any = ({ onSubmit }) => {
  const [result, setResult]  : any : any : any : any = useState('win');
