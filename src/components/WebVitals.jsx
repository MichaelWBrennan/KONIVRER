  ...
  useEffect(() => {
    ...
    const trackAdditionalMetrics = () => {
      ...
      paintEntries.forEach(entry => {
        analytics.performanceMetric(entry.name.replace('-', '_'), Math.round(entry.startTime));
      });
      ...
      Object.entries(resourceStats).forEach(([key, value]) => {
        analytics.performanceMetric(key, Math.round(value), key === 'total_size' ? 'bytes' : 'ms');
      });
    };
    ...
    const trackMemoryUsage = () => {
      if (performance.memory) {
        analytics.performanceMetric('memory_used', 
          Math.round(performance.memory.usedJSHeapSize / 1024 / 1024), 'MB');
        
        analytics.performanceMetric('memory_total', 
          Math.round(performance.memory.totalJSHeapSize / 1024 / 1024), 'MB');
        
        analytics.performanceMetric('memory_limit', 
