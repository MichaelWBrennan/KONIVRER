...
const MobileTouchControls = ({ 
  ...
  const handleTouchMove = (e) => {
    ...
    if (touches.length === 1 && touchMode === 'pan') {
      ...
      if (Math.abs(angleDiff) > 0.1) { // Threshold to prevent accidental rotation
  ...
  const quickActions = [
    ...
    },
    {
      ...
      disabled: !isPlayerTurn,
      className: touchMode === 'attack' ? 'bg-orange-600 text-white' : 'bg-orange-500 hover:bg-orange-600 text-white'
    }
  ];
  ...
  return (
    <>
      ...
      {/* Control Panel */}
      <div className={`fixed bottom-4 left-4 right-4 transition-transform duration-300 z-50 ${
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-gray-200 p-4">
          ...
          <div className="flex justify-center mb-4">
            <div className="bg-gray-100 rounded-lg p-1 flex space-x-1">
              {[
                { mode: 'select', icon: <Hand className="w-4 h-4" />, label: 'Select' },
                { mode: 'pan', icon: <Move className="w-4 h-4" />, label: 'Pan' },
                { mode: 'zoom', icon: <ZoomIn className="w-4 h-4" />, label: 'Zoom' }
              ].map(({ mode, icon, label }) => (
                <button
                  key={mode}
