...
const AdvancedCardDatabase = () => {
  ...
  const cards = [
    ...
    },
    {
      ...
      defense: 1,
      description:
        'Deal 3 damage. If you control a Fire permanent, deal 1 additional damage.',
      playRate: 54.8,
      winRate: 68.9,
      trending: 'up',
      image: '/api/placeholder/200/280',
    },
    {
      ...
      defense: 4,
      description:
        'Prevent the next 4 damage that would be dealt to you this turn.',
      playRate: 43.2,
      winRate: 65.4,
      trending: 'stable',
      image: '/api/placeholder/200/280',
    },
    {
      ...
      defense: 3,
      description:
        'When this enters play, destroy target creature with cost 3 or less.',
      playRate: 28.7,
      winRate: 71.3,
      trending: 'down',
      image: '/api/placeholder/200/280',
    ...
    },
    {
      ...
      defense: 0,
      description:
        'Choose two elements. This card gains the effects of both elements.',
      playRate: 15.9,
      winRate: 74.2,
      trending: 'up',
      image: '/api/placeholder/200/280',
  ...
  const renderCardGrid = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {sortedCards.map(card => (
        <div
          key={card.id}
          className="bg-secondary border border-color rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 group"
        >
          <div className="relative">
            ...
            />
            <div
              className={`absolute top-3 left-3 px-2 py-1 rounded-lg text-xs font-medium text-white bg-gradient-to-r ${getElementColor(card.element)}`}
            >
              {card.element}
            </div>
  ...
  const renderCardList = () => (
    <div className="space-y-4">
      {sortedCards.map(card => (
        <div
          key={card.id}
          className="bg-secondary border border-color rounded-xl p-4 hover:bg-tertiary transition-all duration-200"
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                ...
                <div
                  className={`px-2 py-1 rounded text-xs font-medium text-white bg-gradient-to-r ${getElementColor(card.element)}`}
                >
                  {card.element}
                </div>
                <span
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-primary">
                Advanced Card Database
              </h1>
              <p className="text-secondary">
                Comprehensive card search with meta insights
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="border-t border-color pt-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(filterOptions)
                  ...
                  .map(([key, options]) => (
                    <div key={key}>
                      <label className="block text-sm font-medium text-primary mb-2 capitalize">
                        {key}
                      </label>
                      <select
                        value={filters[key]}
