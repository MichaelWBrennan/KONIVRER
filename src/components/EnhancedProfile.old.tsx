...
const EnhancedProfile = (): any => {
    const { user, updateProfile, applyForJudge, applyForOrganizer 
  } = useAuth() {
  ...
  const renderOverview = (renderOverview: any) => (
    ...
    <img
                ...
                className="w-32 h-32 rounded-2xl bg-tertiary border-4 border-accent-primary/20"  / /></img>
              {user.verified && (
...
``
    ...
    tournament.placement === 1
                            ? 'bg-yellow-500/20 text-yellow-400' : null
                            : tournament.placement <= 4
                            ? 'bg-purple-500/20 text-purple-400' : null
                            : tournament.placement <= 8`
                            ? 'bg-blue-500/20 text-blue-400'` : null`
  ...
  return (
    ...
    activeTab === tab.id`
                    ? 'bg-accent-primary text-white shadow-lg'` : null`
