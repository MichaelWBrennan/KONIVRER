 ...
 */
const PhysicalPlayEnhancements = ({ tournament, players, onUpdateTournament }) => {
...
const DeckRegistrationScanner = ({ 
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Computer Vision Deck Registration</h2>
            ...
            {isScanning && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="border-2 border-purple-500 rounded-lg w-64 h-40 flex items-center justify-center">
                  <span className="text-purple-300 font-medium">Position card here</span>
                </div>
              </div>
            )}
          ...
          {scanResults.length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-600">
              <div className="flex justify-between items-center">
                <span className="font-medium">Total Cards: {scanResults.length}</span>
                <div className="flex space-x-2">
...
const NFCPairingSystem = ({ players, tournament }) => {
  ...
  const startNFCPairing = useCallback(async (playerId) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (opponent) {
          setPairedPlayers(prev => [...prev, { player1: playerId, player2: opponent.id }]);
        }
        setScanningFor(null);
      }, 2000);
    } catch (error) {
            ...
            {players.slice(0, 6).map(player => (
              <div key={player.id} className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-gray-400">Rating: {Math.round(player.rating || 1500)}</p>
                </div>
                ...
                >
                  <div className="flex items-center">
                    <div className="text-center mr-4">
                      ...
                      <p className="text-sm text-gray-400">{Math.round(player1?.rating || 1500)}</p>
                    </div>
                    <div className="mx-4 text-purple-400">VS</div>
                    <div className="text-center ml-4">
                      <p className="font-medium">{player2?.name}</p>
                      <p className="text-sm text-gray-400">{Math.round(player2?.rating || 1500)}</p>
                    </div>
                  </div>
...
const VoiceControlSystem = ({ voiceEnabled, setVoiceEnabled, tournament, onUpdateTournament }) => {
  ...
  const processVoiceCommand = useCallback((command) => {
    ...
    } else if (lowerCommand.includes('match') && lowerCommand.includes('result')) {
      action = 'Record match result';
    } else if (lowerCommand.includes('next') && lowerCommand.includes('round')) {
      action = 'Advance to next round';
    } else if (lowerCommand.includes('time') && lowerCommand.includes('extension')) {
      action = 'Grant time extension';
    }

    if (action) {
      setLastCommand(command);
      setVoiceCommands(prev => [...prev, { command, action, timestamp: new Date() }]);
    }
  }, []);

  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Voice-Controlled Match Reporting</h2>
            ...
            {voiceCommands.map((cmd, index) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium text-purple-300">{cmd.action}</span>
      ...
      <div className="mt-6 bg-gray-700/50 rounded-lg p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <h4 className="font-medium mb-2 text-purple-300">Tournament Control</h4>
            <ul className="space-y-1 text-sm text-gray-400">
              <li>"Next round"</li>
...
const VenueOptimization = ({ tournament, players }) => {
  ...
  const layoutOptions = [
    { id: 'grid', name: 'Grid Layout', description: 'Traditional rectangular grid' },
    { id: 'circular', name: 'Circular', description: 'Tables arranged in circles' },
    { id: 'pods', name: 'Pod System', description: 'Grouped table pods' },
    { id: 'streaming', name: 'Stream Optimized', description: 'Layout optimized for streaming' }
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-3 bg-gray-800/50 rounded-lg text-center">
                ...
                <p className="text-xl font-bold">{Math.ceil(players.length / 2)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-700/50 rounded-lg p-4">
          ...
          <div className="bg-gray-800 rounded-lg p-4 min-h-64 flex items-center justify-center">
            <VenueLayoutVisualization layout={venueLayout} tableCount={tableCount} />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-700/50 rounded-lg p-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Featured Table</label>
              <select
                value={featuredTable}
...
const VenueLayoutVisualization = ({ layout, tableCount }) => {
  const renderLayout = () => {
    const tables = Array.from({ length: Math.min(tableCount, 16) }, (_, i) => i + 1);
      ...
      case 'grid':
        return (
          <div className="grid grid-cols-4 gap-2">
            {tables.map(table => (
              <div key={table} className="w-8 h-6 bg-purple-600 rounded flex items-center justify-center text-xs">
                {table}
              </div>
            ))}
          </div>
            ...
            {[0, 1].map(pod => (
              <div key={pod} className="grid grid-cols-2 gap-1">
                {tables.slice(pod * 8, (pod + 1) * 8).map(table => (
                  <div key={table} className="w-6 h-4 bg-purple-600 rounded flex items-center justify-center text-xs">
                    {table}
                  </div>
                ))}
              </div>
