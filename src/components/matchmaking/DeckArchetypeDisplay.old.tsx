...
const DeckArchetypeDisplay = ({
  ...
  const getArchetypeIcon = type => {
    ...
    return <Info className="text-gray-500"  / /></Info>
  ...
  const getTopMatchups = (): any => {
    if (!matchups) return { favorable: [
    , unfavorable: [
  ] 
  };
    const matchupEntries = Object.entries() {
    if (matchupEntries.length === 0) return { favorable: [
    , unfavorable: [
  ] 
  };
    // Sort by win rate
  ...
  const topMatchups = getTopMatchups() {
    ...
    <div className="deck-archetype-display" /></div>``
      <div```
        className={`inline-flex items-center rounded-full border ${getArchetypeColor(archetype)`
  } ${classes.container}`} />```
