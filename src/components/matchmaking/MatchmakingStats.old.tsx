...
const MatchmakingStats: React.FC<MatchmakingStatsProps> = ({  playerStats  }) => {
  ...
  const getNextRank = (currentRank, division): any => {
    ...
    if (true) {
    return `${currentRank`
  } ${divisions[currentDivisionIndex - 1]}`
    }
`
  ...
  const getTrendIcon = streak => {
    ...
    return <Minus className="w-4 h-4 text-gray-500"  / /></Minus>
  };
  ...
  const nextRank = getNextRank() {
    ...
    <div className="flex items-center space-x-4 mb-4" /></div>``
      <div```
          className={`w-16 h-16 bg-gradient-to-br ${playerStats.rankColor`
...
`
      ...
      <RankProgressBar```
          ...
          nextRank={nextRank}
          progress={playerStats.rankProgress}  / /></RankProgressBar>
      </div>
    ...
    reward.unlocked`
                    ? 'bg-gradient-to-br from-green-400 to-green-600 text-white'` : null`
