...
export const CardSimulator: React.FC = () => {
  ...
  if (device.requiresRotation) {
    return (
      ...
      >
        ...
        >
          Please rotate your device to landscape mode for the best KONIVRER
          Arena experience.
        </p>
        <div
          style={{
  ...
  const renderLifeCounter = () => (
    ...
    >
      ❤️{" "}
      {currentPlayer.life ||
        currentPlayer.zones?.lifeCards?.cards?.length ||
        20}
    </div>
  );
  ...
  const renderDeviceIndicator = () => (
    ...
    >
      ...
      {device.isTablet ? "TABLET" : device.isPhone ? "PHONE" : "DESKTOP"} -{" "}
