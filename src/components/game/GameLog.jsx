  ...
  const formatLogEntry = (log) => {
    switch (log.type) {
      ...
      case 'play':
        return (
          <div>
            <span className={log.player === 0 ? 'text-blue-300' : 'text-red-300'}>
      ...
      case 'block':
        return (
          <div>
            ...
            blocked{' '}
            <span className="font-medium text-yellow-200">{log.attacker.name}</span>
            {' '}with{' '}
            <span className="font-medium text-yellow-200">{log.blocker.name}</span>
          </div>
        );
      ...
      case 'damage':
        return (
          <div>
            <span className="font-medium text-yellow-200">{log.source.name}</span>
            {' '}dealt{' '}
            <span className="text-red-400 font-bold">{log.amount} damage</span>
            {' '}to{' '}
            <span className="font-medium text-yellow-200">{log.target.name}</span>
          </div>
        );
      ...
      case 'azoth':
        return (
          <div>
            ...
            <span className="text-yellow-400 font-bold">{Math.abs(log.amount)} Azoth</span>
          </div>
        );
      ...
      case 'ability':
        return (
          <div>
            ...
            activated{' '}
            <span className="font-medium text-yellow-200">{log.card.name}</span>'s ability
          </div>
        );
      ...
      case 'draw':
        return (
          <div>
            ...
            drew{' '}
            <span className="font-bold">{log.count} card{log.count !== 1 ? 's' : ''}</span>
          </div>
        );
      ...
      case 'life':
        return (
          <div>
            ...
            <span className="text-red-400 font-bold">{Math.abs(log.count)} life</span>
          </div>
        );
