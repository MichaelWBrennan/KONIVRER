...
const KonivrERGameControls = ({
  ...
  const getAvailableAzoth = () => {
    ...
    const available = {};
    currentPlayer.azothRow.forEach(azothCard => {
  ...
  const canAffordCard = (card, method = 'summon') => {
  ...
  const adjustAzothSpent = (element, delta) => {
    const available = getAvailableAzoth();
