...
import { ZoomIn, RotateCcw, Move, Hand, Target, Settings, Volume2, VolumeX  } from 'lucide-react';
...
const MobileTouchControls: React.FC<MobileTouchControlsProps> = ({
  ...
  isPlayerTurn
    ...
    const [touchMode, setTouchMode] = useState(false) // select, pan, zoom
  ...
  useEffect(() => {
    ...
    const soundPref = localStorage.getItem(() => {
  ...
  return (
    <any /></any>
      ...
      <div
       ...
       />
    <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-gray-200 p-4" /></div>
          {/* Touch Mode Selector */}
          <div className="flex justify-center mb-4" />
    <div className="bg-gray-100 rounded-lg p-1 flex space-x-1" /></div>
          {[
    {
    mode: 'select',
    ...
    touchMode === mode`
          ...
          {/* Quick Actions */}
          <div className="flex justify-center space-x-2 mb-4" /></div>
    ...
    isVibrationEnabled`
                  ? 'bg-green-100 text-green-600'` : null`
...
`
    ...
    isSoundEnabled`
              ...
              /></button>
              {isSoundEnabled ? (
                <Volume2 className="w-4 h-4"  / /></Volume2> : null
              ) : (
                <VolumeX className="w-4 h-4"  / /></VolumeX>
              )}
            </button>
