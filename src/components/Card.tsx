...
export const Card: React.FC<CardProps> : any : any : any = ({
  card,
  device,
  dragState,
  onCardSelect,
  ...
  const [isHovered, setIsHovered]  : any : any : any = useState(false);
  const [touchState, setTouchState]  : any : any : any = useState<TouchState>({
    isLongPress: false,
    touchStartTime: 0,
    touchPosition: { x: 0, y: 0 }
  });
  
  const cardRef  : any : any : any = useRef<HTMLDivElement>(null);
  const longPressTimer  : any : any : any = useRef<number | null>(null);

  // Card dimensions based on device and context
  const cardSize  : any : any : any = size || (device.isMobile 
  ...
  const handleMouseDown  : any : any : any = (e: React.MouseEvent) => {
    if (device.isMobile) return; // Handle touch events separately on mobile
    
    e.preventDefault();
    const rect  : any : any : any = cardRef.current?.getBoundingClientRect();
    if (rect) {
      onDragStart(card, { 
  ...
  const handleMouseUp  : any : any : any = (e: React.MouseEvent) => {
  ...
  const handleClick  : any : any : any = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (e.detail === 2) {
  ...
  const handleContextMenu  : any : any : any = (e: React.MouseEvent) => {
    e.preventDefault();
    onCardRightClick(card);
  };

  // Mobile touch handlers
  const handleTouchStart  : any : any : any = (e: React.TouchEvent) => {
    if (!device.isMobile) return;
    
    const touch  : any : any : any = e.touches[0];
    const now  : any : any : any = Date.now();
  ...
  const handleTouchEnd  : any : any : any = (e: React.TouchEvent) => {
    if (!device.isMobile) return;
    ...
    const now  : any : any : any = Date.now();
    const touchDuration  : any : any : any = now - touchState.touchStartTime;
    ...
    if (dragState.isDragging && dragState.draggedCard?.id === card.id) {
      const touch  : any : any : any = e.changedTouches[0];
      onDragEnd(card, { x: touch.clientX, y: touch.clientY });
    }
  };

  const handleTouchCancel  : any : any : any = () => {
    if (longPressTimer.current) {
      window.clearTimeout(longPressTimer.current);
    }
  };
  ...
  const getCardStyle  : any : any : any = (): React.CSSProperties => {
    const baseStyle: React.CSSProperties  : any : any : any = {
