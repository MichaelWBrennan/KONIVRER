  ...
  const getCardStyle = (): React.CSSProperties => {
    const baseStyle: React.CSSProperties = {
      ...
      transform: `
        ...
        ${
          dragState.isDragging && dragState.draggedCard?.id === card.id
            ? "scale(1.05)"
            : ""
        }
      `,
      opacity:
        dragState.isDragging && dragState.draggedCard?.id === card.id ? 0.8 : 1,
      zIndex:
        dragState.isDragging && dragState.draggedCard?.id === card.id
          ? 1000
          : 1,
    ...
    >
      ...
      >
        ...
        {(card.type?.toLowerCase().includes("creature") ||
          card.lesserType?.toLowerCase().includes("familiar")) && (
          <div
            className={cs.ptBadge}
            style={{
