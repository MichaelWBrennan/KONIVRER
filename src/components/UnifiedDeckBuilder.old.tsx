...
const UnifiedDeckBuilder: React.FC<UnifiedDeckBuilderProps> = ({
    ...
    const rules: DeckValidationRule[] = [
      {
        ...
        validate: (deck) => {
          ...
          return {
            valid,
            message: valid ? undefined : `Your deck has ${deck.mainDeck.length} cards, but needs at least 40`
          };
        }
      },
      {
      ...
      },
      {
        ...
        validate: (deck) => {
          ...
          return {
            valid,
            message: valid ? undefined : `Your deck has ${azothCount} Azoth cards, but can have at most 5`
          };
        }
      }
    ];
  ...
  const addCardToDeck = (card: Card, target: 'mainDeck' | 'sideboard' = 'mainDeck') => {
    ...
    if (cardCounts[card.name] >= 3) {
      setError(`You can't add more than 3 copies of ${card.name}`);
      return;
    }
  ...
  const handleSaveDeck = () => {
    ...
    if (deck.mainDeck.length < 40) {
      setError('Your deck must have at least 40 cards');
      return;
    }
  ...
  const handleImportDeck = () => {
    if (!importData.trim()) {
      setError('Please enter deck data to import');
      return;
    }
  ...
  const handleExportDeck = () => {
    try {
      ...
      if (onExport) {
        ...
        switch (exportFormat) {
          case 'json':
            exportData = JSON.stringify(deck, null, 2);
            break;
          ...
          case 'txt':
            ...
            exportData += `Main Deck (${deck.mainDeck.length}):\n`;
            ...
            Object.entries(cardsByType).forEach(([type, cards]) => {
              if (cards.length > 0) {
                ...
                uniqueCards.forEach(cardName => {
                  exportData += `${cardCounts[cardName]}x ${cardName}\n`;
                });
              }
            });
            ...
            if (deck.sideboard.length > 0) {
              ...
              uniqueSideboardCards.forEach(cardName => {
                exportData += `${sideboardCounts[cardName]}x ${cardName}\n`;
              });
            }
            break;
          ...
          case 'csv':
            ...
            Object.values(csvCardCounts).forEach(({ card, count }) => {
              ...
              exportData += `"${card.name}","${card.type}",${count},"${elements}","${card.set || ''}","${card.rarity || ''}"\n`;
            });
            break;
        }
        ...
        a.href = url;
        a.download = `${deck.name.replace(/\s+/g, '_')}.${exportFormat}`;
  ...
  // Render card grid
  const renderCardGrid = (cards: Card[], source: 'availableCards' | 'mainDeck' | 'sideboard') => {
    return (
      <div className="card-grid">
        {cards.map((card, index) => (
          <div 
            key={`${source}-${card.id}-${index}`}
            className="card-item"
            draggable={true}
  ...
  // Render card list
  const renderCardList = (cards: Card[], source: 'availableCards' | 'mainDeck' | 'sideboard') => {
    // Group cards by type
    const cardsByType: Record<CardType, Card[]> = {
      Familiar: [],
      Spell: [],
              ...
              {Object.entries(deck.stats.elementDistribution)
                ...
                .map(([element, count]) => (
                  <div key={element} className="element-bar">
                    ...
                    <div className="element-value-bar">
                      <div 
          ...
          <div className="stat-section">
            ...
            <div className="general-stats">
              ...
              <div className="general-stat">
                ...
                <div className="stat-value colors">
  ...
  const renderStandardDeckBuilder = () => {
    return (
      <div className="standard-deck-builder">
        {/* Deck builder header */}
          ...
          <div className="card-browser">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            setFilters(prev => ({
                              ...prev,
                              types: [...(prev.types || []), type as CardType]
                            }));
                          } else {
                            setFilters(prev => ({
                              ...prev,
            ...
            <div className="browser-results">
              {filteredCards.length === 0 ? (
                ...
                viewMode === 'grid' 
                  ? renderCardGrid(filteredCards, 'availableCards')
            ...
            >
              {showSideboard ? (
                deck.sideboard.length === 0 ? (
                  ...
                  viewMode === 'grid' 
                    ? renderCardGrid(deck.sideboard, 'sideboard')
                ...
                deck.mainDeck.length === 0 ? (
                  ...
                  viewMode === 'grid' 
                    ? renderCardGrid(deck.mainDeck, 'mainDeck')
  ...
  const renderMobileDeckBuilder = () => {
    return (
      <div className="mobile-deck-builder">
        {/* Mobile header */}
        ...
        <div className="mobile-content">
          {showStats ? (
            ...
            <div className="mobile-deck-view">
              ...
              <div className="mobile-card-results">
                {filteredCards.length === 0 ? (
                  <div className="empty-results">
                    <Search size={24} />
                    <p>No cards found</p>
  ...
  return (
    <div className={`unified-deck-builder ${className}`}>
      {renderError()}
      
      {actualVariant === 'enhanced' ? (
        renderEnhancedDeckBuilder()
      ) : actualVariant === 'mobile' ? (
        renderMobileDeckBuilder()
      ) : (
        renderStandardDeckBuilder()
      )}
