...
const DeckValidator: React.FC<DeckValidatorProps> = ({  deck  }) => {
  ...
  const rules = {
    ...
    maxCards: 60,
    maxCopies: 4,
    bannedCards: ['Ancient Dragon'
  ], // Example banned card
    restrictedCards: { 'Lightning Bolt': 1 
  }, // Example restricted card
  };
  ...
  const validations = [
    ...
    type: 'error',
      ...
      message: `Deck must have at least ${rules.minCards`
  } cards (currently ${totalCards})`
    },
    {`
    type: 'error',``
      check: totalCards <= rules.maxCards,```
      message: `Deck cannot exceed ${rules.maxCards`
  } cards (currently ${totalCards})`
    },
    {`
    type: 'error',``
      check: deck.cards.every(card => card.quantity <= rules.maxCopies),```
      message: `No card can have more than ${rules.maxCopies`
  } copies`,`
      details: deck.cards``
    ...
    ```
          return `${cardName`
  }: ${cardInDeck.quantity}/${maxAllowed} allowed`
        })
    }
  ];
  ...
  const getValidationIcon = (type, passed): any => {
    ...
    return <AlertTriangle className="text-yellow-500" size={16}  / /></AlertTriangle>
  };
