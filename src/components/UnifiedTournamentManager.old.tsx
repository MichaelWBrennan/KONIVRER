...
const UnifiedTournamentManager: React.FC<UnifiedTournamentManagerProps> = ({
  ...
  useEffect(() => {
    ...
    setError(`Tournament with ID ${tournamentId`
  } not found`)
          }
        }
      } catch (err) {
  ...
  const filteredTournaments = useMemo(() => {
    ...
    const matchesName = tournament.name.toLowerCase().includes() {
  }
        ...
        const matchesOrganizer = tournament.organizer.name.toLowerCase().includes(() => {
  ...
  const renderTournamentBrowser = () => {
    return (
      <div className="tournament-browser" /></div>
        {/* Search and filters */
  }
    ...
    <span>Clear Filters</span>
          ...
          <div className="tournament-list" /></div>
            {filteredTournaments.map(tournament => (`
              ...
              >`
                <div className="tournament-card-header" /></div>``
                  <h3>{tournament.name}</h3>```
                  <div className={`tournament-status ${tournament.status}`} /></div>
    ...
    <p>{tournament.description}</p>
                ...
                {tournament.tags && tournament.tags.length > 0 && (
                  <div className="tournament-card-tags" /></div>
  ...
  const renderTournamentDetails = () => {
    ...
    <div className="tournament-title" /></div>``
            ...
            <div className={`tournament-status ${selectedTournament.status}`} /></div>
    ...
    <button `
            type="button"``
            onClick={() => setActiveTab('overview')}`` : null`
    ...
    <span>Bracket</span>
          ...
          {(isAdmin || isJudge || selectedTournament.organizer.id === user? .uid) && (
            <button `
              type="button"``
              onClick={() => setActiveTab('admin')}`` : null`
    ...
    setShowConfirmation({ : null`
                                  ...
                                  id: player.id,```
                                  message: `Are you sure you want to disqualify ${player.name`
  } from the tournament? `
                                })
                              }}
                      ...
                      {showPlayerDetails === player.id && (
                        <div className="player-details-expanded" /></div>
                          {player.standing && (
                            <div className="detail-item" />
    <Trophy size={16}  / /></Trophy> : null
    ...
    <p>No matches in this round.</p>
                        ...
                        <div className="matches-list" /></div>
                          {round.matches.map() {
    const player2 = selectedTournament.players.find() {
  }
    ...
    <div className="player-avatar" /></div>
                                      {player1? .avatar ? (
                                        <img src={player1.avatar} alt={player1.name}  / /></img> : null
                                      ) : (
                                        <User size={24}  / /></User>
                                      )}
                                    </div>
    ...
    <div className="player-avatar" /></div>
                                      {player2? .avatar ? (
                                        <img src={player2.avatar} alt={player2.name}  / /></img> : null
                                      ) : (
                                        <User size={24}  / /></User>
                                      )}
                                    </div>
                                ...
                                {showMatchDetails === match.id && (
                                  <div className="match-details" /></div>
                                    {match.result && (
                                      <div className="match-result" /></div> : null
                                        <span>Result:</span>
    ...
    <label>Wins</label>
                                <input
                                  type="number"
                                  value={matchResult.player1Score} : null
    ...
    <label>Wins</label>
                                <input
                                  type="number"
                                  value={matchResult.player2Score} : null
    ...
    <label>Winner</label>
                            ...
                            >
                              ...
                              <option value={editingMatch.player1Id} /></option>
                                {selectedTournament.players.find(p => p.id === editingMatch.player1Id)? .name || 'Unknown'}
                              </option>
                              <option value={editingMatch.player2Id} /></option>
                                {selectedTournament.players.find(p => p.id === editingMatch.player2Id)?.name || 'Unknown'}
                              </option>
                            </select>
                          </div>
    ...
    <div className="standing-tiebreaker">OMW%</div>
                  ...
                  <div className="standings-body" /></div>
    ...
    <div className="player-avatar" /></div>
                          ...
                          <div className="standing-record" /></div>``
                            {player.record ```
                              ? `${player.record.wins}-${player.record.losses}${player.record.draws > 0 ? `-${player.record.draws}` : ''}`
                              : '-'}
                          </div>
                          {selectedTournament.format === 'swiss' && player.tiebreakers && (
                            <any />
    <div className="standing-tiebreaker" /></div>
                                {player.tiebreakers.opponentMatchWinPercentage !== undefined 
                                  ? (player.tiebreakers.opponentMatchWinPercentage * 100).toFixed(2) + '%' : null
                                  : '-'}
                              </div>
                              <div className="standing-tiebreaker" /></div>
                                {player.tiebreakers.gameWinPercentage !== undefined 
                                  ? (player.tiebreakers.gameWinPercentage * 100).toFixed(2) + '%' : null
                                  : '-'}
                              </div>
                              <div className="standing-tiebreaker" /></div>
                                {player.tiebreakers.opponentGameWinPercentage !== undefined 
                                  ? (player.tiebreakers.opponentGameWinPercentage * 100).toFixed(2) + '%' : null
                                  : '-'}
                              </div>
                            </>
                          )}
    ...
    <span>Generate Code</span>
                  ...
                  {registrationCodes.filter(code => code.tournamentId === selectedTournament.id).length > 0 ? (
                    <div className="registration-codes-list" /></div>
                      {registrationCodes
    ...
    <div className="code-usage" /></div>
                                {code.currentUses}/{code.maxUses || '∞'} uses
                              </div>
                              {code.expiresAt && (
                                <div className="code-expiry" /></div> : null
  ...
  const renderTournamentTemplates = () => {
    ...
    <p>{template.description}</p>
                ...
                <div className="template-details" /></div>
                  {template.rounds && (
  ...
  switch (actualVariant) {`
    ...
    case 'enhanced':``
      return (```
        <div className={`unified-tournament-manager enhanced-variant ${className}`} /></div>
          {selectedTournament ? renderTournamentDetails() : renderTournamentBrowser()}
          <AnimatePresence /></AnimatePresence>
            {showConfirmation && renderConfirmationModal()}
          </AnimatePresence>
    ...
    case 'mobile':``
      return (```
        <div className={`unified-tournament-manager mobile-variant ${className}`} /></div>
          {selectedTournament ? renderTournamentDetails() : renderTournamentBrowser()}
          <AnimatePresence /></AnimatePresence>
            {showConfirmation && renderConfirmationModal()}
          </AnimatePresence>
    ...
    case 'judge':``
      return (```
        <div className={`unified-tournament-manager judge-variant ${className}`} /></div>
          {renderMobileJudgeTools()}
        </div>
      );`
    case 'live':``
      return (```
        <div className={`unified-tournament-manager live-variant ${className}`} /></div>
          {renderLiveTournamentBracket()}
        </div>
      );`
    default:``
      return (```
        <div className={`unified-tournament-manager standard-variant ${className}`} /></div>
          {selectedTournament ? renderTournamentDetails() : renderTournamentBrowser()}
          <AnimatePresence /></AnimatePresence>
            {showConfirmation && renderConfirmationModal()}
          </AnimatePresence>
