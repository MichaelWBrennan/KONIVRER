...
const PhysicalMatchmakingApp = () => {
  ...
  const handlePlayerSubmit = useCallback((e) => {
    e.preventDefault();
    try {
      ...
      setErrorMessage(`Error adding player: ${error.message || 'Unknown error'}`);
    }
  }, [newPlayerForm, addPlayer]);

  // Handle tournament form submission with error handling
  const handleTournamentSubmit = useCallback((e) => {
    e.preventDefault();
    try {
      if (!newTournamentForm.name || !newTournamentForm.date || !newTournamentForm.location) {
      ...
      setErrorMessage(`Error creating tournament: ${error.message || 'Unknown error'}`);
    }
  }, [newTournamentForm, createTournament]);

  // Handle match form submission with error handling
  const handleMatchSubmit = useCallback((e) => {
    e.preventDefault();
    try {
      ...
      setErrorMessage(`Error creating match: ${error.message || 'Unknown error'}`);
    }
  }, [newMatchForm, createMatch]);

  // Generate QR code for a match
  const generateMatchQR = useCallback((match) => {
    try {
      if (!match || !match.id) {
      ...
      setErrorMessage(`Error generating QR code: ${error.message || 'Unknown error'}`);
    }
  }, [players, tournaments]);
  ...
  const handleExport = useCallback(() => {
    try {
      ...
      setErrorMessage(`Error exporting data: ${error.message || 'Unknown error'}`);
    }
  }, [exportData]);
  ...
  const handleImport = useCallback(() => {
    ...
    try {
      ...
      if (!Array.isArray(data.players) || !Array.isArray(data.tournaments) || !Array.isArray(data.matches)) {
        setImportError('Invalid data format: missing required arrays (players, tournaments, matches)');
        return;
      }
  ...
  const handleDeletePlayer = useCallback((player) => {
    ...
    if (window.confirm(`Are you sure you want to delete ${player.name}? This action cannot be undone.`)) {
      try {
        ...
        setErrorMessage(`Error deleting player: ${error.message || 'Unknown error'}`);
      }
    }
  }, [removePlayer]);

  // Handle tournament deletion with confirmation
  const handleDeleteTournament = useCallback((tournament) => {
    ...
    if (window.confirm(`Are you sure you want to delete ${tournament.name}? This action cannot be undone.`)) {
      try {
        ...
        setErrorMessage(`Error deleting tournament: ${error.message || 'Unknown error'}`);
      }
    }
  }, [removeTournament]);

  // Handle match deletion with confirmation
  const handleDeleteMatch = useCallback((match) => {
    if (!match || !match.id) return;
    
    const player1 = players?.find(p => p.id === match.player1Id)?.name || 'Unknown';
    const player2 = players?.find(p => p.id === match.player2Id)?.name || 'Unknown';
    
    if (window.confirm(`Are you sure you want to delete the match between ${player1} and ${player2}? This action cannot be undone.`)) {
      try {
        removeMatch(match.id);
        setSelectedMatch(null);
      } catch (error) {
        console.error('Error deleting match:', error);
        setErrorMessage(`Error deleting match: ${error.message || 'Unknown error'}`);
      }
    }
  }, [removeMatch, players]);

  // Record match result
  const handleRecordResult = useCallback((matchId, result) => {
    try {
      if (!matchId || !result) {
      ...
      setErrorMessage(`Error recording match result: ${error.message || 'Unknown error'}`);
    }
  }, [recordMatchResult]);
  ...
  const renderPlayersTab = useCallback(() => (
    <div className="mobile-tab-content esoteric-card">
      ...
      <div className="mobile-player-list">
        {Array.isArray(filteredPlayers) && filteredPlayers.length > 0 ? (
            ...
            player && player.id ? (
              ...
              >
                <div className="mobile-player-info">
                  <h3 className="mobile-player-name esoteric-text-accent">{player.name || 'Unknown'}</h3>
                  <p className="mobile-player-deck esoteric-text-muted">{player.deckName || 'No deck'}</p>
                </div>
                <div className="mobile-player-rating">
                  <span className="esoteric-badge">{Math.round(player.rating || 1500)}</span>
                </div>
              </div>
            ) : null
          ))
          ...
          <div className="mobile-empty-state esoteric-empty-state">
            <p className="esoteric-text-muted">No players found. Add your first player below.</p>
          </div>
        )}
      </div>
      ...
      <div className="mobile-form-container">
        ...
        <form className="mobile-form esoteric-form" onSubmit={handlePlayerSubmit}>
          <div className="mobile-form-group">
            <label htmlFor="playerName" className="esoteric-text-muted">Player Name</label>
            <input
              type="text"
              id="playerName"
              className="mobile-input esoteric-input"
          ...
          <div className="mobile-form-group">
            <label htmlFor="deckName" className="esoteric-text-muted">Deck Name</label>
            <input
              type="text"
              id="deckName"
              className="mobile-input esoteric-input"
              value={newPlayerForm.deckName || ''}
              onChange={(e) => setNewPlayerForm(prev => ({...prev, deckName: e.target.value}))}
              required
              aria-required="true"
              maxLength={50}
            />
          ...
          <div className="mobile-form-group">
            <label htmlFor="contactInfo" className="esoteric-text-muted">Contact Info (optional)</label>
            <input
              type="text"
              id="contactInfo"
              className="mobile-input esoteric-input"
              value={newPlayerForm.contactInfo || ''}
              onChange={(e) => setNewPlayerForm(prev => ({...prev, contactInfo: e.target.value}))}
              maxLength={100}
            />
          </div>
    ...
    <div className="mobile-tab-content esoteric-card">
      ...
      <div className="mobile-tournament-list">
        {Array.isArray(filteredTournaments) && filteredTournaments.length > 0 ? (
            ...
            tournament && tournament.id ? (
              ...
              >
                <div className="mobile-tournament-info">
                  <h3 className="mobile-tournament-name esoteric-text-accent">{tournament.name || 'Unknown'}</h3>
                  <p className="mobile-tournament-date esoteric-text-muted">
                    {tournament.date ? new Date(tournament.date).toLocaleDateString() : 'No date'}
                  </p>
                  <p className="mobile-tournament-location esoteric-text-muted">{tournament.location || 'No location'}</p>
                </div>
                <div className="mobile-tournament-players">
                  <span className="esoteric-badge">
                    {tournament.playerCount || 0} / {tournament.maxPlayers || 32}
                  </span>
                </div>
              </div>
            ) : null
          ...
          <div className="mobile-empty-state esoteric-empty-state">
            <p className="esoteric-text-muted">No tournaments found. Create your first tournament below.</p>
          </div>
        )}
      </div>

      <div className="mobile-form-container">
        <h3 className="mobile-form-title esoteric-rune">Create New Tournament</h3>
        <form className="mobile-form esoteric-form" onSubmit={handleTournamentSubmit}>
          <div className="mobile-form-group">
            <label htmlFor="tournamentName" className="esoteric-text-muted">Tournament Name</label>
            <input
              type="text"
              id="tournamentName"
              className="mobile-input esoteric-input"
          ...
          <div className="mobile-form-group">
            <label htmlFor="tournamentDate" className="esoteric-text-muted">Date</label>
            <input
              type="date"
              id="tournamentDate"
              className="mobile-input esoteric-input"
              value={newTournamentForm.date || new Date().toISOString().split('T')[0]}
              onChange={(e) => setNewTournamentForm(prev => ({...prev, date: e.target.value}))}
              required
              aria-required="true"
            />
          </div>
          ...
          <div className="mobile-form-group">
            <label htmlFor="tournamentLocation" className="esoteric-text-muted">Location</label>
            <input
              type="text"
              id="tournamentLocation"
              className="mobile-input esoteric-input"
              value={newTournamentForm.location || ''}
              onChange={(e) => setNewTournamentForm(prev => ({...prev, location: e.target.value}))}
              required
              aria-required="true"
              maxLength={100}
            />
          ...
          <div className="mobile-form-group">
            <label htmlFor="tournamentMaxPlayers" className="esoteric-text-muted">Max Players</label>
            <input
              type="number"
              id="tournamentMaxPlayers"
              className="mobile-input esoteric-input"
              value={newTournamentForm.maxPlayers || 32}
              onChange={(e) => setNewTournamentForm(prev => ({...prev, maxPlayers: parseInt(e.target.value) || 32}))}
              min="2"
              max="256"
              required
              aria-required="true"
          ...
          <button 
            type="submit" 
            className="mobile-btn mobile-btn-primary esoteric-btn"
            disabled={!newTournamentForm.name || !newTournamentForm.date || !newTournamentForm.location}
          >
            Create Tournament
          </button>
        </form>
  ...
  ), [filteredTournaments, searchTerm, newTournamentForm, handleTournamentSubmit]);
    ...
    <div className="mobile-tab-content esoteric-card">
      ...
      <div className="mobile-match-list">
        {Array.isArray(filteredMatches) && filteredMatches.length > 0 ? (
              ...
              >
                <div className="mobile-match-info">
                  ...
                  <p className="mobile-match-details esoteric-text-muted">
                    {tournament?.name || 'Quick Match'} • Round {match.round || 1} • Table {match.table || 1}
                  </p>
                  <p className="mobile-match-result esoteric-text-muted">
                    Result: <span className={match.result ? 'esoteric-text-accent' : ''}>{resultText}</span>
                  </p>
                </div>
          ...
          <div className="mobile-empty-state esoteric-empty-state">
            <p className="esoteric-text-muted">No matches found. Create your first match below.</p>
          </div>
        )}
      </div>
      ...
      <div className="mobile-form-container">
        ...
        <form className="mobile-form esoteric-form" onSubmit={handleMatchSubmit}>
          <div className="mobile-form-group">
            <label htmlFor="player1" className="esoteric-text-muted">Player 1</label>
            <select
              id="player1"
              className="mobile-select esoteric-select"
              value={newMatchForm.player1Id || ''}
              onChange={(e) => setNewMatchForm(prev => ({...prev, player1Id: e.target.value}))}
              required
              aria-required="true"
            >
              ...
              {Array.isArray(players) && players.map(player => (
                player && player.id ? (
                  <option key={player.id} value={player.id}>
                    {player.name || 'Unknown'} ({player.deckName || 'No deck'})
                  </option>
                ) : null
              ))}
            </select>
          ...
          <div className="mobile-form-group">
            <label htmlFor="player2" className="esoteric-text-muted">Player 2</label>
            <select
              id="player2"
              className="mobile-select esoteric-select"
              value={newMatchForm.player2Id || ''}
              onChange={(e) => setNewMatchForm(prev => ({...prev, player2Id: e.target.value}))}
              required
              aria-required="true"
            >
              <option value="">Select Player 2</option>
              {Array.isArray(players) && players.map(player => (
                player && player.id && player.id !== newMatchForm.player1Id ? (
                  <option key={player.id} value={player.id}>
                    {player.name || 'Unknown'} ({player.deckName || 'No deck'})
                  </option>
                ) : null
          ...
          <div className="mobile-form-group">
            <label htmlFor="tournament" className="esoteric-text-muted">Tournament (optional)</label>
            <select
              id="tournament"
              className="mobile-select esoteric-select"
              value={newMatchForm.tournamentId || ''}
              onChange={(e) => setNewMatchForm(prev => ({...prev, tournamentId: e.target.value}))}
            >
          ...
          <div className="mobile-form-group">
            <label htmlFor="round" className="esoteric-text-muted">Round</label>
            <input
              type="number"
              id="round"
              className="mobile-input esoteric-input"
              value={newMatchForm.round || 1}
              onChange={(e) => setNewMatchForm(prev => ({...prev, round: parseInt(e.target.value) || 1}))}
              min="1"
              max="99"
            />
          </div>
          ...
          <div className="mobile-form-group">
            <label htmlFor="table" className="esoteric-text-muted">Table</label>
            <input
              type="number"
              id="table"
              className="mobile-input esoteric-input"
              value={newMatchForm.table || 1}
              onChange={(e) => setNewMatchForm(prev => ({...prev, table: parseInt(e.target.value) || 1}))}
              min="1"
              max="999"
            />
          </div>
          ...
          <button 
            type="submit" 
            className="mobile-btn mobile-btn-primary esoteric-btn"
            disabled={!newMatchForm.player1Id || !newMatchForm.player2Id || newMatchForm.player1Id === newMatchForm.player2Id}
          >
            Create Match
          </button>
        </form>
  ...
  ), [filteredMatches, searchTerm, newMatchForm, players, tournaments, handleMatchSubmit, generateMatchQR]);
  ...
  const renderSettingsTab = useCallback(() => (
    <div className="mobile-tab-content esoteric-card">
      ...
      <div className="mobile-settings-section">
        <h3 className="mobile-settings-title esoteric-rune">Data Management</h3>
      ...
      <div className="mobile-settings-section">
        ...
        <div className="mobile-about-content esoteric-text-muted">
          <p>
            The Physical Matchmaking system allows you to organize in-person matches and tournaments
            for your KONIVRER card game events.
          </p>
          <p>
            Features include player management, tournament organization, match tracking, and
            advanced analytics with Bayesian rating calculations.
          </p>
        </div>
      </div>
    </div>
        ...
        <div className="mobile-modal-content esoteric-modal-content">
          ...
          <div className="mobile-modal-body">
            <div className="mobile-player-detail">
              <h3 className="esoteric-text-accent">{selectedPlayer.name || 'Unknown'}</h3>
              <p className="esoteric-text-muted">Deck: {selectedPlayer.deckName || 'No deck'}</p>
              {selectedPlayer.contactInfo && (
                <p className="esoteric-text-muted">Contact: {selectedPlayer.contactInfo}</p>
              )}
              <p className="esoteric-text-muted">Rating: {Math.round(selectedPlayer.rating || 1500)}</p>
              ...
              {selectedPlayer.matches && selectedPlayer.matches.length > 0 && (
                <div className="mobile-player-stats">
                  ...
                  <p className="esoteric-text-muted">
                    Wins: {selectedPlayer.wins || 0} | 
                    Losses: {selectedPlayer.losses || 0} | 
                    Draws: {selectedPlayer.draws || 0}
                  </p>
                  <p className="esoteric-text-muted">
                    Win Rate: {((selectedPlayer.wins / selectedPlayer.matches.length) * 100).toFixed(1)}%
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="mobile-modal-content esoteric-modal-content">
          ...
          <div className="mobile-modal-body">
            <div className="mobile-tournament-detail">
              <h3 className="esoteric-text-accent">{selectedTournament.name || 'Unknown Tournament'}</h3>
              <p className="esoteric-text-muted">
                Date: {selectedTournament.date ? new Date(selectedTournament.date).toLocaleDateString() : 'No date'}
              </p>
              <p className="esoteric-text-muted">Location: {selectedTournament.location || 'No location'}</p>
              <p className="esoteric-text-muted">
                Players: {tournamentPlayers.length} / {selectedTournament.maxPlayers || 32}
              </p>
              <p className="esoteric-text-muted">
                ...
                <div className="mobile-tournament-players">
                  ...
                  <div className="mobile-player-chips">
                    {tournamentPlayers.map(player => (
                      <div key={player.id} className="mobile-player-chip esoteric-chip">
                        {player.name || 'Unknown'}
                      </div>
                    ))}
                  </div>
              ...
              {tournamentMatches.length > 0 && (
                <div className="mobile-tournament-matches">
                  ...
                  <p className="esoteric-text-muted">
                    Completed: {tournamentMatches.filter(m => m.result).length} / {tournamentMatches.length}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="mobile-match-detail">
              <div className="mobile-match-players-detail">
                <div className="mobile-player-card esoteric-player-card">
                  <h3 className="esoteric-text-accent">{player1?.name || 'Unknown'}</h3>
                  <p className="esoteric-text-muted">{player1?.deckName || 'No deck'}</p>
                </div>
                
                <div className="mobile-vs-badge esoteric-vs-badge">VS</div>
                
                <div className="mobile-player-card esoteric-player-card">
                  <h3 className="esoteric-text-accent">{player2?.name || 'Unknown'}</h3>
                  <p className="esoteric-text-muted">{player2?.deckName || 'No deck'}</p>
                </div>
              </div>
              ...
              <div className="mobile-match-meta">
                ...
                <p className="esoteric-text-muted">
                  Round: {selectedMatch.round || 1} • Table: {selectedMatch.table || 1}
                </p>
                <p className="esoteric-text-muted">
                ...
                {selectedMatch.result && (
                  <p className="esoteric-text-accent">
                    Result: {
                      selectedMatch.result === 'player1' ? `${player1?.name || 'Player 1'} won` :
                      selectedMatch.result === 'player2' ? `${player2?.name || 'Player 2'} won` :
                      'Draw'
                    }
                  </p>
                )}
              ...
              {!selectedMatch.result && (
                <div className="mobile-match-result-form">
                  ...
                  <div className="mobile-result-buttons">
                    <button
                      onClick={() => handleRecordResult(selectedMatch.id, 'player1')}
                      className="mobile-btn esoteric-btn"
                    >
                      {player1?.name || 'Player 1'} Won
                    </button>
                    <button
                      onClick={() => handleRecordResult(selectedMatch.id, 'player2')}
                      className="mobile-btn esoteric-btn"
                    >
  ...
  }, [selectedMatch, players, tournaments, handleRecordResult, generateMatchQR, handleDeleteMatch]);
        ...
        <div className="mobile-modal-content esoteric-modal-content">
          ...
          <div className="mobile-modal-body">
            <div className="mobile-import-form">
              <p className="esoteric-text-muted">
                Paste the exported JSON data below to import players, tournaments, and matches.
              </p>
