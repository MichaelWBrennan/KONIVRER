  ...
  return [
    ...
    // When a player has 3 or more Familiars
    {
      ...
      check: (gameState, event, card, sourceCard) => {
        return event.type === 'cardPlayed' && 
               ...
               gameState.players[event.player].field.filter(c => c.type === 'Familiar').length >= 3;
      }
    },
    ...
    // When a player has 5 or more Familiars
    {
      ...
      check: (gameState, event, card, sourceCard) => {
        return event.type === 'cardPlayed' && 
               event.card.type === 'Familiar' &&
               gameState.players[event.player].field.filter(c => c.type === 'Familiar').length >= 5;
      }
    },
    ...
    // When a player has 10 or more cards in their graveyard
    {
      ...
      check: (gameState, event, card, sourceCard) => {
        return (event.type === 'cardDestroyed' || event.type === 'spellResolved') && 
    ...
    // At the beginning of a player's turn
    {
      ...
      check: (gameState, event, card, sourceCard) => {
        return event.type === 'phaseChange' && 
               event.newPhase === 'start';
      }
    },
