...
export function getKeywordRules() {
  return [
    ...
    {
      keyword: 'Flying',
      description: 'This Familiar can only be blocked by Familiars with Flying or Reach.',
    ...
    // First Strike
    {
      ...
      description: 'This Familiar deals combat damage before Familiars without First Strike.',
      ...
      modifyCombatDamage: (gameState, attacker, blocker) => {
        ...
        if (blocker && blocker.keywords && blocker.keywords.includes('First Strike')) {
    ...
    // Double Strike
    {
      ...
      description: 'This Familiar deals combat damage both before and with regular combat damage.',
      ...
      modifyCombatDamage: (gameState, attacker, blocker) => {
        ...
        } else if (attacker.keywords && attacker.keywords.includes('First Strike')) {
          attacker.damageStep = 'first';
        } else {
          attacker.damageStep = 'normal';
        }
        
        if (blocker && blocker.keywords && blocker.keywords.includes('Double Strike')) {
          blocker.damageStep = 'both';
        } else if (blocker && blocker.keywords && blocker.keywords.includes('First Strike')) {
          blocker.damageStep = 'first';
        } else if (blocker) {
          blocker.damageStep = 'normal';
        }
    ...
    {
      keyword: 'Trample',
      description: 'This Familiar can deal excess combat damage to the opponent.',
      ...
      modifyCombatDamage: (gameState, attacker, blocker, damage) => {
        ...
        if (attacker.keywords && attacker.keywords.includes('Trample')) {
          ...
          if (excessDamage > 0) {
            ...
            const opponentIndex = 1 - gameState.players.findIndex(p => 
              p.field.some(card => card.id === attacker.id)
            );
            
            const opponent = gameState.players[opponentIndex];
            
            if (opponent.lifeCards.length > 0) {
    ...
    // Lifelink
    {
      ...
      triggerCondition: (gameState, event, card) => {
        ...
        return (event.type === 'combatDamage' || event.type === 'abilityDamage') && 
    ...
    {
      keyword: 'Deathtouch',
      description: 'Any amount of damage this Familiar deals to another Familiar is enough to destroy it.',
      
      // Modify combat damage
      modifyCombatDamage: (gameState, attacker, blocker) => {
        // If attacker has Deathtouch, any damage is lethal
        if (attacker.keywords && attacker.keywords.includes('Deathtouch') && blocker) {
        ...
        if (blocker && blocker.keywords && blocker.keywords.includes('Deathtouch')) {
    ...
    {
      keyword: 'Hexproof',
      description: 'This Familiar can\'t be the target of spells or abilities your opponents control.',
    ...
    // Indestructible
    {
      ...
      description: 'This Familiar can\'t be destroyed by damage or effects that say "destroy".',
      
      // Replacement effect
      replacementCondition: (gameState, event, card) => {
        // Triggers when a Familiar with Indestructible would be destroyed
        return (event.type === 'destroy' || event.type === 'lethalDamage') && 
    ...
    {
      keyword: 'Menace',
      description: 'This Familiar can\'t be blocked except by two or more Familiars.',
    ...
    {
      keyword: 'Protection',
      description: 'This Familiar can\'t be damaged, enchanted, equipped, blocked, or targeted by the specified quality.',
      ...
      canBeTargeted: (target, source) => {
        ...
        if (target.keywords && target.keywords.includes('Protection') && target.protectionFrom) {
          // Check if protection applies to source
          for (const protection of target.protectionFrom) {
            // Protection from color
            if (source.color === protection) {
              return false;
            }
            
            // Protection from type
            if (source.type === protection) {
              return false;
            }
          }
        }
      ...
      canBeBlockedBy: (attacker, blocker) => {
        ...
        if (attacker.keywords && attacker.keywords.includes('Protection') && attacker.protectionFrom) {
          // Check if protection applies to blocker
          for (const protection of attacker.protectionFrom) {
            // Protection from color
            if (blocker.color === protection) {
              return false;
            }
          }
        }
      ...
      canBlock: (blocker, attacker) => {
        ...
        if (blocker.keywords && blocker.keywords.includes('Protection') && blocker.protectionFrom) {
          // Check if protection applies to attacker
          for (const protection of blocker.protectionFrom) {
            // Protection from color
            if (attacker.color === protection) {
              return false;
            }
          }
        }
