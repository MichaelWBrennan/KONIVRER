...
export const useSecurityAudit = () => {
  ...
  const runSecurityAudit = (): VulnerabilityReport => {
    ...
    checks.push({
      ...
      recommendation: window.location.protocol !== 'https:' ? 'Enable HTTPS for production deployment' : undefined
    });
    ...
    checks.push({
      ...
      recommendation: presentHeaders.length < securityHeaders.length ? 'Add missing security headers' : undefined
    });
    ...
    checks.push({
      ...
      recommendation: !isEncrypted ? 'Enable data encryption for sensitive information' : undefined
    });
    ...
    checks.push({
      ...
      recommendation: !hasConsent ? 'Obtain user consent before processing data' : undefined
    });
    ...
    checks.push({
      ...
      recommendation: !sessionId ? 'Implement secure session management' : undefined
    });

    // Check for potential XSS vulnerabilities
    const hasUnsafeInnerHTML = document.querySelectorAll('[data-unsafe]').length > 0;
    checks.push({
      id: 'xss-protection',
      name: 'XSS Protection',
      status: !hasUnsafeInnerHTML ? 'pass' : 'fail',
      description: 'No unsafe innerHTML usage detected',
      recommendation: hasUnsafeInnerHTML ? 'Sanitize all user inputs and avoid unsafe innerHTML' : undefined
    });
        ...
        {lastAudit && (
          <>
            <div style={{
              ...
              <h3 style={{ margin: '0 0 10px 0', color: '#333' }}>Overall Security Score</h3>
              <div style={{
                fontSize: '48px',
                fontWeight: 'bold',
            ...
            <div>
              <h3 style={{ color: '#333', marginBottom: '15px' }}>Security Checks</h3>
              {lastAudit.checks.map(check => (
                ...
                >
                  ...
                  <p style={{ margin: '0 0 8px 0', color: '#666' }}>{check.description}</p>
