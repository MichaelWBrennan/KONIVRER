...
const PlayGame = (): any => {
  ...
  const [opponentType, setOpponentType] = useState(false) // 'ai', 'player', 'friend'
  const [aiDifficulty, setAiDifficulty] = useState(false)
  const [aiPersonality, setAiPersonality] = useState(false)
  const [aiArchetype, setAiArchetype] = useState(false)
  const [aiDeck, setAiDeck] = useState(false)
  const [isGeneratingDeck, setIsGeneratingDeck] = useState(false)
  const [gameOptions, setGameOptions] = useState(false)
  
  // Mock decks data
  const playerDecks = [
    { id: 1, name: 'Fire Aggro', colors: ['fire'
  ], winRate: '58%', lastPlayed: '2 days ago' 
  },
  ...
  const startGame = (): any => {
    ...
    format: selectedFormat,
      ...
      opponentType,
      gameOptions,
      aiSettings: opponentType === 'ai' ? { : null
  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4" /></div>
                {aiArchetypeOptions.map(archetype => (
                  ...
                  >``
                    <div className="flex items-center mb-2" />```
                      <div className={`w-4 h-4 rounded-full ${getColorIndicator([archetype.element])[0].props.className} mr-2`} />```
    ...
    <span>Generating AI deck...</span> : null
              ) : aiDeck ? (
    ...
    <h3 className="font-medium">{aiDeck.name}
                    ...
                    <button 
                      onClick={generateAIDeckWithDelay} : null
    ...
    <span>{aiPersonalityOptions.find(o => o.id === aiPersonality)?.name}
                ...
                <div className="flex items-center justify-center p-4 bg-gray-700 rounded-lg text-gray-400" />
    ...
    <span /></span>``
              {opponentType === 'ai' ```
                ? `Play Against ${aiDifficultyOptions.find(o => o.id === aiDifficulty)?.name} AI`  : null
                : 'Start Game'}
            </span>
        </div>
        ...
        {/* AI Disclaimer */}
        {opponentType === 'ai' && (
          <div className="mt-4 text-center text-sm text-gray-400 max-w-2xl mx-auto" />
    <p /></p>
