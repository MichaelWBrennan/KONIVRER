...
const IntegratedDeckSystem = () => {
  ...
  const filteredDecks = mockDecklists.filter(deck => {
    ...
    const matchesFilters = 
      ...
      (filters.colors.length === 0 || filters.colors.every(color => deck.colors.includes(color)));
  ...
  const paginatedDecks = sortedDecks.slice(startIndex, startIndex + decksPerPage);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400">Discover, analyze, and build the perfect deck</p>
        </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 text-gray-400 mt-1">
                    ...
                    <span className="text-yellow-400">({deck.authorRating})</span>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1">
                  ...
                  <span className="text-xs text-gray-400">{deck.gamesPlayed} games</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white font-medium">{deck.archetype}</span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white font-medium">{deck.format}</span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white font-medium">#{deck.placement}</span>
                  </div>
                </div>
              </div>
                  ...
                  {deck.colors.map((color) => (
                    <div
                      ...
                      className={`w-6 h-6 rounded-full border-2 border-gray-600 ${
                        color === 'Fire' ? 'bg-red-500' :
                        color === 'Water' ? 'bg-blue-500' :
                        color === 'Earth' ? 'bg-green-500' :
                        color === 'Air' ? 'bg-gray-300' :
                        color === 'Lightning' ? 'bg-yellow-500' :
                        color === 'Nature' ? 'bg-emerald-500' :
                        'bg-gray-500'
                      }`}
                      title={color}
                    />
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">{selectedDeck.name}</h1>
            <p className="text-gray-400">Detailed deck analytics and performance metrics</p>
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-green-400">{selectedDeck.winRate}%</div>
              <div className="text-gray-400">Win Rate</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-400">{selectedDeck.gamesPlayed}</div>
              <div className="text-gray-400">Games Played</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-400">{selectedDeck.metaShare}%</div>
              <div className="text-gray-400">Meta Share</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-400">#{selectedDeck.placement}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div key={cost} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-gray-700 rounded-full h-6 relative">
                    <div
                      ...
                      style={{ width: `${(count / Math.max(...analytics.manaCurve)) * 100}%` }}
                    />
          ...
          <AnalyticsCard title="Element Distribution" icon={PieChart}>
            <div className="space-y-3">
              {Object.entries(analytics.elementDistribution).map(([element, percentage]) => (
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-4 h-4 rounded-full ${
                        ...
                        element === 'Air' ? 'bg-gray-300' :
                        element === 'Lightning' ? 'bg-yellow-500' :
                        element === 'Nature' ? 'bg-emerald-500' :
                        'bg-gray-500'
                      }`}
                    />
          ...
          <AnalyticsCard title="Rarity Breakdown" icon={Star}>
            <div className="space-y-3">
              {Object.entries(analytics.rarityDistribution).map(([rarity, percentage]) => (
            ...
            <div className="space-y-3">
              {analytics.matchups.map((matchup, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div>
                    <div className="text-white font-medium">{matchup.archetype}</div>
                    <div className="text-xs text-gray-400">{matchup.games} games</div>
                  </div>
                  <div className={`text-lg font-bold ${
                    matchup.winRate >= 60 ? 'text-green-400' :
                    matchup.winRate >= 40 ? 'text-yellow-400' :
                    'text-red-400'
                  }`}>
                    {matchup.winRate}%
                  </div>
            ...
            {selectedDeck.cards.map((card, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg">
                <div className="flex items-center gap-3">
                  <span className="text-lg font-bold text-blue-400">{card.count}x</span>
                  <div>
    ...
    <div className="space-y-6">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <div className="text-white font-medium">{deck.cardCount}/60</div>
              </div>
              <div>
                <span className="text-gray-400 text-sm">Format</span>
              ...
              <div>
                ...
                <div className={`font-medium ${deck.gamesPlayed > 0 ? 'text-green-400' : 'text-gray-400'}`}>
                  {deck.gamesPlayed > 0 ? `${deck.winRate}%` : 'No games'}
                </div>
              </div>
              <div>
                <span className="text-gray-400 text-sm">Status</span>
                <div className={`font-medium ${deck.isPublic ? 'text-green-400' : 'text-yellow-400'}`}>
      ...
      {mockUserDecks.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 mb-6">Start building your first deck or save one from the community</p>
          <Link
            to="/deckbuilder"
