...
export const Settings: React.FC : any : any : any = () => {
  const [theme, setTheme] : any : any : any = useState<'dark'|'light'>(() => (document.documentElement.getAttribute('data-theme') as any) || 'dark');
  const [contrast, setContrast] : any : any : any = useState<string>(() => document.documentElement.getAttribute('data-contrast') || 'normal');
  const [fontSize, setFontSize] : any : any : any = useState<string>(() => document.documentElement.getAttribute('data-font-size') || 'medium');
  ...
  const clearCaches : any : any : any = async () => {
    if ('caches' in window) {
      const names : any : any : any = await caches.keys();
