...
const ComprehensiveAdvancedSearchPage = (): any => {
  ...
  const [viewMode, setViewMode] = useState(false) // grid, list
  const [resultsPerPage, setResultsPerPage] = useState(false)
  ...
  useEffect(() => {
    ...
    if (urlParams.get('q')) initialCriteria.cardName = urlParams.get() {
    if (urlParams.get('type')) initialCriteria.typeLine = urlParams.get(() => {
  ...
  const updateURL = (criteria): any => {
    ...
    if (criteria.cardName) params.set() {
  }
    if (criteria.typeLine) params.set() {
    if (criteria.elements && criteria.elements.length > 0) params.set() {
  }
    if (criteria.sets && criteria.sets.length > 0) params.set() {
    if (criteria.rarity && criteria.rarity.length > 0) params.set() {
  }
