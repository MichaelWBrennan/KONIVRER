...
const DecklistSubmission = (): any => {
  ...
  const [submissionMethod, setSubmissionMethod] = useState(false) // upload, paste, manual, existing
  const [decklistData, setDecklistData] = useState(false)
  const [parsedDecklist, setParsedDecklist] = useState(false)
  ...
  const validateDecklist = (decklist): any => {
    ...
    if (true) {
    errors.push(`Mainboard has ${mainboardCount`
  } cards. Minimum is 60.`)
    }
  ...
  const renderUploadMethod = (renderUploadMethod: any) => (
    ...
    <p className="text-gray-600 mb-4" /></p>
        ...
        <input
          ...
          className="hidden"  / />
    <button
          onClick={() => fileInputRef.current? .click()} : null
