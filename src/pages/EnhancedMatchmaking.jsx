  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(player => 
        ...
        (player.deckArchetype && player.deckArchetype.toLowerCase().includes(query))
      );
    }
    ...
    if (filterOptions.confidenceBand !== 'all') {
      result = result.filter(player => player.confidenceBand === filterOptions.confidenceBand);
    }
    
    // Apply deck archetype filter
    if (filterOptions.deckArchetype !== 'all') {
      result = result.filter(player => 
        player.deckArchetype && player.deckArchetype.includes(filterOptions.deckArchetype)
      );
    }
  ...
  const findMatch = () => {
    ...
    setTimeout(() => {
      ...
      const calculateMatchQuality = (player) => {
        ...
        const currentBandIndex = confidenceBandOrder.indexOf(currentPlayer.confidenceBand);
        const playerBandIndex = confidenceBandOrder.indexOf(player.confidenceBand);
        ...
        if (currentPlayer.deckArchetype && player.deckArchetype) {
          ...
          archetypeScore = currentPlayer.deckArchetype === player.deckArchetype ? 0.3 : 0.7;
        }
      ...
      setCurrentMatch({
        ...
        matchupDetails: {
          skillDifference: Math.abs(currentPlayer.rating - bestMatch.player.rating),
          confidenceBandMatch: currentPlayer.confidenceBand === bestMatch.player.confidenceBand,
  ...
  return (
    <div className="min-h-screen py-8">
      <div className="container max-w-6xl">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800">Enhanced Matchmaking</h1>
          <p className="text-gray-600 mt-2">
            Find matches based on skill, confidence, deck archetype, and playstyle
          </p>
        </div>
            ...
            {showPreferences && (
              ...
              >
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <h3 className="text-lg font-medium text-gray-900 mb-3">Matchmaking Preferences</h3>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <h4 className="text-sm font-medium text-gray-700 mb-2">Priority Factors</h4>
                      ...
                      <div className="space-y-2">
                        <label className="flex items-center">
                          <input
                            ...
                            onChange={(e) => setMatchmakingPreferences({
                              ...matchmakingPreferences,
                              prioritizeSkill: e.target.checked
                            })}
                          />
                          <span className="ml-2 text-sm text-gray-700">Prioritize skill rating similarity</span>
                        </label>
                        ...
                        <label className="flex items-center">
                          <input
                            type="checkbox"
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                            checked={matchmakingPreferences.prioritizeConfidence}
                            onChange={(e) => setMatchmakingPreferences({
                              ...matchmakingPreferences,
                              prioritizeConfidence: e.target.checked
                            })}
                          />
                          <span className="ml-2 text-sm text-gray-700">Prioritize confidence band matching</span>
                        </label>
                        ...
                        <label className="flex items-center">
                          <input
                            type="checkbox"
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                            checked={matchmakingPreferences.prioritizeDeckArchetype}
                            onChange={(e) => setMatchmakingPreferences({
                              ...matchmakingPreferences,
                              prioritizeDeckArchetype: e.target.checked
                            })}
                          />
                          <span className="ml-2 text-sm text-gray-700">Consider deck archetype matchups</span>
                        </label>
                        ...
                        <label className="flex items-center">
                          ...
                          />
                          <span className="ml-2 text-sm text-gray-700">Consider playstyle compatibility</span>
                        </label>
                      </div>
                    </div>
                    
                    <div>
                      <h4 className="text-sm font-medium text-gray-700 mb-2">Matching Preferences</h4>
                      ...
                      <div className="space-y-2">
                        <label className="flex items-center">
                          <input
                            type="checkbox"
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                            checked={matchmakingPreferences.preferComplementaryPlaystyles}
                            onChange={(e) => setMatchmakingPreferences({
                              ...matchmakingPreferences,
                              preferComplementaryPlaystyles: e.target.checked
                            })}
                          />
                          <span className="ml-2 text-sm text-gray-700">Prefer complementary playstyles</span>
                        </label>
                        ...
                        <label className="flex items-center">
                          <input
                            type="checkbox"
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                            checked={matchmakingPreferences.preferSimilarConfidence}
                            onChange={(e) => setMatchmakingPreferences({
                              ...matchmakingPreferences,
                              preferSimilarConfidence: e.target.checked
                            })}
                          />
                          <span className="ml-2 text-sm text-gray-700">Prefer similar confidence bands</span>
                        </label>
                        
                        <div>
                          <label className="block text-sm text-gray-700 mb-1">Maximum wait time</label>
            ...
            >
              <div className="bg-blue-50 border border-blue-200 rounded-lg shadow-md p-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium text-blue-800">Match Found!</h3>
                  <div className="flex items-center">
                ...
                <div className="bg-white rounded-lg p-3 mb-4">
                  <h4 className="text-sm font-medium text-gray-700 mb-2">Matchup Details</h4>
                  
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    <div>
                      <div className="text-gray-500">Skill Difference</div>
                      <div className="font-medium">{currentMatch.matchupDetails.skillDifference} points</div>
                    </div>
                    <div>
                      ...
                      <div className="font-medium">
                        {currentMatch.matchupDetails.confidenceBandMatch ? (
                          ...
                          <span className="text-orange-600">Different bands</span>
                        )}
                      </div>
                    </div>
                    <div>
                      <div className="text-gray-500">Archetype Matchup</div>
                      <div className="font-medium">{currentMatch.matchupDetails.archetypeMatchup}</div>
                    </div>
                    <div>
                      <div className="text-gray-500">Est. Win Probability</div>
                      <div className="font-medium">{(currentMatch.matchupDetails.estimatedWinRate * 100).toFixed(1)}%</div>
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Confidence Band</label>
              <select
                className="block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Deck Archetype</label>
              <select
                className="block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
