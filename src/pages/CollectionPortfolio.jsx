...
const CollectionPortfolio = () => {
  ...
  const portfolioStats = useMemo(() => {
    const totalValue = collection.reduce((sum, card) => sum + card.totalValue, 0);
    const totalCost = collection.reduce((sum, card) => sum + (card.avgCost * card.quantity), 0);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">
            Track your Magic collection's value, monitor price changes, and analyze your investment performance
          </p>
        </motion.div>
        ...
        >
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-6 border border-slate-600">
            ...
            <p className="text-3xl font-bold text-green-400">{formatCurrency(portfolioStats.totalValue)}</p>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-6 border border-slate-600">
            ...
            <p className={`text-3xl font-bold ${portfolioStats.totalGainLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {portfolioStats.totalGainLoss >= 0 ? '+' : ''}{formatCurrency(portfolioStats.totalGainLoss)}
            </p>
            <p className={`text-sm mt-1 ${portfolioStats.avgGainLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {portfolioStats.avgGainLoss >= 0 ? '+' : ''}{portfolioStats.avgGainLoss.toFixed(1)}%
            </p>
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-6 border border-slate-600">
            ...
            <p className="text-3xl font-bold text-purple-400">{portfolioStats.totalCards.toLocaleString()}</p>
            <p className="text-sm text-gray-400 mt-1">
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-6 border border-slate-600">
            ...
            <div className="flex items-center gap-4">
              <div className="text-center">
                <p className="text-xl font-bold text-green-400">{portfolioStats.gainers}</p>
                <p className="text-xs text-gray-400">Gainers</p>
              </div>
              <div className="text-center">
                <p className="text-xl font-bold text-red-400">{portfolioStats.losers}</p>
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {card.foil && <Star className="w-4 h-4 text-yellow-400" />}
                      </div>
                ...
                <div className="lg:col-span-1 text-center">
                  ...
                  <div className="text-2xl font-bold text-white">{card.quantity}</div>
                </div>

                {/* Current Price */}
                <div className="lg:col-span-2 text-center">
                  <div className="text-sm text-gray-400 mb-1">Current Price</div>
                  <div className="text-xl font-bold text-blue-400">{formatCurrency(card.currentPrice)}</div>
                  <div className="text-sm text-gray-400">Avg Cost: {formatCurrency(card.avgCost)}</div>
                </div>

                {/* Total Value */}
                <div className="lg:col-span-2 text-center">
                  <div className="text-sm text-gray-400 mb-1">Total Value</div>
                  <div className="text-xl font-bold text-green-400">{formatCurrency(card.totalValue)}</div>
                </div>
                ...
                <div className="lg:col-span-2 text-center">
                  ...
                  <div className={`text-xl font-bold flex items-center justify-center gap-1 ${
                    ...
                    {card.priceChange > 0 ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
              ...
              <div className="mt-4">
                <div className="flex justify-between text-sm text-gray-400 mb-1">
                  ...
                  <span>{((card.currentPrice - card.avgCost) / card.avgCost * 100).toFixed(1)}%</span>
                </div>
