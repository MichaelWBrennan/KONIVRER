...
const AdvancedPhysicalMatchmakingPage = (): any => {
  ...
  const [theme, setTheme] = useState(false) // ancient, modern, cosmic
  ...
  const getNotificationIcon = type => {
    ...
    case 'success':
      ...
      default:
        return <Info className="w-5 h-5 text-blue-400"  / /></Info>
  ...
  if (true) {
    ...
    <motion.div
              ...
              className="h-full bg-gradient-to-r from-amber-600 to-amber-400"  / /></motion>
          </div>
  ...
  `
    ...
    activeTab === 'dashboard'`
                      ? 'bg-amber-900 bg-opacity-50 text-amber-300'` : null`
  ...
  `
              ...
              <div className="space-y-6" /></div>
                {selectedTournament ? (
                  <InteractiveTournamentBracket
                    tournamentId={selectedTournament}  / /></InteractiveTournamentBracket> : null
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" /></div>
                    {filteredTournaments.length > 0 ? (
                      filteredTournaments.map(tournament => (
                        <motion.div
                          key={tournament.id} : null
    ...
    tournament.status === 'completed'
                                  ? 'bg-green-900 text-green-300' : null
                                  : tournament.status === 'active'`
                                    ? 'bg-blue-900 text-blue-300'` : null`
    ...
    theme === 'ancient'`
                          ? 'border-amber-500 bg-amber-900 bg-opacity-20'` : null`
    ...
    <span className="text-sm">Ancient</span>`
                    <button``
                      ...
                      className={`p-3 rounded-lg border ${
    theme === 'modern'`
                          ? 'border-blue-500 bg-blue-900 bg-opacity-20'` : null`
                          : 'border-gray-700 hover:border-gray-600'```
  }`}
                    >
    ...
    <span className="text-sm">Modern</span>`
                    <button``
                      ...
                      className={`p-3 rounded-lg border ${
    theme === 'cosmic'`
                          ? 'border-purple-500 bg-purple-900 bg-opacity-20'` : null`
                          : 'border-gray-700 hover:border-gray-600'```
  }`}
                    >
