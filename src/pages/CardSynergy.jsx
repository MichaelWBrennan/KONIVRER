  ...
  const cards = [
    {
      ...
      description: "If an effect would create one or more tokens under your control, it creates twice that many of those tokens instead.",
      playRate: 15.8,
      synergyCards: [
        { 
          name: "Parallel Lives", 
  ...
  const filteredCards = useMemo(() => {
    return cards.filter(card => {
      const matchesSearch = card.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        card.themes.some(theme => theme.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      switch (sortBy) {
        case 'synergy': return b.synergyCards[0]?.synergy - a.synergyCards[0]?.synergy;
        case 'popularity': return b.playRate - a.playRate;
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">
            Discover powerful card combinations and synergies based on real deck data and community insights
          </p>
        </motion.div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm text-gray-300">{card.playRate}%</span>
                        </div>
            ...
            {selectedCard ? (
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-6 border border-slate-600">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <TrendingUp className="w-4 h-4 text-green-400" />
                        <span className="text-gray-300">{selectedCard.playRate}% play rate</span>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedCard.themes.map((theme, index) => (
                      <span key={index} className="px-3 py-1 bg-purple-600/30 text-purple-300 rounded-lg">
                        {theme}
                      </span>
                    ))}
                  </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <h4 className="text-lg font-bold text-white">{synCard.name}</h4>
                              <span className="px-2 py-1 bg-blue-600/30 text-blue-300 text-sm rounded">
                                {synCard.type}
                              </span>
                            </div>
                            
                            <p className="text-gray-300 text-sm mb-3">{synCard.reason}</p>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="text-center">
                                ...
                                <div className={`text-xl font-bold ${getSynergyColor(synCard.synergy)}`}>
                                  {synCard.synergy}%
                                </div>
                              </div>
                              ...
                              <div className="text-center">
                                ...
                                <div className="text-xl font-bold text-green-400">{synCard.inclusion}%</div>
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="space-y-3">
                    {selectedCard.commanders.map((commander, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                        <div>
                          <h4 className="font-bold text-white">{commander.name}</h4>
                          <div className="text-sm text-gray-400">{commander.decks.toLocaleString()} decks</div>
                        </div>
                        <div className="text-right">
                          <div className={`text-lg font-bold ${getSynergyColor(commander.synergy)}`}>
                            {commander.synergy}%
                          </div>
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-6 border border-slate-600">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-3 p-3 bg-blue-900/20 border border-blue-500 rounded-lg">
                      ...
                      <div>
                        <h4 className="font-bold text-blue-400 mb-1">Deck Building Tip</h4>
                        <p className="text-gray-300 text-sm">
                          Include multiple cards from the same synergy package to increase consistency and power level.
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3 p-3 bg-green-900/20 border border-green-500 rounded-lg">
                      ...
                      <div>
                        <h4 className="font-bold text-green-400 mb-1">Meta Consideration</h4>
                        <p className="text-gray-300 text-sm">
                          High synergy cards often become priority removal targets. Include protection spells.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-center h-96 bg-slate-800/30 rounded-lg border-2 border-dashed border-slate-600">
                <div className="text-center">
                  ...
                  <h3 className="text-xl font-bold text-gray-400 mb-2">Select a Card</h3>
                  <p className="text-gray-500">Choose a card from the list to explore its synergies and combinations</p>
                </div>
              </div>
            )}
          </motion.div>
