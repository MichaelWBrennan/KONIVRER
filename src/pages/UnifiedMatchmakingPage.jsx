...
const UnifiedMatchmakingPage = () => {
  ...
  const [rankingEngine] = useState(() => new EnhancedRankingEngine({
  ...
  useEffect(() => {
    if (isAuthenticated && !loading) {
      try {
        ...
        if (mockPlayerData.deckArchetypes && mockPlayerData.deckArchetypes.length > 0) {
  ...
  function generateMatchHistory() {
    ...
    for (let i = 0; i < 75; i++) {
      const date = new Date(startDate.getTime() + (i * 24 * 60 * 60 * 1000 / 75 * 90));
      const gameResult = Math.random() > 0.4 ? 'win' : (Math.random() > 0.5 ? 'loss' : 'draw');
      
      // Simulate rating changes
      const ratingChange = gameResult === 'win' ? Math.random() * 20 + 5 :
                          gameResult === 'loss' ? -(Math.random() * 20 + 5) :
                          Math.random() * 10 - 5;
  ...
  const calculatePlaystyleCompatibility = (playstyle1, playstyle2) => {
    ...
    const factors = ['aggression', 'consistency', 'complexity', 'adaptability', 'riskTaking'];
  ...
  // Calculate win probability
  const calculateWinProbability = (playerRating, playerUncertainty, opponentRating, opponentUncertainty, deckArchetype, playstyleCompatibility) => {
    // Base win probability from ratings
    const combinedUncertainty = Math.sqrt(playerUncertainty * playerUncertainty + opponentUncertainty * opponentUncertainty);
    ...
    return Math.max(0.01, Math.min(0.99, baseWinProb + playstyleAdjustment + deckMatchupAdjustment));
  };

  // Get random archetype different from player's
  const getRandomArchetype = (playerArchetype) => {
    const archetypes = ['Aggro', 'Control', 'Midrange', 'Combo', 'Tempo', 'Ramp'];
  ...
  return (
    <div className="mobile-container esoteric-bg">
      ...
      <div className="mobile-page-header esoteric-page-header">
        <h1 className="mobile-page-title esoteric-page-title">Advanced Matchmaking</h1>
            ...
            {!isSearching && !matchFound && (
              <div className="mobile-matchmaking-form esoteric-card">
                ...
                <div className="mobile-form-group">
                  ...
                  >
                    {playerData && playerData.deckArchetypes && playerData.deckArchetypes.map(deck => (
                      <option key={deck.archetype} value={deck.archetype}>
                        {deck.archetype} ({Math.round(deck.rating)} ±{Math.round(deck.uncertainty)})
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="mobile-match-found esoteric-card">
                ...
                <div className="mobile-match-players">
                  <div className="mobile-player-card esoteric-player-card">
                    ...
                    <p className="esoteric-text-muted">
                      Rating: {Math.round(matchData.player.rating)} ±{Math.round(matchData.player.uncertainty)}
                    </p>
                    <p className="esoteric-text-muted">
                  ...
                  <div className="mobile-player-card esoteric-player-card">
                    ...
                    <p className="esoteric-text-muted">
                      Rating: {Math.round(matchData.opponent.rating)} ±{Math.round(matchData.opponent.uncertainty)}
                    </p>
                    <p className="esoteric-text-muted">
