...
export const TournamentHub: React.FC = () => {
  ...
  return (
    <div className={s.root}>
        ...
        <div>
          ...
          {current && (
            <div className={s.pairingItem}>
              ...
              <div>
                {current.playerA.name} vs {current.playerB.name}
              </div>
            </div>
          )}
          ...
          <form
            ...
            onSubmit={async (e) => {
              e.preventDefault();
              await EventService.reportMatch({
                eventId: currentEventId || "demo",
                round: roundNumber || 1,
                table: report.table,
          ...
          >
            <input
              ...
              value={report.table}
              onChange={(e) =>
                setReport((r) => ({ ...r, table: Number(e.target.value) }))
              }
            />
            <select
              ...
              value={report.result}
              onChange={(e) =>
                setReport((r) => ({ ...r, result: e.target.value }))
              }
            >
              {["2-0", "2-1", "1-2", "0-2", "ID"].map((x) => (
            ...
            <input
              className="search-input"
              placeholder="Winner player ID (optional)"
              value={report.winnerId}
              onChange={(e) =>
                setReport((r) => ({ ...r, winnerId: e.target.value }))
              }
            />
          ...
          {filtered.map((p) => (
            <div key={p.tableNumber} className={s.pairingItem}>
              ...
              <div>
                {p.playerA.name} vs {p.playerB.name}
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === "standings" && <Standings query={query} />}
    </div>
  );
};
    ...
    <div>
      {list.map((row) => (
        <div key={row.rank} className={s.pairingItem}>
          ...
          <div>
            {row.points} pts • OMW {Math.round(row.omw * 100)}%
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        e.preventDefault();
        if (!deckId) return;
        await EventService.registerDeck(eventId, deckId);
        alert("Deck registered to event");
      }}
    >
      <input
        className="search-input"
        placeholder="Deck ID to register"
        value={deckId}
