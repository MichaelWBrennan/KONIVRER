...
export const TournamentHub: React.FC : any : any : any : any = () => {
  const [tab, setTab] : any : any : any : any = useState<EventsTab>('live');
  const [query, setQuery] : any : any : any : any = useState('');
  const { currentEventId, roundNumber, roundEndsAt, pairings, myTable, setPairings, setMyTableFromPairings, setLoading, setError } : any : any : any : any = useEventStore();

  useEffect(() => {
    const handler : any : any : any : any = (e: any) => setQuery(e.detail || '');
  ...
  useEffect(() => {
    ...
    const ctx : any : any : any : any = tab === 'archive' ? 'event-archive' : tab === 'standings' ? 'event-standings' : 'events';
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const data : any : any : any : any = await EventService.fetchPairings(currentEventId || 'demo', roundNumber || 1);
        setPairings(data);
        setMyTableFromPairings();
        const mine : any : any : any : any = data.find(d => d.tableNumber === (useEventStore.getState().myTable || 0));
        if (mine) {
  ...
  const filtered : any : any : any : any = useMemo(() => {
    const q : any : any : any : any = query.toLowerCase().trim();
  ...
  const current : any : any : any : any = useMemo(() => {
    if (!myTable) return undefined;
    return pairings.find(p => p.tableNumber === myTable);
  }, [pairings, myTable]);

  const [report, setReport] : any : any : any : any = useState({ table: myTable || 0, result: '2-1', winnerId: '' });

  // Show timer only when roundEndsAt is provided (round started)
  const showTimer : any : any : any : any = !!roundEndsAt && roundEndsAt > Date.now();
  const remainingMs : any : any : any : any = Math.max(0, (roundEndsAt || 0) - Date.now());
  const mins : any : any : any : any = Math.floor(remainingMs / 60000);
  const secs : any : any : any : any = Math.floor((remainingMs % 60000)/1000).toString().padStart(2,'0');

  // Archive stub data
  const archived : any : any : any : any = useMemo(() => ([
    { name: 'City Championship - June', date: '2025-06-12', players: 128 },
    { name: 'Regional Open - May', date: '2025-05-03', players: 256 },
  ]), []);
  const archivedFiltered : any : any : any : any = useMemo(() => archived.filter(a => (a.name.toLowerCase().includes(query.toLowerCase()) || a.date.includes(query))), [archived, query]);
...
const Standings: React.FC<{ query: string }> : any : any : any : any = ({ query }) => {
  // demo standings; replace with API when available
  const demo : any : any : any : any = [
    { rank: 1, player: 'Alice', points: 9, omw: 0.67 },
    { rank: 2, player: 'Bob', points: 9, omw: 0.61 },
    { rank: 3, player: 'Carol', points: 6, omw: 0.58 },
  ];
  const list : any : any : any : any = demo.filter(r => r.player.toLowerCase().includes(query.toLowerCase()) || String(r.rank) === query.trim());
  return (
    <div>
      {list.map(row => (
...
const DeckRegistration: React.FC<{ eventId: string }> : any : any : any : any = ({ eventId }) => {
  const [deckId, setDeckId] : any : any : any : any = useState('');
  const { isAuthenticated } : any : any : any : any = require('../hooks/useAuth').useAuth();
