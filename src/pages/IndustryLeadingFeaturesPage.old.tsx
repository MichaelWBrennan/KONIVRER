...
const IndustryLeadingFeaturesPage = (): any => {
  ...
  const { players, tournaments, matches 
  ...
  return (
    ...
    activeFeature === feature.id`
                  ? 'border-purple-500 bg-purple-600/20 shadow-lg shadow-purple-500/25'` : null`
    ...
    <motion.div
            ...
            {activeFeature === 'overview' && (
              <FeatureOverview
                ...
                setFeaturesEnabled={setFeaturesEnabled}  / /></FeatureOverview>
            )}
            {activeFeature === 'meta-adaptive' && (
              <MetaAdaptiveFeature
                tournamentEngine={tournamentEngine}
                tournaments={tournaments}
                players={players}  / /></MetaAdaptiveFeature>
            )}
            {activeFeature === 'advanced-analytics' && (
              <AdvancedAnalyticsEngine
                players={players}
                matches={matches}
                tournaments={tournaments}  / /></AdvancedAnalyticsEngine>
            )}
            ...
            {activeFeature === 'content-creation' && (
              <ContentCreationTools
                tournament={tournaments[0]}
                matches={matches}
                players={players}  / /></ContentCreationTools>
            )}
            {activeFeature === 'bayesian-ml' && (
              <BayesianMLFeature
                bayesianEngine={bayesianEngine}
                players={players}
                matches={matches}  / /></BayesianMLFeature>
            )}
          </motion.div>
        </AnimatePresence>
    </div>
...
const MetaAdaptiveFeature: React.FC<MetaAdaptiveFeatureProps> = ({  tournamentEngine, tournaments, players  }) => {
  ...
  const [pairingResults, setPairingResults] = useState(false)
  const [tournamentStructure, setTournamentStructure] = useState(false)
  ...
  return (
    ...
    <span className="font-medium">Table {pairing.table}
                    ...
                    <div className="text-xs text-gray-500 mt-1" /></div> : null
...
const BayesianMLFeature: React.FC<BayesianMLFeatureProps> = ({  bayesianEngine, players, matches  }) => {
  ...
  const [matchPrediction, setMatchPrediction] = useState(false)
  const [ratingAnalysis, setRatingAnalysis] = useState(false)
