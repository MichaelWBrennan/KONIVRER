...
const GamePage = () => {
  ...
  const handleGameJoined = (gameInfo) => {
    ...
    if (players.length >= 2) {
      const playerIndex = players.findIndex(p => p.id === networkManagerRef.current.getPlayerId());
  ...
  const handleDisconnected = () => {
    setChatMessages(prev => [
      ...prev,
      { type: 'system', text: 'Disconnected from server. Attempting to reconnect...' }
    ]);
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex items-center">
                  <button
                    onClick={() => setGameOptions(prev => ({ ...prev, deckSize: Math.max(20, prev.deckSize - 5) }))}
                    className="bg-gray-700 text-white p-2 rounded-l-lg"
                  >
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="bg-gray-600 text-white px-4 py-2">{gameOptions.deckSize}</span>
                  <button
                    onClick={() => setGameOptions(prev => ({ ...prev, deckSize: Math.min(60, prev.deckSize + 5) }))}
              ...
              <div>
                <label className="block text-gray-400 mb-1">Starting Life</label>
                <div className="flex items-center">
                  <button
                    onClick={() => setGameOptions(prev => ({ ...prev, startingLife: Math.max(1, prev.startingLife - 1) }))}
                    className="bg-gray-700 text-white p-2 rounded-l-lg"
                  >
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="bg-gray-600 text-white px-4 py-2">{gameOptions.startingLife}</span>
                  <button
                    onClick={() => setGameOptions(prev => ({ ...prev, startingLife: Math.min(10, prev.startingLife + 1) }))}
                    className="bg-gray-700 text-white p-2 rounded-r-lg"
                  >
                    <Plus className="w-4 h-4" />
                  </button>
  ...
  const renderGameEnded = () => {
    const winner = gameState?.winner === 0 ? playerData.name : opponentData.name;
