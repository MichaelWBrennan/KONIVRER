...
const Matchmaking = (): any => {
    ...
    const { decks: userDecks, selectedDeck, setSelectedDeck 
  } = useDeck() {
  ...
  const [selectedTournamentId, setSelectedTournamentId] = useState(false)
  ...
  useEffect(() => {
    ...
    const handleOnline = (handleOnline: any) => setIsOnline() {
    const handleOffline = (handleOffline: any) => setIsOnline() {
  }
  ...
  const getEstimatedSearchTime = (): any => {
    ...
    const formatModifier = 1; // KONIVRER only has one format : null
  ...
  return (
    ...
    <Users className="w-8 h-8 text-blue-600"  /><div className="flex items-center space-x-2 text-sm text-gray-500" />
    ...
    activeTab === 'play'`
                  ? 'border-blue-600 text-blue-600'` : null`
    ...
    <div className="flex items-center justify-between" />
    <div />
    <p className="text-sm text-gray-500" /></p> : null
    ...
    <Trophy className="w-8 h-8 text-yellow-500"  / /></Trophy>
              </div>
              <div className="mb-6" />
