...
export const usePhysicalMatchmaking = () => useContext(PhysicalMatchmakingContext);
...
export const PhysicalMatchmakingProvider = ({ children }) => {
  ...
  const [players, setPlayers] = useLocalStorage(STORAGE_KEYS.PLAYERS, initialPlayers);
  const [tournaments, setTournaments] = useLocalStorage(STORAGE_KEYS.TOURNAMENTS, initialTournaments);
  const [matches, setMatches] = useLocalStorage(STORAGE_KEYS.MATCHES, initialMatches);

  // Generate QR code data for matches - memoized with useCallback
  const generateMatchQRData = useCallback((matchId) => {
    ...
    return {
      ...
      player1: {
        ...
        rating: player1 ? calculateBayesianRating(player1, matches) : DEFAULT_RATING
      },
      player2: {
        id: match.player2.id,
        name: player2?.name || 'Unknown',
        rating: player2 ? calculateBayesianRating(player2, matches) : DEFAULT_RATING
      },
      format: match.format,
  ...
  const generateTournamentQRData = useCallback((tournamentId) => {
    ...
    const participantDetails = tournament.players?.map(playerId => {
      ...
      return {
        ...
        rating: player ? calculateBayesianRating(player, matches) : DEFAULT_RATING
      };
    }) || [];
  ...
  // Update a tournament
  const updateTournament = useCallback((tournamentId, updates) => {
  ...
  const [selectedMatchId, setSelectedMatchId] = useLocalStorage(STORAGE_KEYS.SELECTED_MATCH_ID, null);
  const [selectedTournamentId, setSelectedTournamentId] = useLocalStorage(STORAGE_KEYS.SELECTED_TOURNAMENT_ID, null);
