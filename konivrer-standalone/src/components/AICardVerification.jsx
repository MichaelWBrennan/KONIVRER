...
const AICardVerification = () => {
  ...
  const cardDatabase = useMemo(() => [
    ...
    { id: 'KON003', name: 'Shadow Assassin', rarity: 'Uncommon', set: 'Core' },
    { id: 'KON004', name: 'Ethereal Dragon', rarity: 'Mythic', set: 'Expansion I' },
    { id: 'KON005', name: 'Temporal Mage', rarity: 'Rare', set: 'Expansion I' },
  ], []);
  ...
  const detectCard = useCallback(() => {
    ...
    if (Math.random() > 0.95) { // Simulate occasional card detection
      ...
      context.fillText(`${detectedCardData.name} (${detectedCardData.id})`, 10, canvas.height - 35);
      context.fillText(`Confidence: ${Math.round(detectionConfidence * 100)}%`, 10, canvas.height - 15);
    }
  ...
  return (
    <div className={`ai-card-verification ${isAncientTheme ? 'ancient-theme' : ''}`}>
      ...
      {detectedCard && (
        <div className="detected-card-info">
          ...
          <p><strong>ID:</strong> {detectedCard.id}</p>
          <p><strong>Rarity:</strong> {detectedCard.rarity}</p>
          <p><strong>Set:</strong> {detectedCard.set}</p>
