...
interface PeerMessage {
  type: 'chat' | 'gameState' | 'action' | 'offer' | 'answer' | 'ice' | 'join' | 'leave' | 'ready';
  sender: string;
  content: any;
  timestamp: number;
}
...
const WebRTCMatch: React.FC<WebRTCMatchProps> = ({
  ...
  const [networkStats, setNetworkStats] = useState<{ latency: number; packetLoss: number; bandwidth: number }>({
    latency: 0,
    packetLoss: 0,
    bandwidth: 0
  });
  ...
  const initializeAsHost = useCallback(() => {
    ...
    // Create data channel
    const dataChannel = peerConnection.connection.createDataChannel('gameChannel', {
      ordered: true
    });
      ...
      try {
        ...
        stats.forEach(report => {
          if (report.type === 'remote-inbound-rtp') {
            ...
            packetLoss = report.packetsLost ? report.packetsLost / report.packetsReceived * 100 : 0;
          }
  ...
  const handlePeerMessage = useCallback((message: PeerMessage) => {
    ...
    switch (message.type) {
      ...
      case 'join':
        ...
        setPlayers(prev => {
          ...
          if (playerExists) {
            return prev.map(p => 
              p.id === message.sender 
                ? { ...p, isConnected: true, name: message.content.name, isHost: message.content.isHost }
                : p
            );
          } else {
            return [
      ...
      case 'ready':
        ...
        const allPlayersReady = players.every(p => p.isReady || p.id === playerIdRef.current);
  ...
  const toggleReady = useCallback(() => {
    ...
    const allPlayersReady = players.every(p => 
      (p.id === playerIdRef.current && newReadyStatus) || (p.id !== playerIdRef.current && p.isReady)
    );
  ...
  const simulateGameProgression = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      Object.keys(newGameState.players).forEach(playerId => {
        ...
        // Draw 7 cards
        for (let i = 0; i < 7; i++) {
          player.hand.push(`card_${Math.random().toString(36).substring(2, 9)}`);
          player.library--;
        }
      });
  ...
  const simulateGameActions = useCallback(() => {
    ...
    simulateAction(12000, () => {
      ...
      if (player1Id && player2Id) {
        newGameState.activePlayer = newGameState.activePlayer === player1Id ? player2Id : player1Id;
  ...
  const endMatch = useCallback(() => {
    ...
    const duration = matchStartTimeRef.current ? (Date.now() - matchStartTimeRef.current) / 1000 : 0;
      ...
      <div className="match-container">
        <div className="video-section">
          ...
          <div className="video-container remote">
            ...
            <div className="player-info">
              <span className="player-name">{players.find(p => p.id !== playerIdRef.current)?.name || 'Waiting for opponent...'}</span>
              <span className="player-role">{players.find(p => p.id !== playerIdRef.current)?.isHost ? 'Host' : 'Guest'}</span>
            </div>
            
            {connectionStatus !== 'connected' && (
              <div className="connecting-overlay">
                <div className="loading-spinner"></div>
                <p>{connectionStatus === 'connecting' ? 'Connecting to opponent...' : 'Disconnected'}</p>
              </div>
            )}
          </div>
          ...
          <div className="network-stats">
            <div className="stat">
              <span>Latency:</span>
              <span className={networkStats.latency < 50 ? 'good' : networkStats.latency < 100 ? 'medium' : 'bad'}>
                {networkStats.latency.toFixed(0)} ms
              </span>
            </div>
            <div className="stat">
              <span>Packet Loss:</span>
              <span className={networkStats.packetLoss < 1 ? 'good' : networkStats.packetLoss < 5 ? 'medium' : 'bad'}>
              ...
              <div className="players-list">
                {players.map(player => (
                  <div key={player.id} className={`player-item ${player.isConnected ? '' : 'disconnected'}`}>
                    ...
                    <span className="player-status">{player.isReady ? 'Ready' : 'Not Ready'}</span>
                  </div>
                ))}
              </div>
              ...
              {matchResult && (
                <>
                  <div className="result-item">
                    <span>Winner:</span>
                    <span>{players.find(p => p.id === matchResult.winner)?.name || 'Unknown'}</span>
                  </div>
                  
                  <div className="result-item">
                    <span>Loser:</span>
                    <span>{players.find(p => p.id === matchResult.loser)?.name || 'Unknown'}</span>
                  </div>
                  ...
                  <div className="result-item">
                    <span>Duration:</span>
                    <span>{Math.floor(matchResult.duration / 60)}:{(matchResult.duration % 60).toFixed(0).padStart(2, '0')}</span>
                  </div>
                    ...
                    <div className="actions-list">
                      {matchResult.gameActions.map((action, index) => (
                        <div key={index} className="action-item">
                          <span className="action-player">{players.find(p => p.id === action.player)?.name || 'Unknown'}</span>
                          <span className="action-type">{action.type}</span>
                          {action.card && <span className="action-card">{action.card}</span>}
                          {action.target && <span className="action-target">→ {players.find(p => p.id === action.target)?.name || action.target}</span>}
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="game-board">
              <div className="game-info">
                ...
                <div className="active-player">
                  Active Player: {players.find(p => p.id === gameState?.activePlayer)?.name || 'Unknown'}
                </div>
              </div>
              
              <div className="players-board">
                {gameState && Object.entries(gameState.players).map(([playerId, playerState]) => {
                  const player = players.find(p => p.id === playerId);
                  const isLocalPlayer = playerId === playerIdRef.current;
                  
                  return (
                    <div key={playerId} className={`player-board ${isLocalPlayer ? 'local-player' : 'opponent-player'}`}>
                      <div className="player-header">
                        <span className="player-name">{player?.name || 'Unknown'}</span>
                        <span className="player-life">Life: {playerState.life}</span>
                      </div>
                      
                      <div className="player-zones">
                        <div className="zone library">
                          <div className="zone-label">Library</div>
                          <div className="zone-content">{playerState.library} cards</div>
                        </div>
              ...
              <div className="game-controls">
                <button className="game-button" disabled>Draw Card</button>
                <button className="game-button" disabled>Play Card</button>
                <button className="game-button" disabled>Attack</button>
                <button className="game-button" disabled>End Turn</button>
                <button className="game-button end-match" onClick={endMatch}>End Match</button>
              </div>
            </div>
          )}
        </div>
