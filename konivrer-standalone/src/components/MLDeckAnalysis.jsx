...
const MLDeckAnalysis = () => {
  ...
  const metaArchetypes = useMemo(
    () => ({
      ...
      Control: {
        description:
          'Defensive deck focused on countering threats and winning late game',
        winRate: 0.48,
      ...
      Combo: {
        description:
          'Deck focused on assembling specific card combinations for powerful effects',
        winRate: 0.49,
    ...
    setTimeout(() => {
      ...
      parsedDeck.forEach(card => {
        ...
        typeDistribution[card.type] =
          (typeDistribution[card.type] || 0) + card.count;

        // Count mana costs
        if (card.cost !== undefined) {
          costDistribution[card.cost] =
            (costDistribution[card.cost] || 0) + card.count;
        }
      ...
      analysisResult.basicStats = {
        ...
        averageCost:
          parsedDeck.reduce(
            (sum, card) => sum + (card.cost || 0) * card.count,
            0,
          ) /
          parsedDeck.reduce(
            (sum, card) => sum + (card.cost !== undefined ? card.count : 0),
            0,
          ),
      };
      ...
      if (selectedAnalysisType === 'archetype') {
        ...
        Object.entries(tagCounts).forEach(([tag, count]) => {
          if (['Aggro', 'Damage'].includes(tag))
            archetypeScores['Aggro'] += count;
          if (['Control', 'Counter', 'Board Clear'].includes(tag))
            archetypeScores['Control'] += count;
          if (['Midrange', 'Ramp'].includes(tag))
            archetypeScores['Midrange'] += count;
          if (['Combo', 'Draw'].includes(tag))
            archetypeScores['Combo'] += count;
        });
        ...
        analysisResult.archetypeAnalysis = {
          ...
          unfavorableMatchups:
            metaArchetypes[primaryArchetype].unfavorableMatchups,
        };
        ...
        if (lowCostCount < totalCards * 0.4) {
          suggestions.push({
            ...
            suggestion:
              'Add more cards with cost 1-3 to improve early game presence',
          });
        }
        ...
        if (highCostCount > totalCards * 0.3) {
          suggestions.push({
            ...
            suggestion:
              'Reduce the number of cards with cost 5+ to avoid dead draws',
          });
        }
        ...
        const primaryArchetype =
          Object.entries(tagCounts)
            .filter(([tag]) =>
              ['Aggro', 'Control', 'Midrange', 'Combo'].includes(tag),
            )
            .sort((a, b) => b[1] - a[1])[0]?.[0] || 'Unknown';

        if (
          primaryArchetype !== 'Unknown' &&
          metaArchetypes[primaryArchetype]
        ) {
          const missingKeyCards = metaArchetypes[
            primaryArchetype
          ].keyCards.filter(card => !parsedCardNames.includes(card));
        ...
        for (const [archetype, data] of Object.entries(metaArchetypes)) {
          ...
          const archetypeTags = data.keyCards.flatMap(
            card => cardDatabase[card]?.tags || [],
          );
          ...
          const counterTags = deckTags.filter(tag => {
            ...
            if (
              archetype === 'Aggro' &&
              ['Board Clear', 'Healing'].includes(tag)
            )
              return true;
            if (archetype === 'Control' && ['Aggro', 'Combo'].includes(tag))
              return true;
            if (archetype === 'Midrange' && ['Control'].includes(tag))
              return true;
            if (archetype === 'Combo' && ['Counter', 'Aggro'].includes(tag))
              return true;
            return false;
          });

          const counterScore =
            counterTags.reduce((sum, tag) => sum + (tagCounts[tag] || 0), 0) /
            totalCards;
          baseWinRate += counterScore * 0.2; // Adjust win rate based on counter cards

          predictedMatchups[archetype] = Math.min(
            Math.max(baseWinRate, 0.3),
            0.7,
        ...
        analysisResult.predictionAnalysis = {
          ...
          predictedMatchups,
          overallWinRate:
            Object.values(predictedMatchups).reduce(
              (sum, rate) => sum + rate,
              0,
...
2x Plains`,
  ...
  return (
    <div
      className={`ml-deck-analysis ${isAncientTheme ? 'ancient-theme' : ''}`}
    >
      ...
      <div className="deck-input-section">
        <div className="textarea-container">
          <label htmlFor="deckList">
            Enter Deck List (Format: "2x Card Name")
          </label>
          <textarea
            id="deckList"
            value={deckList}
        ...
        <div className="analysis-results">
          ...
          <div className="basic-stats">
            ...
            <p>
              ...
              {analysisResults.basicStats.totalCards}
            </p>
            <p>
              <strong>Average Cost:</strong>{' '}
              {analysisResults.basicStats.averageCost.toFixed(2)}
            </p>
            ...
            <div className="distribution-section">
              <div className="distribution">
                ...
                <ul>
                  {Object.entries(
                    analysisResults.basicStats.typeDistribution,
                  ).map(([type, count]) => (
                    <li key={type}>
                      <span>{type}:</span> {count} (
                      {(
                        (count / analysisResults.basicStats.totalCards) *
                        100
                      ).toFixed(1)}
                      %)
                    </li>
          ...
          {analysisResults.archetypeAnalysis && (
            <div className="archetype-analysis">
              ...
              <p>
                ...
                {analysisResults.archetypeAnalysis.primaryArchetype}
              </p>
              <p>
                <strong>Secondary Archetype:</strong>{' '}
                {analysisResults.archetypeAnalysis.secondaryArchetype}
              </p>
              <p>
                <strong>Description:</strong>{' '}
                {analysisResults.archetypeAnalysis.archetypeDescription}
              </p>
              <p>
                <strong>Estimated Win Rate:</strong>{' '}
                {(
                  analysisResults.archetypeAnalysis.estimatedWinRate * 100
                ).toFixed(1)}
                %
              </p>
              ...
              <div className="matchups">
                <div className="favorable">
                  ...
                  <ul>
                    {analysisResults.archetypeAnalysis.favorableMatchups.map(
                      matchup => (
                ...
                <div className="unfavorable">
                  ...
                  <ul>
                    {analysisResults.archetypeAnalysis.unfavorableMatchups.map(
                      matchup => (
                        <li key={matchup}>{matchup}</li>
                      ),
                    )}
              ...
              <div className="archetype-scores">
                ...
                <div className="score-bars">
                  {Object.entries(
                    analysisResults.archetypeAnalysis.archetypeScores,
                  ).map(([archetype, score]) => (
            ...
            <div className="optimization-analysis">
              ...
              {analysisResults.optimizationAnalysis.suggestions.length > 0 ? (
                <>
                  <p>
                    We've identified{' '}
                    {analysisResults.optimizationAnalysis.suggestions.length}{' '}
                    potential improvements for your deck:
                  </p>

                  <div className="suggestions">
                    {analysisResults.optimizationAnalysis.suggestions.map(
                      (suggestion, index) => (
                        <div key={index} className="suggestion">
                          <div className="suggestion-header">
                            ...
                            <span className="suggestion-issue">
                              {suggestion.issue}
                            </span>
                          </div>
                ...
                <p className="no-suggestions">
                  Your deck appears to be well-optimized! No significant issues
                  were found.
                </p>
              )}
            </div>
          ...
          {analysisResults.predictionAnalysis && (
            <div className="prediction-analysis">
              ...
              <p>
                <strong>Deck Archetype:</strong>{' '}
                {analysisResults.predictionAnalysis.deckArchetype}
              </p>
              <p>
                <strong>Overall Predicted Win Rate:</strong>{' '}
                {(
                  analysisResults.predictionAnalysis.overallWinRate * 100
                ).toFixed(1)}
                %
              </p>
              <p>
                <strong>Prediction Confidence:</strong>{' '}
                {(analysisResults.predictionAnalysis.confidence * 100).toFixed(
                  0,
                )}
                %
              </p>
              ...
              <div className="matchup-predictions">
                ...
                {Object.entries(
                  analysisResults.predictionAnalysis.predictedMatchups,
