...
const BlockchainVerification = () => {
  ...
  const [selectedVerificationType, setSelectedVerificationType] = useState('match');
    ...
    matches: [
      {
        ...
        transactionHash: '0x7f9e4e5d3b2a1c8f7e6d5c4b3a2e1d0f9e8d7c6b5a4f3e2d1c0b9a8f7e6d5c4b3a'
      },
      {
        id: 'match_02',
    ...
    tournaments: [
      {
        ...
        transactionHash: '0x9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b'
      }
    ]
  }), []);
   ...
   */
  const verifyTransaction = useCallback((hash) => {
    if (!hash) return;
    ...
    setTimeout(() => {
      ...
      if (selectedVerificationType === 'match') {
        data = blockchainData.matches.find(match => match.transactionHash === hash);
      } 
      // Check cards
      else if (selectedVerificationType === 'card') {
        data = blockchainData.cards.find(card => card.transactionHash === hash);
      }
      // Check tournaments
      else if (selectedVerificationType === 'tournament') {
        data = blockchainData.tournaments.find(tournament => tournament.transactionHash === hash);
      }
  ...
  const recordNewTransaction = useCallback(() => {
    ...
    alert(`New ${selectedVerificationType} recorded on blockchain with transaction hash: ${newHash}`);
  }, [selectedVerificationType]);

  return (
    <div className={`blockchain-verification ${isAncientTheme ? 'ancient-theme' : ''}`}>
      ...
      <form onSubmit={handleSubmit} className="verification-form">
        ...
        <div className="button-group">
          ...
          >
            Record New {selectedVerificationType.charAt(0).toUpperCase() + selectedVerificationType.slice(1)}
          </button>
        </div>
      </form>
      
      {verificationStatus && (
        <div className={`verification-result ${verificationStatus.verified ? 'verified' : 'failed'}`}>
          <h3>Verification Result</h3>
          <p>
            <strong>Status:</strong> {verificationStatus.verified ? 'Verified ✓' : 'Not Verified ✗'}
          </p>
          <p>
            <strong>Timestamp:</strong> {formatTimestamp(verificationStatus.timestamp)}
          </p>
          ...
          {verificationStatus.verified && verificationStatus.data && (
            <div className="verification-data">
              {selectedVerificationType === 'match' && (
                <>
                  <h4>Match Details</h4>
                  <p><strong>Match ID:</strong> {verificationStatus.data.id}</p>
                  <p><strong>Date:</strong> {formatTimestamp(verificationStatus.data.timestamp)}</p>
                  <p><strong>Players:</strong> {verificationStatus.data.player1.name} vs {verificationStatus.data.player2.name}</p>
                  <p><strong>Result:</strong> {verificationStatus.data.result.score}</p>
                  <p><strong>Winner:</strong> {
                    verificationStatus.data.player1.id === verificationStatus.data.result.winner 
              ...
              {selectedVerificationType === 'card' && (
                <>
                  <h4>Card Details</h4>
                  <p><strong>Card ID:</strong> {verificationStatus.data.id}</p>
                  <p><strong>Name:</strong> {verificationStatus.data.name}</p>
                  <p><strong>Rarity:</strong> {verificationStatus.data.rarity}</p>
                  <p><strong>Owner:</strong> {verificationStatus.data.owner.name}</p>
                  <p><strong>Mint Date:</strong> {formatTimestamp(verificationStatus.data.mintDate)}</p>
                </>
              )}
              ...
              {selectedVerificationType === 'tournament' && (
                <>
                  ...
                  <p><strong>Tournament ID:</strong> {verificationStatus.data.id}</p>
                  <p><strong>Name:</strong> {verificationStatus.data.name}</p>
                  <p><strong>Date:</strong> {formatTimestamp(verificationStatus.data.date)}</p>
                  <p><strong>Participants:</strong> {verificationStatus.data.participants}</p>
                  <p><strong>Winner:</strong> {verificationStatus.data.winner.name}</p>
                </>
              )}
            </div>
          )}
