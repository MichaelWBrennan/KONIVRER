...
const AugmentedRealityViewer = () => {
  ...
  const cardDatabase = {
    'KON001': {
      ...
      toughness: 6,
      cost: 5,
      description: 'When Ancient Guardian enters the battlefield, create a 1/1 Spirit token with flying.',
    ...
    'KON004': {
      ...
      toughness: 6,
      cost: 7,
      description: 'Flying. When Ethereal Dragon deals combat damage to a player, draw a card.',
    ...
    'KON008': {
      ...
      rarity: 'Rare',
      cost: 6,
      description: 'Volcanic Eruption deals 5 damage to each creature and each player.',
  ...
  const detectCards = useCallback(() => {
    ...
    if (Math.random() > 0.95) { // Simulate occasional card detection
   ...
   */
  const drawArEffects = useCallback((context, card) => {
    if (!card) return;
    ...
    context.fillRect(canvas.width * 0.3, canvas.height * 0.2, canvas.width * 0.4, canvas.height * 0.6);
    ...
    arEffects.forEach(effect => {
      ...
      if (effect.type.includes('particles_fire')) {
        ...
        drawAura(context, effect.type.includes('mystical') ? '#4287f5' : '#f5d742', effect.opacity);
      }
  ...
  const drawCardStats = (context, card) => {
    ...
    if (card.type === 'Creature') {
      context.fillText(`Power/Toughness: ${card.power}/${card.toughness}`, 10, canvas.height - 55);
    }
    context.fillText(`Cost: ${card.cost}`, 10, canvas.height - 35);
    
    // Draw card description (truncated if needed)
    const description = card.description.length > 60 
    ...
    card.arAnimations.forEach((animation, index) => {
      context.fillText(animation, canvas.width - 10, canvas.height - 75 + index * 20);
    });
  };
  ...
  const triggerAnimation = useCallback(() => {
    ...
    const animation = currentCard.arAnimations[Math.floor(Math.random() * currentCard.arAnimations.length)];
  ...
  return (
    <div className={`ar-viewer ${isAncientTheme ? 'ancient-theme' : ''}`}>
        ...
        {!isActive && (
          <div className="ar-placeholder">
            ...
            <p className="ar-description">
              Point your camera at a physical card to see it come to life with 3D models and animations.
            </p>
          ...
          <div className="card-details">
            <div className="card-main-info">
              ...
              {currentCard.type === 'Creature' && (
                <p><strong>Stats:</strong> {currentCard.power}/{currentCard.toughness}</p>
              )}
              <p><strong>Cost:</strong> {currentCard.cost}</p>
            </div>
            
            <div className="card-ar-info">
              <p><strong>3D Model:</strong> {currentCard.arModel}</p>
              <p><strong>Animations:</strong> {currentCard.arAnimations.join(', ')}</p>
              <p><strong>Effects:</strong> {currentCard.arEffects.join(', ')}</p>
            </div>
          </div>
      ...
      <style jsx>{`
        ...
        .ancient-theme h2, .ancient-theme h3 {
