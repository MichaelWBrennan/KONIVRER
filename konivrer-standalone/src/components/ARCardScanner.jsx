...
const ARCardScanner = ({
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        setError(
          `Failed to initialize camera: ${err instanceof Error ? err.message : String(err)}`,
        );
        setIsInitialized(false);
      }
    };
  ...
  const loadDetectionModel = async () => {
    try {
        ...
        detect: async imageData => {
          ...
          if (shouldDetect) {
            const randomIndex = Math.floor(
              Math.random() * sampleCardDatabase.length,
            );
      ...
      throw new Error(
        `Failed to load detection model: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  };
  ...
  const renderAROverlay = useCallback(() => {
    ...
    if (currentCard && currentCard.corners) {
      ...
      context.beginPath();
      context.moveTo(
        currentCard.corners[0].x * width,
        currentCard.corners[0].y * height,
      );

      for (let i = 1; i < currentCard.corners.length; i++) {
        context.lineTo(
          currentCard.corners[i].x * width,
          currentCard.corners[i].y * height,
        );
      }
      ...
      context.fillText(
        `${currentCard.type} - ${currentCard.rarity}`,
        infoX + 10,
        infoY + 40,
      );
      ...
      if (
        currentCard.power !== undefined &&
        currentCard.toughness !== undefined
      ) {
        context.fillText(
      ...
      // Draw confidence
      context.fillStyle =
        currentCard.confidence > 0.9
          ? '#00ff00'
          : currentCard.confidence > 0.7
            ? '#ffff00'
            : '#ff0000';
      context.fillText(
        `Confidence: ${(currentCard.confidence * 100).toFixed(1)}%`,
        infoX + 150,
        infoY + 60,
      );
    }
    ...
    if (showDebugInfo) {
      ...
      context.fillText(
        `Detection Time: ${scanStats.detectionTime.toFixed(1)} ms`,
        20,
        50,
      );
      ...
      context.fillText(
        `Threshold: ${debugInfo.detectionThreshold.toFixed(2)}`,
        20,
        90,
      );
    }
  ...
  const manualScan = useCallback(async () => {
    ...
    isInitialized,
    debugInfo.processingRegion,
    debugInfo.detectionThreshold,
    handleCardDetection,
  ]);
  ...
  if (!isInitialized) {
    return (
      <div
        className={`ar-card-scanner ${isAncientTheme ? 'ancient-theme' : ''}`}
      >
                    ...
                    {card.features && card.features.length > 0 && (
                      <div className="card-features">
                        {card.features.map(feature => (
                          <span key={feature} className={`feature ${feature}`}>
                            {feature}
                          </span>
                        ))}
                      </div>
        ...
        <div className="debug-panel">
          ...
          <div className="debug-section">
            ...
            <div className="debug-item">
              ...
              <span>
                {debugInfo.resolution.width}x{debugInfo.resolution.height}
              </span>
            </div>
          ...
          <div className="debug-section">
            ...
            <div className="debug-item">
              ...
              <span>
                <input
                  ...
                  onChange={e =>
                    adjustDetectionThreshold(parseFloat(e.target.value))
                  }
                />
            ...
            <div className="debug-item">
              ...
              <span>
                {debugInfo.processingRegion.width}x
                {debugInfo.processingRegion.height}
              </span>
            </div>
