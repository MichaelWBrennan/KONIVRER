...
async function getPrInfo(repo: string, pr: string, token: string) {
  ...
  return {
    ...
    baseRef: data.base.ref as string,
...
async function main() {
  ...
  const hasRemote = await runShell(
    ...
    .then(() => true)
    .catch(() => false);
  if (hasRemote) await runShell(`git checkout -B ${headRef} origin/${headRef}`);
  else {
    await runShell(`git fetch origin ${headSha}`);
    await runShell(`git checkout -B ${headRef} ${headSha}`);
  }

  // attempt rebase
  if (
    await runShell(`git rebase origin/${baseRef}`)
      .then(() => true)
      .catch(() => false)
  ) {
    ...
    if (
      !(await runShell(`git merge -X patience origin/${baseRef}`)
        .then(() => true)
        .catch(() => false))
    ) {
      ...
      if (
        !(await runShell(`git merge -X theirs origin/${baseRef}`)
          .then(() => true)
          .catch(() => false))
      ) {
