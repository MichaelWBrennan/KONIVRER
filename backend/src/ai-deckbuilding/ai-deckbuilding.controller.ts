...
export class AiDeckbuildingController {
  ...
  async getMetaAnalysis(
    ...
    return this.aiDeckbuildingService.generateMetaAnalysis(userId || 'default-user', format);
  }
  ...
  async getSkillBasedRecommendations(
    ...
    const suggestions = await this.aiDeckbuildingService.generateDeckSuggestions(
      user.id,
      format,
