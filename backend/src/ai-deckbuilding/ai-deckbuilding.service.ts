...
export class AiDeckbuildingService {
  ...
  async generateDeckSuggestions(
    ...
    const playerRating = await this.matchmakingService.getPlayerRating(userId, format);
    ...
    const archetypePerformance = await this.analyzeArchetypePerformance(playerDecks, playerRating);
    ...
    return suggestions.map(suggestion => ({
      ...suggestion,
      confidenceScore: this.calculateConfidenceScore(suggestion, playerRating, metaData),
    }));
  }
  ...
  async generateMetaAnalysis(userId: string, format: string): Promise<MetaAnalysisDto> {
    ...
    const skillBasedMeta = await this.analyzeSkillBasedMeta(format, skillDistribution);
    ...
    return {
      ...
      personalizedMeta: {
        ...
        recommendedArchetypes: await this.getRecommendedArchetypes(playerRating, skillBasedMeta),
        improvementSuggestions: await this.generateImprovementSuggestions(userId, playerRating),
      },
    };
  }
  ...
  async optimizeDeck(request: DeckOptimizationRequest): Promise<{
    ...
    const playerRating = await this.matchmakingService.getPlayerRating(request.userId, request.format);
    const deckAnalysis = await this.analyzeDeckComposition(request.currentDeckList, request.format);
    
    // Use Bayesian rating to determine optimization strategy
    const optimizationStrategy = this.getOptimizationStrategy(playerRating, request.playstyle);
  ...
  private async getUserDeckHistory(userId: string, format: string): Promise<Deck[]> {
  ...
  private async getMetaSnapshot(format: string): Promise<any> {
    ...
    const popularDecks = await this.deckRepository
      ...
      .andWhere('deck.created_at > :date', { date: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) })
  ...
  private async getFormatSkillDistribution(format: string): Promise<any> {
    ...
    return {
      ...
      intermediate: ratings.filter(r => r.conservativeRating >= 10 && r.conservativeRating < 20),
      advanced: ratings.filter(r => r.conservativeRating >= 20 && r.conservativeRating < 30),
  ...
  private async analyzeArchetypePerformance(decks: Deck[], playerRating: any): Promise<any> {
  ...
  private async generateSkillBasedSuggestions(
    ...
    if (skillTier === 'novice') {
      ...
      suggestions.push({
        cardName: 'Tech cards',
        synergy: 0.7,
        reasoning: 'Advanced players can leverage situational cards effectively',
      });
    }
  ...
  private calculateConfidenceScore(suggestion: any, playerRating: any, metaData: any): number {
  ...
  private async analyzeSkillBasedMeta(format: string, skillDistribution: any): Promise<any> {
    // Analyze what strategies work at different skill levels
    return {
      novice: [
        { archetype: 'Aggro', effectiveness: 0.8, confidence: 0.9, winRate: 52 },
        { archetype: 'Midrange', effectiveness: 0.7, confidence: 0.8, winRate: 48 },
      ],
      intermediate: [
        { archetype: 'Midrange', effectiveness: 0.9, confidence: 0.8, winRate: 55 },
        { archetype: 'Control', effectiveness: 0.6, confidence: 0.7, winRate: 47 },
      ],
      advanced: [
        { archetype: 'Control', effectiveness: 0.8, confidence: 0.9, winRate: 58 },
        { archetype: 'Combo', effectiveness: 0.7, confidence: 0.8, winRate: 53 },
      ],
      expert: [
        { archetype: 'Combo', effectiveness: 0.9, confidence: 0.9, winRate: 62 },
        { archetype: 'Tech Control', effectiveness: 0.8, confidence: 0.8, winRate: 59 },
      ],
    };
  }
  ...
  private async getRecommendedArchetypes(playerRating: any, skillBasedMeta: any): Promise<string[]> {
    const skillTier = this.getSkillTier(playerRating.conservativeRating);
  ...
  private async generateImprovementSuggestions(userId: string, playerRating: any): Promise<string[]> {
    const suggestions = [];
    ...
    if (playerRating.conservativeRating < 15) {
      suggestions.push('Master fundamental strategies before trying complex combos');
    }
    
    return suggestions;
  }

  private async analyzeDeckComposition(deckList: string[], format: string): Promise<any> {
  ...
  private async identifyDeckSynergies(deckList: string[]): Promise<any[]> {
    ...
    return [
      { cards: ['Card A', 'Card B'], synergy: 0.9, impact: 'high' },
    ];
  }

  private async identifyWeaknesses(deckList: string[], format: string): Promise<string[]> {
    // Identify potential weaknesses against current meta
    return ['Vulnerable to fast aggro decks'];
  }

  private getOptimizationStrategy(playerRating: any, playstyle?: string): string {
    const skillTier = this.getSkillTier(playerRating.conservativeRating);
