import { IsEnum, IsString, IsUUID, IsOptional, IsObject, IsDateString } from 'class-validator';
...
export class CreateNotificationDto {
  ...
  @IsDateString()
  @IsOptional()
  @ApiProperty({ description: 'When notification should be sent', required: false })
  scheduledFor?: Date;
}

export class UpdateNotificationStatusDto {
  @IsEnum(['pending', 'sent', 'failed', 'read'])
  @ApiProperty({ enum: ['pending', 'sent', 'failed', 'read'], description: 'New notification status' })
...
export class NotificationFiltersDto {
  ...
  @IsEnum(NotificationType)
  @IsOptional()
  @ApiProperty({ enum: NotificationType, description: 'Filter by notification type', required: false })
  type?: NotificationType;

  @IsEnum(['pending', 'sent', 'failed', 'read'])
  @IsOptional()
  @ApiProperty({ enum: ['pending', 'sent', 'failed', 'read'], description: 'Filter by status', required: false })
  status?: 'pending' | 'sent' | 'failed' | 'read';
