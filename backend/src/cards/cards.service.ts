...
export class CardsService {
  ...
  async create(createCardDto: CreateCardDto): Promise<Card> {
    ...
    if (existingCard) {
      throw new BadRequestException(`Card with name "${createCardDto.name}" already exists`);
    }
  ...
  async findAll(filterDto?: CardFilterDto): Promise<PaginatedResult<Card>> {
    const { 
      search, type, element, rarity, minCost, maxCost, legalOnly,
    ...
    // Apply sorting
    const validSortFields = ['name', 'cost', 'power', 'metaRating', 'createdAt'];
  ...
  async update(id: string, updateCardDto: UpdateCardDto): Promise<Card> {
    ...
    if (updateCardDto.name && updateCardDto.name !== card.name) {
      ...
      if (existingCard) {
        throw new BadRequestException(`Card with name "${updateCardDto.name}" already exists`);
      }
    }
  ...
  async bulkCreate(cards: CreateCardDto[]): Promise<Card[]> {
    ...
    for (const cardDto of cards) {
      try {
        ...
        console.warn(`Failed to create card "${cardDto.name}": ${error instanceof Error ? error.message : String(error)}`);
      }
    }
