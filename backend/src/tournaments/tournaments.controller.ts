...
export class TournamentsController {
  ...
  @ApiResponse({ status: 201, description: 'Tournament created successfully', type: Tournament })
  ...
  @ApiQuery({ name: 'format', required: false, enum: TournamentFormat, description: 'Filter by format' })
  @ApiQuery({ name: 'type', required: false, enum: TournamentType, description: 'Filter by type' })
  @ApiQuery({ name: 'status', required: false, enum: TournamentStatus, description: 'Filter by status' })
  ...
  @ApiResponse({ status: 200, description: 'Featured tournaments retrieved successfully' })
  ...
  @ApiResponse({ status: 200, description: 'User tournaments retrieved successfully' })
  async getMyTournaments(@Query() filters: TournamentSearchFilters, @Request() req) {
  ...
  async getMyRegistrations(@Query() filters: TournamentSearchFilters, @Request() req) {
  ...
  @ApiResponse({ status: 200, description: 'Tournament retrieved successfully', type: Tournament })
  @ApiResponse({ status: 404, description: 'Tournament not found' })
  @ApiResponse({ status: 403, description: 'Access denied (private tournament)' })
  ...
  @ApiResponse({ status: 200, description: 'Tournament updated successfully', type: Tournament })
  @ApiResponse({ status: 404, description: 'Tournament not found' })
  ...
  @ApiResponse({ status: 200, description: 'Tournament started successfully', type: Tournament })
  ...
  @ApiResponse({ status: 201, description: 'Pairings generated successfully', type: [TournamentMatch] })
  ...
  @ApiResponse({ status: 200, description: 'Standings retrieved successfully', type: [TournamentStanding] })
  @ApiResponse({ status: 404, description: 'Tournament not found' })
  ...
  @ApiResponse({ status: 200, description: 'Matches retrieved successfully', type: [TournamentMatch] })
  @ApiResponse({ status: 404, description: 'Tournament not found' })
  @ApiQuery({ name: 'round', required: false, type: Number, description: 'Filter by round' })
  async getMatches(
    @Param('id', ParseUUIDPipe) id: string,
  ...
  @ApiResponse({ status: 200, description: 'Match result submitted successfully', type: TournamentMatch })
  ...
  async submitMatchResult(
    ...
    return this.tournamentsService.submitMatchResult(matchId, resultDto, req.user.id);
  }

  @Post(':id/drop')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Drop from tournament' })
  @ApiResponse({ status: 200, description: 'Successfully dropped from tournament' })
  ...
  @ApiQuery({ name: 'format', required: false, enum: ['json', 'csv', 'pdf'], description: 'Export format' })
  async exportTournament(
    ...
    const statistics = await this.tournamentsService.getTournamentStatistics(id);
  ...
  @ApiResponse({ status: 200, description: 'Bracket data retrieved successfully' })
  ...
  async updateStream(
    ...
    const tournament = await this.tournamentsService.update(id, updateDto, req.user.id);
