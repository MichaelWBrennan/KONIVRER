...
export class TournamentsService {
  ...
  async create(createTournamentDto: CreateTournamentDto, organizerId: string): Promise<Tournament> {
    ...
    if (![UserRole.TOURNAMENT_ORGANIZER, UserRole.ADMIN].includes(organizer.role)) {
      throw new ForbiddenException('User does not have permission to organize tournaments');
    }
  ...
  async findAll(filters: TournamentSearchFilters, userId?: string): Promise<{ tournaments: Tournament[]; total: number; page: number; limit: number }> {
    const {
      page = 1,
      limit = 20,
      format,
    ...
    if (userId) {
      ...
      queryBuilder.andWhere('tournament.visibility = :public', { public: TournamentVisibility.PUBLIC });
    }
    ...
    if (startDateFrom) {
      queryBuilder.andWhere('tournament.startDate >= :startDateFrom', { startDateFrom });
    }

    if (startDateTo) {
      queryBuilder.andWhere('tournament.startDate <= :startDateTo', { startDateTo });
    }
    ...
    // Apply sorting
    const validSortFields = ['startDate', 'createdAt', 'name', 'registeredPlayers'];
    if (validSortFields.includes(sortBy)) {
      if (sortBy === 'registeredPlayers') {
        queryBuilder.addSelect('COUNT(participants.id)', 'participantCount')
  ...
  async update(id: string, updateTournamentDto: UpdateTournamentDto, userId: string): Promise<Tournament> {
    ...
    if (tournament.organizerId !== userId) {
      throw new ForbiddenException('Only the tournament organizer can update this tournament');
    }

    // Prevent certain updates once tournament has started
    if (tournament.status === TournamentStatus.IN_PROGRESS && updateTournamentDto.settings) {
      throw new BadRequestException('Cannot modify settings while tournament is in progress');
    }
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (tournament.organizerId !== userId) {
      throw new ForbiddenException('Only the tournament organizer can delete this tournament');
    }

    if (tournament.status === TournamentStatus.IN_PROGRESS) {
      throw new BadRequestException('Cannot delete tournament while in progress');
    }

    await this.tournamentRepository.remove(tournament);
  }

  async register(tournamentId: string, userId: string, registerDto: RegisterForTournamentDto): Promise<void> {
    ...
    if (tournament.settings.requireDeckList && registerDto.deckId) {
      ...
      if (deck.format.toString() !== tournament.format.toString()) {
        throw new BadRequestException(`Deck format must be ${tournament.format}`);
      }
  ...
  async unregister(tournamentId: string, userId: string): Promise<void> {
    ...
    if (tournament.status === TournamentStatus.IN_PROGRESS) {
      throw new BadRequestException('Cannot unregister while tournament is in progress. Use drop instead.');
    }
  ...
  async startTournament(id: string, userId: string): Promise<Tournament> {
    ...
    if (tournament.organizerId !== userId) {
      throw new ForbiddenException('Only the tournament organizer can start this tournament');
    }

    if (tournament.status !== TournamentStatus.REGISTRATION_CLOSED) {
      throw new BadRequestException('Tournament must be in registration closed state to start');
    }

    if (!tournament.participants || tournament.participants.length < tournament.settings.minPlayers) {
      throw new BadRequestException(`Need at least ${tournament.settings.minPlayers} players to start`);
    }
  ...
  async generatePairings(tournamentId: string, round: number): Promise<TournamentMatch[]> {
    const tournament = await this.tournamentRepository.findOne({
      where: { id: tournamentId },
    ...
    switch (tournament.type) {
      case TournamentType.SWISS:
        pairings = await this.generateSwissPairings(tournamentId, round, participants, tournament.standings || []);
        break;
      case TournamentType.SINGLE_ELIMINATION:
        pairings = await this.generateEliminationPairings(tournamentId, round, participants);
        break;
      case TournamentType.ROUND_ROBIN:
        pairings = await this.generateRoundRobinPairings(tournamentId, round, participants);
        break;
      default:
        throw new BadRequestException('Unsupported tournament type for pairing generation');
    }

    return this.matchRepository.save(pairings);
  }

  async submitMatchResult(matchId: string, result: SubmitMatchResultDto, userId: string): Promise<TournamentMatch> {
    ...
    const canSubmit = match.player1Id === userId || 
                     match.player2Id === userId || 
                     match.judgeId === userId ||
                     match.tournament.organizerId === userId;

    if (!canSubmit) {
      throw new ForbiddenException('Not authorized to submit result for this match');
    }
  ...
  async dropPlayer(tournamentId: string, playerId: string, userId: string): Promise<void> {
  ...
  async getTournamentStatistics(tournamentId: string): Promise<any> {
    ...
    const avgMatchLength = completedMatches.length > 0 ? 
      completedMatches.reduce((sum, m) => sum + m.durationMinutes, 0) / completedMatches.length : 0;
  ...
  private calculateTotalRounds(type: TournamentType, maxPlayers: number): number {
    switch (type) {
      case TournamentType.SWISS:
  ...
  private async updateStandings(tournamentId: string): Promise<void> {
    ...
    for (const standing of standings) {
      const playerMatches = matches.filter(
        m => m.player1Id === standing.playerId || m.player2Id === standing.playerId
      );
      ...
      standing.gameWinPercentage = gameWins + gameLosses > 0 ? 
        (gameWins / (gameWins + gameLosses)) * 100 : 0;
  ...
  private async checkRoundComplete(tournamentId: string, round: number): Promise<void> {
