...
export class PhysicalGameSimulationService {
  ...
  private executeStartPhase(gameState: GameState, random: () => number): void {
    ...
    if (currentPlayer.hand.length > 0 && random() > 0.3) { // 70% chance to place Azoth
  ...
  private async executePostCombatMainPhase(gameState: GameState, random: () => number): Promise<void> {
  ...
  private executeRefreshPhase(gameState: GameState): void {
    ...
    this.logEvent(gameState, currentPlayer.id, 'refresh_phase', {
      azothGenerated: Object.values(currentPlayer.azothPool).reduce((a, b) => a + b, 0),
    });
  }
  ...
  private async executeMainPhase(gameState: GameState, random: () => number): Promise<void> {
    ...
    this.logEvent(gameState, currentPlayer.id, 'main_phase', {
      azothAvailable: Object.values(currentPlayer.azothPool).reduce((a, b) => a + b, 0),
      handSize: currentPlayer.hand.length,
    });
  }
  ...
  private async executeCombatPhase(gameState: GameState, random: () => number): Promise<void> {
    ...
    const attackers = [...currentPlayer.field, ...currentPlayer.combatRow].filter(card => 
  ...
  private choosePlayMode(card: Card, player: PhysicalPlayer, randomValue: number): string {
  ...
  private canAffordCard(card: Card, azothPool: Record<string, number>): boolean {
    const cost = this.getCardAzothCost(card);
  ...
  private payAzothCost(player: PhysicalPlayer, cost: Record<string, number>): void {
    for (const [element, amount] of Object.entries(cost)) {
      if (player.azothPool[element] !== undefined) {
        player.azothPool[element] = Math.max(0, player.azothPool[element] - amount);
      }
    }
  }
