...
export class DecksService {
  ...
  async create(createDeckDto: CreateDeckDto, userId: string): Promise<Deck> {
    ...
    await this.validateDeckConstruction(createDeckDto.mainboard, createDeckDto.format);

    // Calculate analytics
    const analytics = await this.calculateDeckAnalytics(createDeckDto.mainboard, cards);
    ...
    const deck = this.deckRepository.create({
      ...
      isLegal: await this.checkLegality(createDeckDto.mainboard, createDeckDto.format),
    });
  ...
  async findAll(filters: DeckSearchFilters, userId?: string): Promise<{ decks: Deck[]; total: number; page: number; limit: number }> {
    const {
      page = 1,
      limit = 20,
      format,
    ...
    if (includePrivate && userId) {
      queryBuilder.andWhere('(deck.visibility != :private OR deck.userId = :userId)', {
        private: DeckVisibility.PRIVATE,
        userId,
      });
    } else {
      queryBuilder.andWhere('deck.visibility = :public', { public: DeckVisibility.PUBLIC });
    }
    ...
    // Apply sorting
    const validSortFields = ['createdAt', 'updatedAt', 'name', 'metaRating', 'powerLevel', 'likes', 'views'];
  ...
  async update(id: string, updateDeckDto: UpdateDeckDto, userId: string): Promise<Deck> {
    ...
    if (updateDeckDto.mainboard) {
      ...
      const analytics = await this.calculateDeckAnalytics(updateDeckDto.mainboard, cards);
  ...
  async clone(id: string, userId: string, name?: string): Promise<Deck> {
    ...
    if (originalDeck.visibility === DeckVisibility.PRIVATE && originalDeck.userId !== userId) {
  ...
  async like(id: string, userId: string): Promise<{ liked: boolean; likes: number }> {
    const deck = await this.deckRepository.findOne({ where: { id } });
  ...
  async getTopDecks(format?: DeckFormat, archetype?: DeckArchetype, limit: number = 10): Promise<Deck[]> {
    const query = this.deckRepository.createQueryBuilder('deck')
      .leftJoinAndSelect('deck.user', 'user')
      .where('deck.visibility = :visibility', { visibility: DeckVisibility.PUBLIC })
  ...
  async getMetaSnapshot(format: DeckFormat): Promise<any> {
    ...
    const archetypeStats = await this.deckRepository
      ...
      .andWhere('deck.visibility = :visibility', { visibility: DeckVisibility.PUBLIC })
      .andWhere('deck.isLegal = true')
    ...
    const trendingDecks = await this.deckRepository
      ...
      .where('deck.format = :format', { format })
      .andWhere('deck.visibility = :visibility', { visibility: DeckVisibility.PUBLIC })
      .andWhere('deck.createdAt > :date', { date: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) }) // Last 7 days
  ...
  private async validateDeckConstruction(mainboard: any[], format: DeckFormat): Promise<void> {
    ...
    // Check deck size
    const totalCards = mainboard.reduce((total, card) => total + card.quantity, 0);
    const minSize = format === DeckFormat.COMMANDER ? 100 : 60;
    
    if (totalCards < minSize) {
      throw new BadRequestException(`Deck must have at least ${minSize} cards for ${format} format`);
    }
    ...
    for (const [cardId, count] of Object.entries(cardCounts)) {
      ...
      if (cardCount > 4) {
        ...
        if (!card?.keywords?.includes('Basic')) {
          throw new BadRequestException(`Cannot have more than 4 copies of ${card?.name || 'card'}`);
        }
      }
    }
  }

  private async calculateDeckAnalytics(mainboard: any[], cards: Card[]): Promise<any> {
    const cardMap = new Map(cards.map(card => [card.id, card]));
    
    // Calculate mana curve
    const manaCurve = Array(10).fill(0).map((_, i) => ({ cost: i, count: 0 }));
    let totalManaCost = 0;
    let totalCards = 0;
    ...
    for (const deckCard of mainboard) {
      ...
      // Element distribution
      elementCounts[card.element] = (elementCounts[card.element] || 0) + quantity;
    ...
    const rarityDistribution = Object.entries(rarityCounts).map(([rarity, count]) => ({
      rarity,
      count: count as number,
      percentage: Math.round(((count as number) / totalCards) * 100),
    }));
  ...
  private detectArchetype(mainboard: any[], cards: Card[]): DeckArchetype | null {
    // Simple archetype detection based on card types and mana curve
    const creatureCount = cards.filter(card => card.type === CardType.CREATURE).length;
    const spellCount = cards.filter(card => card.type === CardType.SPELL).length;
    const avgManaCost = cards.reduce((sum, card) => sum + card.cost, 0) / cards.length;
  ...
  private calculateCompetitivenessScore(cards: Card[]): number {
    ...
    const avgMetaRating = cards.reduce((sum, card) => sum + (card.metaRating || 0), 0) / cards.length;
    return Math.min(100, avgMetaRating * 10);
  }

  private async checkLegality(mainboard: any[], format: DeckFormat): Promise<boolean> {
  ...
  private async findSimilarDecks(deckId: string, limit: number): Promise<Deck[]> {
