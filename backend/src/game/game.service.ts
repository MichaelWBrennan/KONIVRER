...
export class GameService {
  ...
  async createGame(createGameDto: CreateGameDto, player1Id: string): Promise<Game> {
    ...
    const initialGameState: GameState = {
      ...
      turnTimer: {
        ...
        ...(createGameDto.player2Id ? {
          [createGameDto.player2Id]: createGameDto.settings.timePerPlayer * 60
        } : {}),
      },
      gameActions: [],
    };
    ...
    const game = this.gameRepository.create({
      ...
      status: createGameDto.player2Id ? GameStatus.MULLIGAN_PHASE : GameStatus.WAITING_FOR_PLAYERS,
    });
  ...
  async joinGame(gameId: string, userId: string, joinDto: JoinGameDto): Promise<Game> {
  ...
  async findAll(filters: GameSearchFilters, userId?: string): Promise<{ games: Game[]; total: number; page: number; limit: number }> {
    const {
      page = 1,
      limit = 20,
      format,
    ...
    if (playerId) {
      queryBuilder.andWhere('(game.player1Id = :playerId OR game.player2Id = :playerId)', { playerId });
    }

    if (spectatable) {
      queryBuilder.andWhere('game.settings ->> \'enableSpectators\' = \'true\'');
      queryBuilder.andWhere('game.status = :inProgress', { inProgress: GameStatus.IN_PROGRESS });
    }
  ...
  async findOne(id: string, userId?: string): Promise<Game> {
    ...
    const canView = game.player1Id === userId || 
                   game.player2Id === userId ||
  ...
  async performAction(gameId: string, userId: string, actionDto: GameActionDto): Promise<Game> {
    const game = await this.gameRepository.findOne({ where: { id: gameId } });
  ...
  async submitMulliganDecision(gameId: string, userId: string, decision: MulliganDecisionDto): Promise<Game> {
    const game = await this.gameRepository.findOne({ where: { id: gameId } });
  ...
  private async processGameAction(game: Game, playerId: string, action: GameActionDto): Promise<GameState> {
  ...
  private processPlayCard(gameState: GameState, playerId: string, action: GameActionDto): GameState {
    ...
    this.addGameActionToState(gameState, playerId, 'play_card', action.data, `Played ${action.cardId}`);
    return gameState;
  }

  private processPassPriority(gameState: GameState, playerId: string): GameState {
    ...
    this.addGameActionToState(gameState, playerId, 'pass_priority', {}, 'Passed priority');
    return gameState;
  }

  private processActivateAbility(gameState: GameState, playerId: string, action: GameActionDto): GameState {
    // TODO: Implement ability activation
    this.addGameActionToState(gameState, playerId, 'activate_ability', action.data, 'Activated ability');
    return gameState;
  }

  private processDeclareAttackers(gameState: GameState, playerId: string, action: GameActionDto): GameState {
    // TODO: Implement attacker declaration
    this.addGameActionToState(gameState, playerId, 'declare_attackers', action.data, 'Declared attackers');
    return gameState;
  }

  private processDeclareBlockers(gameState: GameState, playerId: string, action: GameActionDto): GameState {
    // TODO: Implement blocker declaration
    this.addGameActionToState(gameState, playerId, 'declare_blockers', action.data, 'Declared blockers');
    return gameState;
  }

  private canPerformActionOutOfTurn(actionType: string, gameState: GameState): boolean {
    // Certain actions like instant spells or abilities can be performed at any time
    const instantActions = ['play_instant', 'activate_ability', 'respond_to_spell'];
  ...
  private addGameAction(game: Game, playerId: string, type: string, data: any, description: string): void {
  ...
  private addGameActionToState(gameState: GameState, playerId: string, type: string, data: any, description: string): void {
    const action: GameAction = {
      id: `action_${Date.now()}_${Math.random()}`,
      playerId,
      type,
