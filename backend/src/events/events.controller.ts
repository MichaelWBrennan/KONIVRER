...
export class EventsController {
  ...
  async findAll(@Query() filters: EventSearchFiltersDto, @Request() req): Promise<{
    events: Event[];
    total: number;
    page: number;
    limit: number;
  ...
  async findOne(@Param('id', ParseUUIDPipe) id: string, @Request() req): Promise<Event> {
  ...
  @ApiResponse({ status: 201, description: 'Registration successful', type: EventRegistration })
  @ApiResponse({ status: 400, description: 'Registration not available or invalid' })
  ...
  @HttpCode(HttpStatus.NO_CONTENT)
  async unregister(@Param('id', ParseUUIDPipe) eventId: string, @Request() req): Promise<void> {
    await this.eventsService.unregister(eventId, req.user.userId);
  }

  @Post(':id/checkin')
  @ApiOperation({ summary: 'Check in player for event' })
  @ApiResponse({ status: 200, description: 'Check-in successful', type: EventRegistration })
  @ApiResponse({ status: 400, description: 'Check-in not available' })
  @ApiResponse({ status: 403, description: 'Only organizers and judges can check in players' })
  @UseGuards(RolesGuard)
  @Roles(UserRole.TOURNAMENT_ORGANIZER, UserRole.JUDGE_L1, UserRole.JUDGE_L2, UserRole.JUDGE_L3, UserRole.ADMIN)
  async checkIn(
    @Param('id', ParseUUIDPipe) eventId: string,
  ...
  @ApiResponse({ status: 200, description: 'Pairings retrieved successfully', type: [Pairing] })
  ...
  @ApiResponse({ status: 403, description: 'Only organizers and judges can generate pairings' })
  @UseGuards(RolesGuard)
  @Roles(UserRole.TOURNAMENT_ORGANIZER, UserRole.JUDGE_L1, UserRole.JUDGE_L2, UserRole.JUDGE_L3, UserRole.ADMIN)
  ...
  @ApiResponse({ status: 403, description: 'Only organizers can publish pairings' })
  @UseGuards(RolesGuard)
  @Roles(UserRole.TOURNAMENT_ORGANIZER, UserRole.ADMIN)
  ...
  @ApiResponse({ status: 200, description: 'Matches retrieved successfully', type: [Match] })
  @ApiQuery({ name: 'round', required: false, type: Number })
  async getMatches(
    @Param('id', ParseUUIDPipe) eventId: string,
    @Query('round', new ParseIntPipe({ optional: true })) round?: number,
  ...
  @ApiResponse({ status: 200, description: 'Result reported successfully', type: Match })
  @ApiResponse({ status: 400, description: 'Invalid result or match already complete' })
  ...
  async reportResult(
    ...
    return this.eventsService.reportResult(match.id, resultDto, req.user.userId);
  }

  @Post('matches/:matchId/confirm')
  @ApiOperation({ summary: 'Judge confirms match result' })
  @ApiResponse({ status: 200, description: 'Result confirmed successfully', type: Match })
  @ApiResponse({ status: 403, description: 'Only judges can confirm results' })
  @UseGuards(RolesGuard)
  @Roles(UserRole.JUDGE_L1, UserRole.JUDGE_L2, UserRole.JUDGE_L3, UserRole.TOURNAMENT_ORGANIZER, UserRole.ADMIN)
  async confirmResult(
  ...
  @ApiResponse({ status: 201, description: 'Ruling applied successfully', type: Judging })
  ...
  async getStandings(@Param('id', ParseUUIDPipe) eventId: string): Promise<any[]> {
  ...
  async getStatistics(@Param('id', ParseUUIDPipe) eventId: string): Promise<any> {
    ...
    const completedMatches = pairings.flatMap(p => p.matches || [])
    ...
    return {
      ...
      eventDuration: event.endAt ? 
        Math.round((event.endAt.getTime() - event.startAt.getTime()) / (1000 * 60 * 60)) + ' hours' :
        'In progress',
    };
  }
  ...
  @ApiQuery({ name: 'data', required: false, enum: ['participants', 'pairings', 'results', 'standings', 'all'] })
  @ApiQuery({ name: 'round', required: false, type: Number })
  async exportData(
    @Param('id', ParseUUIDPipe) eventId: string,
    @Query('format') format: 'csv' | 'json' = 'json',
    @Query('data') data: 'participants' | 'pairings' | 'results' | 'standings' | 'all' = 'all',
    @Query('round', new ParseIntPipe({ optional: true })) round?: number,
  ): Promise<any> {
  ...
  @ApiOperation({ summary: 'Get updates since timestamp (polling fallback for WebSocket)' })
  ...
  async getUpdates(
    ...
    return {
      ...
      updates: {
        ...
        pairings: recentPairings.filter(p => 
          p.publishedAt && p.publishedAt > lastUpdate
        ),
        standings: event.updatedAt > lastUpdate ? 
          await this.eventsService.getStandings(eventId) : null,
      },
    };
  }
