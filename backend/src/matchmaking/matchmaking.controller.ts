import { Controller, Get, Post, Put, Body, Param, Query, ValidationPipe, UseGuards } from '@nestjs/common';
...
export class MatchmakingController {
  ...
  @ApiResponse({ status: 200, description: 'Ratings updated successfully', type: [PlayerRatingResponseDto] })
  async updateRatings(
  ...
  @ApiOperation({ summary: 'Generate optimal pairings using Bayesian matchmaking' })
  @ApiResponse({ status: 200, description: 'Pairings generated successfully', type: GeneratePairingsResponseDto })
  ...
  @ApiParam({ name: 'format', description: 'Game format (e.g., Standard, Modern)' })
  @ApiResponse({ status: 200, description: 'Player rating retrieved', type: PlayerRatingResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Match quality calculated', type: MatchQualityResponseDto })
  async calculateMatchQuality(
    ...
    return this.matchmakingService.calculateMatchQuality(player1Id, player2Id, format);
  }

  @Post('simulate')
  @ApiOperation({ summary: 'Simulate match outcomes based on player ratings' })
  @ApiResponse({ status: 200, description: 'Match simulation completed', type: SimulationResultDto })
  async simulateMatch(
  ...
  @ApiResponse({ status: 200, description: 'Leaderboard retrieved', type: [PlayerRatingResponseDto] })
