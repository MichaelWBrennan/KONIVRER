...
export class KonivrRulesSearchService {
  ...
  async getRulesByKeyword(keyword: string): Promise<KonivrRule[]> {
    ...
    return ruleIds.map(id => this.rulesIndex.get(id)).filter(Boolean) as KonivrRule[];
  }
    ...
    const rules: KonivrRule[] = [
      {
        ...
        elements: [],
        examples: ['1v1 battles', '2v2 team battles', 'free-for-all multiplayer'],
      ...
      },
      {
        ...
        keywords: ['phases', 'turn structure', 'start', 'main', 'combat', 'refresh'],
        elements: [],
        examples: ['Draw 2 cards first turn', 'Play cards in Main', 'Attack in Combat'],
      ...
      },
      {
        ...
        keywords: ['elements', 'fire', 'water', 'earth', 'air', 'aether', 'nether', 'generic'],
        elements: ['🜂', '🜄', '🜃', '🜁', '⭘', '▢', '✡⃝'],
      ...
      },
      {
        ...
        keywords: ['play modes', 'summon', 'spell', 'azoth', 'tribute', 'burst'],
        elements: [],
      ...
      },
      {
        ...
        text: 'After damage dealt, add extra damage ≤ 🜂 paid. Does not affect 🜄 cards.',
        keywords: ['inferno', 'damage', 'fire'],
        elements: ['🜂', '🜄'],
        examples: ['Extra fire damage', 'Water immunity'],
        relatedRules: ['KR-700.5', 'KR-500.1'],
        category: 'keywords',
      },
      {
        id: 'KR-800.1',
        ...
        keywords: ['zones', 'field', 'combat row', 'azoth row', 'removed from play'],
        elements: [],
  ...
  private calculateRelevance(rule: KonivrRule, searchTerms: string[]): number {
    ...
    for (const term of searchTerms) {
      ...
      if (rule.elements.some(e => e.includes(term) || e.toLowerCase().includes(term))) {
        score += 7;
      }
    }
  ...
  private findRuleInteractions(rule: KonivrRule): string[] {
    ...
    if (rule.keywords.includes('brilliance')) {
      interactions.push('Interacts with Aether (⭘) costs and Life Cards placement');
    }
    if (rule.keywords.includes('gust')) {
      interactions.push('Interacts with Air (🜁) costs and returns cards to hand');
    }
